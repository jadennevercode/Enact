...
The `enact` CLI connects your local machine to Enact. It handles authentication, workspace management, issue tracking, and runs the agent daemon that executes AI tasks locally.
...
```bash
git clone https://github.com/enact-ai/enact.git
cd enact
make build
cp server/bin/enact /usr/local/bin/enact
```
...
`enact update` auto-detects your installation method and upgrades accordingly.
...
`enact login` automatically discovers all workspaces you belong to and adds them to the daemon watch list.
...
The daemon is the local agent runtime. It detects available AI CLIs on your machine, registers them with the Enact server, and executes tasks when agents are assigned work.
...
directory of the profile it was started with — **not always `~/.enact/`**:

| Profile | State directory |
| --- | --- |
| Default (no `--profile`) | `~/.enact/` |
| Named (`--profile <name>`) | `~/.enact/profiles/<name>/` |
...
both, `~/.enact/daemon.log` and `~/.enact/profiles/<name>/daemon.log` both
exist and both read as plausible logs — only one is being written to. Don't
guess: `enact daemon logs` prints the absolute path it resolved (see
[Logs](#logs)).

To run in the foreground (useful for debugging):

```bash
enact daemon start --foreground
```

#### Following a replaced binary

A CLI-launched daemon periodically compares its own compile-time version against
the `--version` output of the `enact` binary it would re-exec. When they differ
— `brew upgrade enact`, a re-download, a local `make build` — it waits for any
...
check, and `enact daemon status` shows why it's still on the old version.
...
```bash
ENACT_DAEMON_AUTO_RELOAD=0 enact daemon start
# or
enact daemon start --no-auto-reload
# or persist it
enact config set disable_auto_reload true
```
...
**without restarting**, so subsequent tasks pick up the new CLI while Enact's
...
```bash
enact daemon logs              # Last 50 lines
enact daemon logs -f           # Follow (tail -f)
enact daemon logs -n 100       # Last 100 lines
enact daemon logs --profile staging
```
...
```
$ enact daemon logs -n 100
Reading /Users/you/.enact/profiles/desktop-mbp/daemon.log (profile: desktop-mbp)
...
```
...
```bash
enact daemon logs -n 500 | grep ERROR   # the path line is not in the pipe
```
...
`enact daemon status --profile <name>` confirms which one is live.
...
| [DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness) | `dsh` | DeepSeek Harness (`dsh --profile enact --stdio`; requires the Enact runtime profile to be installed; reads AGENTS.md and .dsh/skills/) |
...
|---------|------|--------------|---------|
| Poll interval | `--poll-interval` | `ENACT_DAEMON_POLL_INTERVAL` | `3s` |
| Heartbeat interval | `--heartbeat-interval` | `ENACT_DAEMON_HEARTBEAT_INTERVAL` | `15s` |
| Agent timeout | `--agent-timeout` | `ENACT_AGENT_TIMEOUT` | `0` (no cap; bounded by the watchdogs) |
| Codex semantic inactivity timeout | `--codex-semantic-inactivity-timeout` | `ENACT_CODEX_SEMANTIC_INACTIVITY_TIMEOUT` | `10m` |
| OpenCode idle watchdog | — | `ENACT_OPENCODE_IDLE_WATCHDOG` | `10m` (`0` falls back to the generic idle watchdog; cannot extend it) |
| Max concurrent tasks | `--max-concurrent-tasks` | `ENACT_DAEMON_MAX_CONCURRENT_TASKS` | `20` |
| Daemon ID | `--daemon-id` | `ENACT_DAEMON_ID` | hostname |
| Device name | `--device-name` | `ENACT_DAEMON_DEVICE_NAME` | hostname |
| Runtime name | `--runtime-name` | `ENACT_AGENT_RUNTIME_NAME` | `Local Agent` |
| Workspaces root | — | `ENACT_WORKSPACES_ROOT` | `~/enact_workspaces` |
| GC enabled | — | `ENACT_GC_ENABLED` | `true` (set `false`/`0` to disable) |
| GC scan interval | — | `ENACT_GC_INTERVAL` | `2h` |
| GC TTL (done/cancelled issues) | — | `ENACT_GC_TTL` | `24h` |
| GC completed-task TTL (issue tasks) | — | `ENACT_GC_COMPLETED_TASK_TTL` | `14d` on Enact Cloud, `0` (disabled) elsewhere |
| GC orphan TTL (no `.gc_meta.json`) | — | `ENACT_GC_ORPHAN_TTL` | `72h` |
| GC artifact TTL (completed tasks) | — | `ENACT_GC_ARTIFACT_TTL` | `12h` (set `0` to disable) |
| GC artifact patterns | — | `ENACT_GC_ARTIFACT_PATTERNS` | `node_modules,.next,.turbo` |
| GC repo cache TTL (`.repos`) | — | `ENACT_GC_REPO_TTL` | `720h` (30d; set `0` to disable) |
| GC repo maintenance | — | `ENACT_GC_REPO_MAINTENANCE_ENABLED` | `true` (set `false`/`0` to disable heavy Git maintenance only) |
| GC Hermes memory TTL (per-agent `memories/`) | — | `ENACT_GC_HERMES_MEMORY_TTL` | `2160h` (90d; set `0` to disable) |
| GC Hermes session TTL (per-conversation `state.db`) | — | `ENACT_GC_HERMES_SESSION_TTL` | `336h` (14d; set `0` to disable) |
| GC task temp legacy TTL (pre-lock `enact-task-*`) | — | `ENACT_GC_TASK_TEMP_LEGACY_TTL` | `0` (disabled; set a duration to opt in) |

#### Workspace garbage collection

The daemon periodically scans `ENACT_WORKSPACES_ROOT` and applies several disk-reclamation policies:

- **Full task cleanup** — when an issue's status is `done` or `cancelled` and has been idle for `ENACT_GC_TTL`, the entire task directory is removed.
- **Completed-task retention bound** — `ENACT_GC_COMPLETED_TASK_TTL` fully removes an inactive issue task once its `.gc_meta.json` `completed_at` age exceeds the configured duration, even while the parent issue remains open. Cleanup waits for a successful parent-issue status check, never removes an active environment, and never fully removes a `local_directory` environment. A later rerun provisions a fresh environment instead of resuming the removed checkout.
  - The default depends on where the daemon points: `14d` against Enact Cloud, and `0` (disabled, retain indefinitely) for self-host and every other origin — including cloud staging and previews. Set the variable to opt in or out on either side; an explicit `0` disables the policy on Cloud too.
  - Removing an environment discards work an agent left uncommitted or unpushed on its branch, along with that task's `output/` and `logs/`. The per-issue Codex session store lives outside `ENACT_WORKSPACES_ROOT` under its own TTL, so a later rerun still resumes the agent's prior session — it just starts from a fresh checkout. Size the TTL against that trade, and keep it comfortably above `ENACT_GC_INTERVAL`: the active-root guard protects a task that is currently running, not one whose follow-up run is queued but unclaimed.
- **Orphan cleanup** — task directories with no `.gc_meta.json` (e.g. left over from a daemon crash) are removed once they exceed `ENACT_GC_ORPHAN_TTL`.
- **Artifact-only cleanup** — when a task has been completed for at least `ENACT_GC_ARTIFACT_TTL` but the issue is still open, regenerable build outputs whose directory basename matches `ENACT_GC_ARTIFACT_PATTERNS` are removed. The daemon also reclaims the exact managed path `codex-home/.sandbox-bin`; old task metadata without `completed_at` becomes eligible for this managed-only cleanup after its `.gc_meta.json` file has been idle for `ENACT_GC_ORPHAN_TTL`. The rest of the task (source, `.git`, `output/`, `logs/`, `.gc_meta.json`, Codex auth/config/session state) is preserved so the agent can resume it.
- **Managed-cache reclamation** — the exact managed path above is reclaimed for *every* task kind once the task has been completed for `ENACT_GC_ARTIFACT_TTL`, not just for issue tasks whose issue is still open. It applies even while the parent record says the directory itself must stay — an active chat session, a still-running autopilot run — and even when the parent record could not be reached this cycle, because the contents are regenerable and the next run re-provisions them on demand. A task currently running on the directory is never touched. Set `ENACT_GC_ARTIFACT_TTL=0` to disable this along with the rest of artifact cleanup.

- **Repo cache eviction** — the bare git clones under `.repos/` are shared object stores: each task workdir is a `git worktree` off one of them rather than its own clone, so a task's `.git` is only a pointer file. They are evicted only when all of the following hold: the repo is no longer attached to any workspace this daemon watches, it has no worktrees left, and no task has created a worktree from it for `ENACT_GC_REPO_TTL`. A cache created before this stamp existed is not treated as ancient — its clock starts at the first GC cycle that sees it, so upgrading does not wipe every cache. Evicting is safe by construction: the next task that needs the repo re-clones it on demand, so a wrong eviction costs a clone, not a failure.

  Short worktree cleanup and eligible cache eviction continue on every GC cycle, including while agents are active. Heavy repo maintenance (`reflog expire` and `git gc`) starts only while the daemon is otherwise idle. A checkout or newly claimed task cancels it and takes priority; interrupted work remains pending for a later idle GC cycle. Operators can disable only these heavy commands with `ENACT_GC_REPO_MAINTENANCE_ENABLED=false` without disabling worktree cleanup or cache eviction.

- **Hermes session store reclamation** — a conversation's Hermes transcript (`state.db`) lives at `<profile dir>/hermes-sessions/<agent-id>/<hermes-profile>/<conversation>/`, outside any task directory, so a follow-up turn can resume it (see [Hermes agent memory](#hermes-agent-memory)). A store untouched for `ENACT_GC_HERMES_SESSION_TTL` is removed. The default matches the Codex session store rather than the memory store above: these hold full transcripts, and reclaiming an idle one costs a thread that starts fresh (with a continuity notice), not an agent that forgot what it learned. A store a running task holds is never reclaimed.
- **Hermes memory store reclamation** — a Hermes agent's long-term memory (`memories/`) lives at `<profile dir>/hermes-state/<agent-id>/<hermes-profile>/`, outside any task directory, so it survives across tasks and issues (see [Hermes agent memory](#hermes-agent-memory)). A store untouched for `ENACT_GC_HERMES_MEMORY_TTL` is removed, giving a deleted agent's memory an eventual-reclamation guarantee. The default is deliberately long: these are a handful of markdown files, and reclaiming one is user-visible amnesia rather than a cache miss. A store a running task holds is never reclaimed.
- **Task temp dir reclamation** — every task gets a private temp directory (`enact-task-*`) under the system temp base (`/tmp`, or `ENACT_AGENT_TEMP_BASE`), exported to the agent as `TMPDIR`/`TMP`/`TEMP`. It is removed when the run ends, but that removal never happens when the daemon is killed and does not succeed while a file inside is still open — common on Windows, where an open handle makes the delete fail outright. These directories live outside `ENACT_WORKSPACES_ROOT`, so nothing else reclaimed them and whatever the end-of-run removal missed accumulated forever. Every GC cycle now sweeps the temp base. Liveness is decided by the directory's `.task_lock` — the same OS advisory lock an env root uses, which the kernel releases when the holding process dies — not by age: a directory still in use is never removed however old it is, including one owned by a different daemon sharing the same temp base, and a directory whose owner is gone is removed on the next cycle however new it is. Directories left by a daemon predating that lock carry no lock file, so nothing can be proven about them and age is the only signal available. Reclaiming those is an operator's explicit decision: `ENACT_GC_TASK_TEMP_LEGACY_TTL` defaults to `0`, which leaves them in place. Set it to a duration only once you know no pre-lock daemon is still running tasks on this machine — a task may legitimately run for weeks (there is no default agent timeout), a daemon on another profile can still be on the old binary, and every daemon on the machine shares one temp base, so a TTL here can delete a `TMPDIR` that is still in use. Each GC cycle logs how many such directories it left alone. Even with a TTL set, a directory holding no task content is never reclaimed on age — an old empty leftover, or a shell left by a daemon that died between creating the directory and publishing its lock — because holding no content is exactly what a directory currently being published looks like, and deleting one of those would take the `TMPDIR` of a task that is starting. Those shells are a few bytes each. Only entries carrying the `enact-task-` prefix are ever considered — the temp base itself is usually shared with other programs — and a directory this sweep cannot read is never touched.

Configured patterns are basename-only — entries containing `/` or `\` are silently dropped — and `.git` subtrees are never descended into. The managed Codex cache is matched by its exact relative path, so a repository's own `.sandbox-bin` is not removed unless an operator explicitly adds that basename to `ENACT_GC_ARTIFACT_PATTERNS`. The default list (`node_modules`, `.next`, `.turbo`) is intentionally narrow; extend it per deployment if your repos consistently produce other regenerable directories (for example, `ENACT_GC_ARTIFACT_PATTERNS=node_modules,.next,.turbo,target,__pycache__`). To disable artifact cleanup entirely, including the managed Codex cache, set `ENACT_GC_ARTIFACT_TTL=0`.

`enact daemon disk-usage` reports the `.repos` footprint on its own line rather than folding it into the per-task totals — every task in a workspace checks out from that shared cache, so attributing it to individual task directories would double-count it. Note that the repo cache is reclaimed on the schedule above and not by any per-issue status change, so it is normal for it to persist after every task directory is gone.

Agent-specific overrides:

| Variable | Description |
|----------|-------------|
| `ENACT_CLAUDE_PATH` | Custom path to the `claude` binary |
| `ENACT_CLAUDE_MODEL` | Override the Claude model used |
| `ENACT_CLAUDE_ARGS` | Default extra arguments for Claude Code runs |
| `ENACT_ANTIGRAVITY_PATH` | Custom path to the `agy` binary |
| `ENACT_ANTIGRAVITY_MODEL` | Override the Antigravity model used |
| `ENACT_CODEBUDDY_PATH` | Custom path to the `codebuddy` binary |
| `ENACT_CODEBUDDY_MODEL` | Override the CodeBuddy model used |
| `ENACT_CODEBUDDY_ARGS` | Default extra arguments for CodeBuddy runs |
| `ENACT_DEVECO_PATH` | Custom path to the `deveco` binary |
| `ENACT_DEVECO_MODEL` | Override the DevEco Code model used |
| `ENACT_CODEX_PATH` | Custom path to the `codex` binary |
| `ENACT_CODEX_MODEL` | Override the Codex model used |
| `ENACT_CODEX_ARGS` | Default extra arguments for Codex runs |
| `ENACT_COPILOT_PATH` | Custom path to the `copilot` binary |
| `ENACT_COPILOT_MODEL` | Override the Copilot model used (note: GitHub Copilot routes models through your account entitlement, so this may not be honoured) |
| `ENACT_OPENCODE_PATH` | Custom path to the `opencode` binary |
| `ENACT_OPENCODE_MODEL` | Override the OpenCode model used |
| `ENACT_OPENCLAW_PATH` | Custom path to the `openclaw` binary |
| `ENACT_OPENCLAW_MODEL` | Override the OpenClaw model used |
| `ENACT_OPENCLAW_CLI_TIMEOUT` | Deadline for each `openclaw config ...` call during task preparation (default 30s; accepts `45s` or `45`). Raise it when the local CLI is slow to start; the daemon also reads it from `backends.openclaw.cli_timeout` in the CLI config |
| `ENACT_HERMES_PATH` | Custom path to the `hermes` binary |
| `ENACT_HERMES_MODEL` | Override the Hermes model used |
| `ENACT_PI_PATH` | Custom path to the `pi` binary |
| `ENACT_PI_MODEL` | Override the Pi model used |
| `ENACT_CURSOR_PATH` | Custom path to the `cursor-agent` binary |
| `ENACT_CURSOR_MODEL` | Override the Cursor Agent model used |
| `ENACT_KIMI_PATH` | Custom path to the `kimi` binary |
| `ENACT_KIMI_MODEL` | Override the Kimi model used |
| `ENACT_REASONIX_PATH` | Custom path to the `reasonix` binary |
| `ENACT_REASONIX_MODEL` | Override the Reasonix model used |
| `ENACT_DIM_PATH` | Custom path to the `dim` binary |
| `ENACT_DIM_MODEL` | Override the Dim model used |
| `ENACT_KIRO_PATH` | Custom path to the `kiro-cli` binary |
| `ENACT_KIRO_MODEL` | Override the Kiro model used |
| `ENACT_QODER_PATH` | Custom path to the `qodercli` binary |
| `ENACT_QODER_MODEL` | Override the Qoder model used |
| `ENACT_QODERCLICN_PATH` | Custom path to the `qoderclicn` binary |
| `ENACT_QODERCLICN_MODEL` | Override the Qoder CN model used |
| `ENACT_TRAECLI_PATH` | Custom path to the `traecli` binary |
| `ENACT_TRAECLI_MODEL` | Override the Trae model used (a model id from your logged-in traecli catalog, e.g. `Doubao-Seed-2.1-Pro`) |
| `ENACT_GROK_PATH` | Custom path to the `grok` binary (defaults to `grok` on PATH; often `~/.grok/bin/grok`) |
| `ENACT_GROK_MODEL` | Override the Grok model used (e.g. `grok-4.5`) |
| `ENACT_QWEN_PATH` | Custom path to the `qwen` binary |
| `ENACT_QWEN_MODEL` | Override the Qwen Code model used |
| `ENACT_QWEN_ARGS` | Daemon-wide extra Qwen arguments (POSIX shellword parsing; managed protocol flags are filtered) |
| `ENACT_QWENPAW_PATH` | Custom path to the `qwenpaw` binary |
| `ENACT_QWENPAW_ARGS` | Daemon-wide extra QwenPaw arguments (POSIX shellword parsing; managed protocol flags are filtered) |
| `ENACT_MCODE_PATH` | Custom path to the `mcode` binary |
| `ENACT_DSH_PATH` | Custom path to the `dsh` binary |
| `ENACT_DSH_MODEL` | Override the DeepSeek Harness model used (a model id from the dsh catalog, e.g. `deepseek-official/deepseek-chat`) |

If a previously generated `~/.enact/hooks` wrapper is first on `PATH` and calls the same command name again, the daemon skips that hooks directory during built-in agent discovery and records the real binary path behind it. If your interactive shell still recurses when you run `claude`, `codex`, or `hermes` manually, remove the hooks entry from your shell startup file or replace the wrapper body with an absolute `exec /path/to/real-binary "$@"`.
...
The daemon launches QwenPaw as `qwenpaw acp --workspace <per-task dir>`. It writes the task brief to `AGENTS.md`, and materialises the run's bound skills into `<per-task dir>/skills/` plus a `skill.json` manifest, so QwenPaw discovers them through its own workspace skill discovery. `acp` and `--workspace` are reserved: `custom_args` cannot override them. QwenPaw is the one runtime with no `ENACT_QWENPAW_MODEL`: its `session/set_model` writes to a shared, persistent agent config rather than the session, so Enact never sends it a model and leaves that choice to QwenPaw's own configuration.

The daemon launches MiniMax Code as `mcode acp`, writes the task brief to `AGENTS.md`, and injects bound skills under `.minimax/skills/`. MCode owns model selection and currently advertises `loadSession: false`; Enact therefore starts a fresh MCode session when a later run cannot load the saved session.

#### Hermes agent memory

Hermes discovers skills only from its own home, so binding Enact skills to a Hermes agent makes the daemon build a per-task `HERMES_HOME` overlay for that agent. The agent's long-term memory (`memories/`) does **not** live inside that task-scoped overlay: it is linked to a persistent store at
...
- **Every Hermes agent gets the overlay in practice**, so every one of them gets a persistent memory store. The daemon builds the overlay only when a task carries skills, but the server appends Enact's built-in skills to every agent's skill set (`LoadAgentSkillBundles`), so that list is never empty — leaving an agent's own skill list empty does not opt out of the overlay, and is not a way to keep using the host's `~/.hermes/memories`.

`ENACT_CLAUDE_ARGS`, `ENACT_CODEX_ARGS`, `ENACT_CODEBUDDY_ARGS`, `ENACT_QWEN_ARGS`, and `ENACT_QWENPAW_ARGS` are parsed with POSIX shellword quoting, so values such as `--model "gpt-5.1 codex" --sandbox read-only` are split like a shell command line. Agent arguments are applied in this order: hardcoded Enact defaults, daemon-wide env defaults, then per-agent `custom_args` from the task.

### Self-Hosted Server

When connecting to a self-hosted Enact instance, the easiest approach is:
...
enact config set server_url http://localhost:8080
enact config set app_url http://localhost:3000

# For production with TLS:
# enact config set server_url https://api.example.com
# enact config set app_url https://app.example.com
...
enact setup self-host --profile staging --server-url https://api-staging.example.com --app-url https://staging.example.com

# Start its daemon
enact daemon start --profile staging

# Default profile runs separately
enact daemon start
```

Each profile gets its own config directory (`~/.enact/profiles/<name>/`), daemon state, health port, and workspace root. Daemon state means that profile's own `daemon.log`, `daemon.err.log`, and `daemon.pid` live in that directory too — see [Start](#start) for the layout, and pass `--profile <name>` to `daemon status` / `daemon logs` to act on it.
...
2. `ENACT_WORKSPACE_ID` environment variable
3. The default workspace stored in your current profile (set by `enact workspace switch` or `enact login`)

`enact workspace switch <id|slug>` is the day-to-day way to change the default workspace. For scripting and headless setups where you don't want any stored state, prefer the `--workspace-id` flag or the env variable. `enact config set workspace_id <id>` is the low-level equivalent of `switch` (it writes the same setting but skips the access check).
...
enact workspace list --full-id
enact workspace list --output json
```
...
```bash
enact workspace switch <workspace-id>
enact workspace switch <slug>
```

Verifies you have access to the workspace, then sets it as the default for the current profile. Subsequent commands without `--workspace-id` and `ENACT_WORKSPACE_ID` target this workspace. Pair `--profile` if you want to change a non-default profile's workspace.

### Get Details

```bash
enact workspace get <workspace-id>
enact workspace get <workspace-id> --output json
```

Passing no `<workspace-id>` resolves to the current default workspace, so `enact workspace get` doubles as "what workspace am I on?".

### List Members

```bash
enact workspace member list <workspace-id>
```
...
```bash
enact issue list
enact issue list --status in_progress
enact issue list --priority urgent --assignee "Agent Name"
enact issue list --assignee-id 5fb87ac7-23b5-4a7a-81fa-ed295a54545d
enact issue list --full-id
enact issue list --limit 20 --output json
enact issue list --status todo --sort position       # board order (the default)
...
```bash
enact issue list --metadata pipeline_status=waiting_review
enact issue list --metadata pr_number=482 --metadata is_blocked=true
```
...
```bash
enact issue get <id>
enact issue get <id> --output json
```

### Create Issue

```bash
enact issue create --title "Fix login bug" --description "..." --priority high --assignee "Lambda"
...
```bash
enact issue update <id> --title "New title" --priority urgent
enact issue update <id> --position 4.5
```
...
```bash
enact issue reorder <id> --top              # top of its status column
enact issue reorder <id> --bottom           # bottom of its status column
enact issue reorder <id> --before <other>   # directly above another issue in the same column
enact issue reorder <id> --after  <other>   # directly below another issue in the same column
```
...
```bash
enact issue assign <id> --to "Lambda"
enact issue assign <id> --to-id 5fb87ac7-23b5-4a7a-81fa-ed295a54545d
enact issue assign <id> --unassign
```
...
```bash
enact issue status <id> in_progress
```
...
enact issue comment list <issue-id>
...
enact issue comment list <issue-id> --thread <comment-id>
...
enact issue comment list <issue-id> --thread <comment-id> --tail 30
...
enact issue comment list <issue-id> --recent 10
...
# Add a comment
enact issue comment add <issue-id> --content "Looks good, merging now"

# Reply to a specific comment
enact issue comment add <issue-id> --parent <comment-id> --content "Thanks!"

# Delete a comment
enact issue comment delete <comment-id>
```
...
no-op. The server emits the cursor headers (`X-Enact-Next-Before` /
`X-Enact-Next-Before-Id`) only when an older page actually exists —
...
enact issue metadata list <issue-id>

# Read a single key
enact issue metadata get <issue-id> --key pipeline_status

# Write a single key — value auto-typed (true/false → bool, numbers → number, else string)
enact issue metadata set <issue-id> --key pipeline_status --value waiting_review
enact issue metadata set <issue-id> --key pr_number --value 482
enact issue metadata set <issue-id> --key is_blocked --value true

# Force a specific type when sniffing would pick the wrong one
enact issue metadata set <issue-id> --key code --value 42 --type string

# Remove a key
enact issue metadata delete <issue-id> --key pipeline_status
```

All writes are single-key atomic — concurrent agents writing different keys do not lose each other's updates. To query, use `enact issue list --metadata key=value` (see *List Issues* above).

### Subscribers

```bash
# List subscribers of an issue
enact issue subscriber list <issue-id>

# Subscribe yourself to an issue
enact issue subscriber add <issue-id>

# Subscribe another member or agent by name
enact issue subscriber add <issue-id> --user "Lambda"

# Unsubscribe yourself
enact issue subscriber remove <issue-id>

# Unsubscribe another member or agent
enact issue subscriber remove <issue-id> --user "Lambda"
```
...
enact issue runs <issue-id>
enact issue runs <issue-id> --full-id
enact issue runs <issue-id> --output json

# View messages for a specific execution run
enact issue run-messages <task-id>
enact issue run-messages <short-task-id> --issue <issue-id>
enact issue run-messages <task-id> --output json

# Incremental fetch (only messages after a given sequence number)
enact issue run-messages <task-id> --since 42 --output json

# Aggregated token usage for an issue (sum across all its task runs)
enact issue usage <issue-id>
enact issue usage <issue-id> --output json
```
...
```bash
# One-command setup for Enact Cloud: configure, authenticate, and start the daemon
enact setup
...
# Custom ports
enact setup self-host --port 9090 --frontend-port 4000

# On-premise with custom domains
enact setup self-host --server-url https://api.example.com --app-url https://app.example.com
```

`enact setup` configures the CLI, opens your browser for authentication, and starts the daemon — all in one step. Use `enact setup self-host` to connect to a self-hosted server instead of Enact Cloud.
...
enact config set server_url https://api.example.com
enact config set app_url https://app.example.com
enact config set workspace_id <workspace-id>
```

`config set workspace_id <id>` is the low-level interface — it writes the value verbatim without checking that the workspace exists or that you have access. Prefer `enact workspace switch <id|slug>` for day-to-day workspace changes; it does both checks before saving.
...
```bash
enact autopilot list
enact autopilot list --full-id
enact autopilot list --status active --output json
```
...
```bash
enact autopilot get <id>
enact autopilot get <id> --output json   # includes triggers
```

### Create / Update / Delete

```bash
enact autopilot create \
...
enact autopilot update <id> --status paused
enact autopilot update <id> --description "New prompt"
enact autopilot update <id> --subscriber "Alice" --subscriber "Bob"
enact autopilot update <id> --clear-subscribers
enact autopilot delete <id>
```
...
```bash
enact autopilot trigger <id>            # Fires the autopilot once, returns the run
```

### Run History

```bash
enact autopilot runs <id>
enact autopilot runs <id> --limit 50 --output json
```

### Schedule Triggers

```bash
enact autopilot trigger-add <autopilot-id> --cron "0 9 * * 1-5" --timezone "America/New_York"
enact autopilot trigger-update <autopilot-id> <trigger-id> --enabled=false
enact autopilot trigger-delete <autopilot-id> <trigger-id>
```
...
```bash
enact version              # Show CLI version and commit hash
enact update               # Update to latest version
enact agent list           # List agents in the current workspace
```
...
- **Friendly, single-line message.** Transport failures (timeout, DNS,
  ...
  `ENACT_HTTP_TIMEOUT`, and a 401 tells you to run `enact login`.
...
```bash
LANG=zh_CN.UTF-8 enact issue get ENA-9999   # 错误信息显示为中文
```
...
```bash
enact issue get ENA-9999
if [ $? -eq 4 ]; then echo "no such issue"; fi
```

### Seeing the full detail (`--debug`)

Pass the global `--debug` flag (or set `ENACT_DEBUG=1`) to print the complete
...
```bash
enact issue list --debug
ENACT_DEBUG=1 enact issue update ENA-1234 --title "x"
```

### Request timeout

API requests use a default timeout of 30 seconds. Override it with
`ENACT_HTTP_TIMEOUT` when you are on a slow network; it accepts a Go duration
...
```bash
ENACT_HTTP_TIMEOUT=60s enact issue list
```
