...
What a Enact plugin surface imports.

```js
import { enact } from "@enact/plugin-sdk";

const ctx   = await enact.context.get();
const issue = await enact.issue.get();
await enact.issue.comment({ body: "hello" });
const note  = await enact.storage.user.get("note");
enact.ui.resize(320);
```
...
is no module graph: you publish an artifact, Enact stores it and inlines your
...
- **No browser storage.** `localStorage`, `sessionStorage` and cookies all throw
  or are empty. Use `enact.storage` — it is server-side, scoped per workspace
...
- **A CSP you did not write.** The host generates the document and derives
  ...
  policy now that Enact serves your code. `net:` is an exact host, so declare
...
`EnactPluginError` with a `status` mirroring HTTP.
...
`enact.ui.onThemeChange(fn)` if you need to react in JS.

## Sizing

The frame does not auto-size. Call `enact.ui.resize(px)` after your content
