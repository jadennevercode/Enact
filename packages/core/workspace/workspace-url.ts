// Brand host shown as the workspace URL prefix on the managed Enact Cloud,
...
// self-hosted server that has not set ENACT_APP_URL / FRONTEND_ORIGIN), so
// this literal must remain the ultimate fallback.
const BRAND_WORKSPACE_HOST = "enact.ai";
 ...
 * self-hosted instances show their own domain instead of `enact.ai`. Falls
