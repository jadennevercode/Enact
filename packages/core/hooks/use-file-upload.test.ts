...
function makeAttachment(overrides: Partial<Attachment> = {}): Attachment {
  return {
    ...
    markdown_url: "https://api.enact.test/api/attachments/att-1/download",
...
describe("useFileUpload — markdownLink picks the durable URL with three-layer fallback", () => {
  it("prefers att.markdown_url when the server populates it (modern deployment)", async () => {
    const att = makeAttachment({
      markdown_url: "https://cdn.enact.test/uploads/abc.png",
    });
    const upload = await runUpload(makeApi(att));
    expect(upload?.markdownLink).toBe("https://cdn.enact.test/uploads/abc.png");
  ...
  it("falls back to att.url when there's no attachment-row id (no-workspace avatar branch)", async () => {
    const att = makeAttachment({
      id: "",
      markdown_url: "",
      url: "https://cdn.enact.test/avatars/u-1.png",
    });
    const upload = await runUpload(makeApi(att));
    expect(upload?.markdownLink).toBe("https://cdn.enact.test/avatars/u-1.png");
    expect(upload?.link).toBe("https://cdn.enact.test/avatars/u-1.png");
  });
