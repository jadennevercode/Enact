...
const detail = z.object({ construction: constructionSchema, issue, tasks: z.array(task).catch([]), events: z.array(record).catch([]), model_operations: z.array(record).catch([]) }).transform(v => ({ ...v, modelOperations: v.model_operations }));
