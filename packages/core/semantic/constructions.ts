...
const task = z.object({ id: z.string(), issue_id: z.string() }).passthrough().transform(v => ({ ...v, issueId: v.issue_id }));
export const constructionDetailSchema = z.object({ construction: constructionSchema, issue, tasks: z.array(task).catch([]), events: z.array(record).catch([]), model_operations: z.array(record).catch([]) }).transform(v => ({ ...v, modelOperations: v.model_operations }));
export const constructionApi = {
  ...
  detail: (id: string) => semanticRequest(`/constructions/${encodeURIComponent(id)}`, constructionDetailSchema),
