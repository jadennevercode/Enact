...
describe("clearWorkspaceStorage", () => {
  it("removes all non-draft workspace-scoped keys for the given slug", () => {
    ...
    expect(adapter.removeItem).toHaveBeenCalledWith("enact_issue_surface_views:ws_123");
    expect(adapter.removeItem).toHaveBeenCalledWith("enact_issues_view:ws_123");
  ...
  it("also clears registered draft keys via the registry", () => {
    ...
    registerDraftCleanup({
      storageKey: "enact_test_draft",
      workspaceScoped: true,
      resetInMemory: vi.fn(),
    });
    registerDraftCleanup({
      storageKey: "enact_test_global_draft",
    ...
    expect(adapter.removeItem).toHaveBeenCalledWith("enact_test_draft:ws_123");
