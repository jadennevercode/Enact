 ...
 * Enact agent. Wire shape mirrors `WecomInstallationResponse` in
...
export interface ListWecomInstallationsResponse {
  ...
  /** Whether ENACT_WECOM_SECRET_KEY is set on this deployment. When false the
 ...
 * with the deployment's ENACT_WECOM_SECRET_KEY before writing it, so
...
export interface RegisterWecomBYORequest {
   ...
   * containing a space ("Enact Bot") otherwise swallows the slash command
 ...
 * bound to the logged-in Enact user in this workspace/installation. */
