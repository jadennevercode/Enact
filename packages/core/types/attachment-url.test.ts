...
describe("stripChannelMediaMarkers", () => {
  it("removes provenance while retaining the visible channel image", () => {
    ...
    const marker = `<!-- enact:channel-media:${ID} -->`;
...
describe("contentReferencesAttachment", () => {
  const att = {
    ...
    markdown_url: `https://enact-api.copilothub.ai/api/attachments/${ID}/download`,
  };
  ...
  // (`<ENACT_PUBLIC_URL>/api/attachments/<id>/download`) into the body,
