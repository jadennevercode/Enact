...
function daemonCommands(serverUrl: string | undefined, appUrl: string | undefined) {
  ...
  if (normalizedServerUrl && normalizedAppUrl) {
    return {
      setupCmd: `enact setup self-host --server-url ${normalizedServerUrl} --app-url ${normalizedAppUrl}`,
      tokenCmd: `enact config set server_url ${normalizedServerUrl}
enact config set app_url ${normalizedAppUrl}
enact login --token <YOUR_TOKEN>
enact daemon start`,
    };
  }

  return {
    setupCmd: "enact setup",
    tokenCmd: `enact config set server_url ${CLOUD_SERVER_URL}
enact config set app_url ${CLOUD_APP_URL}
enact login --token <YOUR_TOKEN>
enact daemon start`,
  };
}
...
export function ConnectRemoteDialog({ onClose }: { onClose: () => void }) {
  ...
  // `enact setup` is one blocking command that handles config + login
