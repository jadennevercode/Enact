...
// self-hosted instances instead of the hardcoded `enact.ai`.
describe("StepWorkspace — workspace URL prefix", () => {
  it("shows the brand host when no app URL is configured", () => {
    renderStep({ existing: null, disabled: false });
    expect(screen.getByText("enact.ai/")).toBeInTheDocument();
  });
  ...
  it("shows the deployment host for self-hosted instances", () => {
    renderStep({
      existing: null,
      disabled: false,
      daemonAppUrl: "https://enact.example.com",
    });
    expect(screen.getByText("enact.example.com/")).toBeInTheDocument();
    expect(screen.queryByText("enact.ai/")).not.toBeInTheDocument();
  });
});
