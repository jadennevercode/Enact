...
vi.mock("@enact/core/onboarding", async () => {
  const actual =
    await vi.importActual<typeof import("@enact/core/onboarding")>(
      "@enact/core/onboarding",
    );
...
vi.mock("@enact/core/paths", async () => {
  const actual =
    await vi.importActual<typeof import("@enact/core/paths")>(
      "@enact/core/paths",
    );
...
vi.mock("@enact/core/api", async () => {
  const actual =
    await vi.importActual<typeof import("@enact/core/api")>(
      "@enact/core/api",
    );
  return {
    ...actual,
...
function wipeDismissCounters() {
  for (let i = window.localStorage.length - 1; i >= 0; i--) {
    ...
    if (k && k.startsWith("enact.source_backfill.dismiss.")) {
...
describe("SourceBackfillModal", () => {
  ...
  // @enact/core/onboarding/needs-backfill.test.ts. What belongs here is the
  ...
  it("does not render when the user already recorded a source", () => {
    ...
    renderModal();
    expect(
      screen.queryByText(/How did you hear about Enact/i),
  ...
  it("opens for an onboarded user with empty source once agents completed enough issues", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/How did you hear about Enact/i),
      ).toBeInTheDocument();
    });
  });
  ...
  it("does not open once the per-user dismiss cap is reached on this browser", () => {
    window.localStorage.setItem("enact.source_backfill.dismiss.u1", "3");
    setUser({
      id: "u1",
      onboarded_at: "2026-01-01T00:00:00Z",
      onboarding_questionnaire: { source: [] },
