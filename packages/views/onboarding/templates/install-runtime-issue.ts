...
4. Confirm your terminal can find it:
   which codex
   codex --version
5. Wait for Enact to pick it up. A running daemon re-checks for newly
...
5. 等 Enact 识别到它。运行中的守护进程每隔几分钟会重新检查一次新装的 CLI,通常不需要重启。
   想立刻生效:
   enact daemon restart
...
전체 가이드: https://enact.ai/docs/install-agent-runtime
...
4. 터미널에서 찾을 수 있는지 확인합니다:
   which codex
   codex --version
5. Enact가 인식할 때까지 기다립니다. 실행 중인 daemon은 몇 분마다 새로 설치된 CLI를
...
詳しいガイド: https://enact.ai/docs/install-agent-runtime
...
4. ターミナルから見つけられるか確認します:
   which codex
   codex --version
5. Enact が認識するまで待ちます。動作中の daemon は数分ごとに新しくインストールされた
