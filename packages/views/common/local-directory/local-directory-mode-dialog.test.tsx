...
const TEST_RESOURCES = { en: { resources: enResources, common: enCommon } };
...
describe("LocalDirectoryModeDialog", () => {
  ...
  it("blocks parallel mode when the server cannot honour it", () => {
    ...
    const option = worktreeOption();
    expect(option.hasAttribute("disabled")).toBe(true);
    const notice = screen.getByText(/Enact server is too old/i);
  ...
  it("shows a server rejection inline so the dialog stays actionable", () => {
    renderDialog({
      errorMessage:
        "the Enact runtime on that machine does not support it. Update the Enact app on that machine",
    });
