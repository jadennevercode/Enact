  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          <DialogTitle>{t(($) => $.mode_dialog_title)}</DialogTitle>
          <DialogDescription>
            {t(($) => $.mode_dialog_description)}
          </DialogDescription>
        </DialogHeader>
 ...
 * every other entry point offer literally the same options, copy and blocked
  ...
  return (
    <div className="flex flex-col gap-2">
      <ModeOption
        ...
        title={t(($) => $.mode_in_place_title)}
        description={t(($) => $.mode_in_place_description)}
        identifier="in_place"
      ...
      <ModeOption
        ...
        title={t(($) => $.mode_worktree_title)}
        description={t(($) => $.mode_worktree_description)}
        identifier="worktree"
        selected={value === "worktree"}
        disabled={worktreeDisabled}
        disabledReason={
          unavailableReason === "not_git"
            ? t(($) => $.mode_worktree_needs_git)
            : unavailableReason === "server_outdated"
              ? t(($) => $.mode_worktree_needs_server_upgrade)
              : undefined
        }
