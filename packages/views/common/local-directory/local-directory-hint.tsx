...
import type { LocalDirectoryResourceRef, WorkspaceResource } from "@enact/core/types";
...
/**
 * Banner shown at the top of the issue's Activity section when the workspace
 * has a `local_directory` resource on **this** daemon.
 ...
 * resources read-only in workspace settings but no Activity-section hint.
 *
...
export function LocalDirectoryHint() {
  ...
  const matches: Array<WorkspaceResource & { resource_ref: LocalDirectoryResourceRef }> =
    resources
      .filter(
        (r): r is WorkspaceResource & { resource_ref: LocalDirectoryResourceRef } =>
