...
import { useFileUpload } from "@enact/core/hooks/use-file-upload";
import { resolvePublicFileUrl } from "@enact/core/workspace/avatar-url";
import {
  AVATAR_EMOJI_SUGGESTIONS,
  formatAvatarEmoji,
  parseAvatarEmoji,
} from "@enact/ui/lib/avatar-emoji";
import {
  Popover,
  PopoverContent,
  PopoverTrigger,
...
const EmojiPicker = lazy(() =>
  import("@enact/ui/components/common/emoji-picker").then((m) => ({
...
export function AvatarUploadControl({
  ...
  const avatarButton = (
    ...
    >
      ...
      {!disabled && (
        <div className="absolute inset-0 flex items-center justify-center bg-foreground/40 text-background opacity-0 transition-opacity group-hover:opacity-100">
          {busy ? (
