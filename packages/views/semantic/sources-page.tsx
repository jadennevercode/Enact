...
function CatalogList({ entries, allEntries, selectedId, onSelect }: {
  ...
  function row(entry: CatalogEntry, folder: string) {
    ...
    return <button key={entry.id} title={entry.path || entry.name}
      className={`mb-1 flex w-full items-center gap-2 rounded-md py-2.5 pr-2 text-left text-caption hover:bg-muted ${["pl-2", "pl-5", "pl-8", "pl-11", "pl-14", "pl-17", "pl-20"][depth(entry)]} ${selectedId === entry.id ? "bg-surface-selected font-semibold" : ""}`}
