...
const bootstrap = `(()=>{let port;let next=0;const pending=new Map();window.enact={call(operation,input={}){return new Promise((resolve,reject)=>{if(!port){reject(new Error('Application connection is not ready'));return;}const id=String(++next);pending.set(id,{resolve,reject});port.postMessage({id,operation,input});});}};window.addEventListener('message',event=>{if(event.source!==parent||event.data?.type!=='enact.application.init'||!event.ports[0]||port)return;port=event.ports[0];port.onmessage=e=>{const p=pending.get(e.data.id);if(!p)return;pending.delete(e.data.id);if(e.data.ok){p.resolve(e.data.value);return;}const data=e.data.error;const error=new Error(typeof data==='string'?data:typeof data?.message==='string'?data.message:'Application request failed');if(Number.isInteger(data?.status)&&data.status>=400&&data.status<=599){error.status=data.status;const d=data.existing_draft;const uuid=/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;if(data.status===409&&d&&typeof d.approval_id==='string'&&uuid.test(d.approval_id)&&typeof d.run_id==='string'&&uuid.test(d.run_id)&&typeof d.status==='string'&&/^[a-z_]{1,40}$/.test(d.status))error.existing_draft={approval_id:d.approval_id,run_id:d.run_id,status:d.status};}p.reject(error);};port.start();window.dispatchEvent(new Event('enact-ready'));});parent.postMessage({type:'enact.application.ready'},'*');})();`;
