...
describe("toInternalAppPath", () => {
  it("returns the path (with search and hash) for a URL on the app origin", () => {
    expect(
      toInternalAppPath(`${APP_ORIGIN}/acme/issues/ENA-1?tab=a#c`, APP_ORIGIN),
    ).toBe("/acme/issues/ENA-1?tab=a#c");
  });
...
describe("openLink", () => {
  it("navigates in-app for a URL pointing back at this deployment (ENA-5208)", () => {
    openLink(`${APP_ORIGIN}/acme/issues/ENA-1`, "acme", APP_ORIGIN);
  ...
  it("navigates in-app for a cross-workspace app URL without rewriting the slug", () => {
    openLink(`${APP_ORIGIN}/other/issues/ENA-1`, "acme", APP_ORIGIN);
    expect(navigatedPaths()).toEqual(["/other/issues/ENA-1"]);
  });

  it("opens an external URL in a new window", () => {
    openLink("https://github.com/enact-ai/enact/pull/1", "acme", APP_ORIGIN);
  ...
  it("still opens an app URL externally when no app origin is known", () => {
    openLink(`${APP_ORIGIN}/acme/issues/ENA-1`, "acme");
    expect(dispatched).toHaveLength(0);
    expect(openSpy).toHaveBeenCalled();
  });

  it("prefixes the current slug on a slugless workspace path", () => {
    openLink("/issues/ENA-1", "acme", APP_ORIGIN);
    expect(navigatedPaths()).toEqual(["/acme/issues/ENA-1"]);
  });

  it("leaves a path that already carries a slug alone", () => {
    openLink("/other/issues/ENA-1", "acme", APP_ORIGIN);
    expect(navigatedPaths()).toEqual(["/other/issues/ENA-1"]);
  });

  it("defaults the disposition to push and carries an explicit click intent through the event", () => {
    openLink("/acme/issues/ENA-1", "acme", APP_ORIGIN);
    openLink("/acme/issues/ENA-2", "acme", APP_ORIGIN, "background-tab");
    const details = dispatched.map(
      (e) => (e as CustomEvent<{ path: string; disposition: string }>).detail,
    );
    expect(details).toEqual([
      { path: "/acme/issues/ENA-1", disposition: "push" },
      { path: "/acme/issues/ENA-2", disposition: "background-tab" },
    ]);
  });
...
describe("parseWorkspaceEntityLink", () => {
  ...
  it("parses an issue addressed by identifier", () => {
    expect(parseWorkspaceEntityLink("/acme/issues/ENA-1")).toEqual({
      kind: "issue",
      id: "ENA-1",
      slug: "acme",
    });
  ...
  it("returns null for an id that is neither a UUID nor an identifier", () => {
    ...
    expect(parseWorkspaceEntityLink("/acme/issues/ena-1")).toBeNull();
  });
