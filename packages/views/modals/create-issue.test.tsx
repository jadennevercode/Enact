...
// Coordinator-owned upload entry persisted in the shared pool (ENA-5181 L2).
...
vi.mock("@enact/core/issues/stores/issue-create-settings-store", () => ({
...
vi.mock("@enact/core/api", async () => {
  ...
  const { parseWithFallback } = await vi.importActual<typeof import("@enact/core/api/schema")>(
    "@enact/core/api/schema",
  );
  const { DuplicateIssueErrorBodySchema } = await vi.importActual<
    typeof import("@enact/core/api/schemas")
  >("@enact/core/api/schemas");
  return {
    api: {
...
vi.mock("@enact/ui/components/ui/dropdown-menu", () => ({
...
vi.mock("@enact/ui/components/ui/switch", () => ({
  Switch: ({
    checked,
    onCheckedChange,
  }: {
...
describe("CreateIssueModal", () => {
  beforeEach(() => {
    ...
    mockApiUploadFile.mockResolvedValue({
      ...
      markdown_url: "https://enact-api.copilothub.ai/api/attachments/11111111-2222-3333-4444-555555555555/download",
  ...
  it("persists manual-mode uploads in the issue draft", async () => {
    ...
    // Coordinator flow (ENA-5181 L2): a placeholder is written at pick time,
  ...
  it("prunes draft attachments the reopened description no longer references", async () => {
    ...
    const deleted = {
      ...
      markdown_url: "https://enact-api.copilothub.ai/api/attachments/99999999-8888-7777-6666-555555555555/download",
    };
  ...
  it("shows duplicate-issue toast with a working view-existing link", async () => {
    ...
    mockCreateIssue.mockRejectedValue(
      new ApiError("An active issue with this title already exists: ENA-7 – Login bug", 409, "Conflict", {
        code: "active_duplicate_issue",
        error: "An active issue with this title already exists: ENA-7 – Login bug",
        issue: {
          id: "issue-dup",
          identifier: "ENA-7",
          title: "Login bug",
    ...
    expect(screen.getByText(/ENA-7/)).toBeInTheDocument();
  ...
  // tags along so the agent panel can render a "Sub-issue of ENA-XX" chip
  ...
  // ENA-5181: switching to agent must PRESERVE the manual draft. The agent
  ...
  // ENA-4808 — manual create had no upload gate at all: Create, Enter on the
  ...
  // ENA-5181 P0: the issue draft is a SINGLETON store. A submit that outlives
  ...
  describe("upload submit gate", () => {
     ...
     *  Controls the coordinator's `api.uploadFile` promise (ENA-5181 L2). */
    ...
    // fired from a half-typed title. ENA-4931 adds the explicit `send` chord
  ...
  // ENA-4931 — manual create had no submit shortcut at all, while agent create
  ...
  describe("send shortcut", () => {
    ...
    it("creates once when the chord is pressed twice in the same tick", async () => {
      ...
      await act(async () => {
        release({ id: "issue-1", identifier: "ENA-1", title: "Double tap", status: "todo" });
      });
  ...
  // ENA-6236 — the manual panel shares the agent panel's phone treatment; it
