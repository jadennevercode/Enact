...
// matrix lives in `packages/core/artifacts/artifact-tree.test.ts`, and the icon
...
import type { Artifact } from "@enact/core/types";
import {
...
vi.mock("@enact/core/artifacts", () => ({
  artifactsOptions: () => ({ queryKey: ["artifacts"], queryFn: vi.fn() }),
  buildArtifactFolders,
  filterArtifactFolders,
  UNFILED_FOLDER_ID,
}));
...
describe("ArtifactsPage", () => {
  ...
  // A file uploaded in chat belongs to a session, not an issue, so the server
  // sends every owner field as null. It is the ordinary way to reach Unfiled —
  // not an error path — and it must be listed and downloadable like any other.
  it("files a chat-origin artifact under Unfiled with no issue link", () => {
    renderPage([
      artifact({
        id: "from-chat",
        filename: "notes-from-chat.pdf",
        issue_id: null,
    ...
    fireEvent.click(screen.getByRole("button", { name: /notes-from-chat\.pdf/ }));
    expect(screen.getByText(/were not produced under an issue/)).toBeTruthy();
  ...
  // Chat uploads and issue uploads arrive in one listing; the chat ones must
  // not disturb the issue folders they sort behind.
  it("keeps chat-origin files out of the issue folders and sorts them last", () => {
    renderPage([
      artifact({ id: "filed", filename: "summary.pdf" }),
      artifact({
        id: "from-chat",
    ...
    expect(folders.map((f) => f.textContent)).toEqual([
      expect.stringContaining("ENC-42"),
      expect.stringContaining("Unfiled"),
    ]);
    expect(within(folders[0]!.parentElement!).getByText("summary.pdf")).toBeTruthy();
    expect(within(folders[1]!.parentElement!).getByText("scratch.pdf")).toBeTruthy();
  });
