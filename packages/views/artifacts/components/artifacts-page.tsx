...
/**
 * ArtifactsPage — folder-style browser for every file this workspace produced.
 *
 * Two panes. The left is the tree: one folder per issue the files came from,
 * each row an artifact (a version CHAIN, not a single upload). Files with no
 * owning issue — a chat upload belongs to a session, not an issue — collect in
 * the trailing "Unfiled" folder. The right pane is the selected artifact's
 * detail: every version of it, newest first, each downloadable on its own.
 *
...
export function ArtifactsPage() {
  ...
  const query = useQuery(artifactsOptions(wsId));
  ...
  return (
    <div className="mb-0.5">
      ...
      >
        ...
        {isUnfiled ? (
          <span className="truncate">{t(($) => $.unfiled_folder)}</span>
        ) : (
          <>
