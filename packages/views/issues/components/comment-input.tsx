...
function CommentInput({ issueId, onSubmit, onAccepted }: CommentInputProps) {
  ...
  // Uploads for this composer session (ENA-5181). Owned by the module-level
  ...
  // Await-then-render send (ENA-5181): the shared hook reads the markdown,
  ...
  // Stale-submit guard (ENA-5181 P0): if this composer unmounts mid-submit
  ...
  const { submitting, submit } = useComposerSubmit({
    ...
    onSubmit: (content) => {
      ...
      // Bind only uploads the BODY still references (ENA-5181): deleting an
    ...
    onAccepted: () => {
      // Success may only consume the entry it submitted (ENA-5181 P0): edits
