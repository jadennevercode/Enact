...
import type { UploadResult } from "@enact/core/hooks/use-file-upload";
import type { Attachment } from "@enact/core/types";
import { useCommentComposerStore, useCommentDraftStore } from "@enact/core/issues/stores";
...
// `api.uploadFile(file, ctx, signal)` (ENA-5181). Tests drive uploads by
...
// editor records it — that prop being absent is exactly the ENA-5588 bug.
const editorQuickActionMenu = vi.hoisted(() => ({
  last: undefined as QuickActionMenuProp | undefined,
}));
// Observability + failure control for the write-back insert path (ENA-5181):
...
vi.mock("@enact/core/hooks/use-file-upload", async () => ({
  ...(await vi.importActual<typeof import("@enact/core/hooks/use-file-upload")>(
    "@enact/core/hooks/use-file-upload",
  )),
...
describe("quick action `/` menu", () => {
  ...
  // both. The reply box shipped without the prop entirely (ENA-5588): its menu
...
describe("comment composers", () => {
  ...
  // ENA-5181 P0: the editor stays interactive during a send — text typed
  ...
  // ENA-5181 P0: a submit that outlives its composer may only clear the draft
...
// ENA-4808 — posting mid-upload strips the pending image's blob URL out of the
...
describe("comment composers — upload submit gate", () => {
  ...
  it("does not redraw a placeholder the user deleted mid-upload", async () => {
    // ENA-5181's rule: a placeholder the user removed stays removed. The
  ...
  it("does NOT bind an upload the user deleted from the body", async () => {
    ...
    // is not referenced must not ship — deleting really unbinds (ENA-5181).
