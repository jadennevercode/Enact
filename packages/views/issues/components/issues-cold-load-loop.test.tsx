 ...
 *
 * ENA-4985 regression — cold-load render loop on the Issues route.
 *
...
vi.mock("@enact/core/issues/mutations", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@enact/core/issues/mutations")>();
  return {
    ...actual,
  };
});
...
const mockViewState: Record<string, unknown> = {
  ...
  sortBy: "position",
  sortDirection: "asc",
  cardProperties: { priority: true, assignee: true, dueDate: true, childProgress: true, labels: true },
...
describe("Issues cold-load render loop (ENA-4985)", () => {
  beforeEach(() => {
    vi.clearAllMocks();
