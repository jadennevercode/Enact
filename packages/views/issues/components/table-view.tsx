...
export function InlineTitle({
  ...
  // the issue (ENA-5108 review R1#2).
 ...
 * (ENA-5108). Data flows through meta instead so the element types never
 * change.
 */
type TableViewMeta = {
  childProgressMap: Map<string, ChildProgress>;
 ...
 * (ENA-5108 review R1#3). Clearing the key iff this unmounting cell still owns
...
export function TableView({
    ...
    () =>
      serverBranchPageTargets.map(({ branch, cursor }) => {
        ...
        return {
          ...
          // receives, so the two forms are equivalent (ENA-5477).
  ...
  const serverGroupLabel = useCallback(
    (descriptor: IssueTableGroupDescriptor) => {
      ...
      if (value.kind === "status") {
        ...
        // of collapsing to the schema fallback or an empty label. (ENA-6243)
  ...
  // window and closing the popup the user just opened (ENA-5108). The snapshot
  ...
  // starts a run, and must confirm rather than fire from one click (ENA-6463).
