...
const {
  ...
  mockGetShareableUrl: vi.fn((p: string) => `https://app.enact/${p}`),
  mockMembers: {
    current: [] as Array<{
      id: string;
      workspace_id: string;
...
vi.mock("@enact/core/issues/stores", () => {
...
vi.mock("@enact/ui/components/common/theme-provider", () => ({
...
describe("SearchCommand", () => {
  beforeEach(() => {
    ...
    mockGetShareableUrl.mockReset().mockImplementation((p: string) => `https://app.enact/${p}`);
    mockMembers.current = [];
  ...
  it("cmd-click on a result opens it in a new tab instead of navigating in place", async () => {
    ...
    expect(open).toHaveBeenCalledWith(
      "https://app.enact//ws-test/settings",
      "_blank",
      "noopener,noreferrer",
    );
  ...
  it("copies issue link and identifier when on an issue detail route", async () => {
    ...
    expect(mockClipboardWrite).toHaveBeenCalledWith("https://app.enact//ws-test/issues/issue-1");
