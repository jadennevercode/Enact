...
export function CreateAgentDialog({
  ...
  // ENA-4010: rolls out the private / public_to access model in the create
  // flow to match the AccessPicker on the agent detail page. Shares the
  // `composio_mcp_apps` switch with the Composio rollout — the ENA-3963
  ...
  // New access state (ENA-3963 aligned). When duplicating, seed from the
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (accessPickerEnabled) {
        // New ENA-3963 shape: send the authoritative permission fields and
      ...
      if (template) {
        ...
        // ENA-2600 the agent resource shape no longer carries
      ...
      // ENA-2178) — a partial failure surfaces a warning toast and
 ...
 * `permission_mode` + `invocation_targets` (ENA-3963), not the legacy
 * `visibility`.
 *
