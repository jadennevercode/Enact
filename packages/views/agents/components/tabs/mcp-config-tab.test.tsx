...
vi.mock("@enact/core/runtimes", async () => {
  const actual =
    await vi.importActual<typeof import("@enact/core/runtimes")>(
      "@enact/core/runtimes",
    );
  return {
    ...actual,
...
describe("McpConfigTab", () => {
  ...
  it("projects historical aggregate config into individually managed rows", () => {
    ...
    expect(screen.getByRole("heading", { name: /managed by enact/i })).toBeInTheDocument();
...
describe("McpConfigTab effective set", () => {
  ...
  it("marks a runtime server shadowed by an assigned server as overridden", async () => {
    ...
    expect(
      await screen.findByText("Overridden by Enact"),
    ).toBeInTheDocument();
  });
  ...
  it("does not mark a runtime server overridden by a disabled assignment", async () => {
    ...
    expect(screen.queryByText("Overridden by Enact")).toBeNull();
  });
