  ...
  return (
    <section className="space-y-3">
      <div className="flex flex-wrap items-end justify-between gap-3">
        ...
        {canManage && <div className="flex gap-2"><Button variant="outline" size="sm" onClick={() => void connectGitHub()} disabled={connectingGitHub || githubData?.configured === false}><GitHubMark className="size-3.5" />{t(($) => $.hosting.connect_github)}</Button><Button size="sm" onClick={() => setConnectOpen(true)} disabled={!vcsData?.configured}><Plus className="size-3.5" />{t(($) => $.hosting.connect_gitlab)}</Button></div>}
      </div>
...
function StatusGrid({ statuses }: { statuses: Array<[string,string]> }) {
  ...
  return <div className="grid grid-cols-2 gap-2 sm:grid-cols-5">{statuses.map(([label,status]) => { const good = ["ok","ready","verified"].includes(status); const pending = ["pending","unknown","untested"].includes(status); const detail=help(status); return <div key={label} className="rounded-md border px-2 py-2" title={detail}><div className="flex items-center gap-1.5">{good ? <CheckCircle2 className="size-3.5 text-success" /> : <AlertTriangle className={`size-3.5 ${pending ? 'text-warning' : 'text-destructive'}`} />}<span className="text-micro font-medium">{label}</span></div><p className="mt-1 truncate text-micro text-muted-foreground">{status}</p>{detail ? <p className="mt-1 line-clamp-2 text-micro text-muted-foreground">{detail}</p> : null}</div>; })}</div>;
}
