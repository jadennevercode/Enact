...
vi.mock("@enact/core/api", () => ({ api: { callPluginAction: mockCall } }));

import { createSurfaceBridge } from "./surface-bridge";

// The bridge is the only path from a surface into Enact, so what it refuses
...
function announce(source: Window | null) {
  const event = new MessageEvent("message", { data: { type: "enact:plugin-surface-ready" } });
