...
// Lives here rather than beside the SDK on purpose: @enact/plugin-sdk ships
...
async function loadSdk() {
  ...
  return import("@enact/plugin-sdk");
}

function initFrom(source: unknown, port: MessagePort) {
  const event = new MessageEvent("message", {
    data: { type: "enact:plugin-bridge-init", version: 1 },
  });
