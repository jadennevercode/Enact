...
beforeEach(() => {
  navigatedPaths = [];
  window.addEventListener("enact:navigate", captureNavigate);
  openSpy = vi.spyOn(window, "open").mockImplementation(() => null);
});

afterEach(() => {
  window.removeEventListener("enact:navigate", captureNavigate);
  vi.restoreAllMocks();
});
...
describe("RichContent link routing", () => {
  ...
  it("still hands a genuinely external link to the browser", () => {
    const external = "https://github.com/enact-ai/enact/pull/1";
