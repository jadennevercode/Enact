...
import { larkInstallationsOptions, larkKeys } from "@enact/core/lark";
import { api, ApiError } from "@enact/core/api";
import type { LarkInstallation, LarkInstallStatusResponse } from "@enact/core/types";
...
// install path is per-agent (each Enact Agent gets exactly one Bot —
...
function InstallationRow({
  ...
  const { t } = useT("settings");
  // The bot is bound 1:1 to a Enact Agent (per the (workspace_id,
  // agent_id) UNIQUE in lark_installation). Render the Enact agent's
