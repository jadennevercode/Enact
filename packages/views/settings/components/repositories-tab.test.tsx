...
const workspaceRef = vi.hoisted(() => ({
  current: {
    ...
    name: "Test Workspace",
    slug: "test-workspace",
    repos: [{ url: "https://github.com/enact-ai/enact" }] as {
      url: string;
      description?: string;
    }[],
  },
...
describe("RepositoriesTab — automatic updates", () => {
  beforeEach(() => {
    ...
    workspaceRef.current = {
      ...
      name: "Test Workspace",
      slug: "test-workspace",
      repos: [{ url: "https://github.com/enact-ai/enact" }],
    };
  ...
  it("renders persisted repositories as the same shared input controls used for editing", () => {
    ...
    expect(inputs[0]!.value).toBe("https://github.com/enact-ai/enact");
  ...
  it("updates a changed URL automatically on blur", async () => {
    ...
    await user.type(urlInput, "https://github.com/enact-ai/edited");
    await user.tab();

    await waitFor(() => {
      expect(mockUpdateWorkspace).toHaveBeenCalledWith("workspace-1", {
        repos: [{ url: "https://github.com/enact-ai/edited" }],
      });
  ...
  it("does not persist a new row until its URL is non-empty", async () => {
    ...
    await user.type(newUrlInput, "git@github.com:enact-ai/second.git");
    await user.tab();
    ...
    await waitFor(() => {
      expect(mockUpdateWorkspace).toHaveBeenCalledWith("workspace-1", {
        repos: [
          ...
          { url: "git@github.com:enact-ai/second.git" },
        ],
      });
    });
  });
  ...
  it("accepts scp-like repository shorthand", async () => {
    ...
    await user.clear(urlInput);
    await user.type(urlInput, "git@github.com:enact-ai/enact.git");
    ...
    await waitFor(() => {
      expect(mockUpdateWorkspace).toHaveBeenCalledWith("workspace-1", {
        repos: [{ url: "git@github.com:enact-ai/enact.git" }],
      });
    });
  });
  ...
  it("includes the description in the automatic update payload", async () => {
    workspaceRef.current = {
      ...workspaceRef.current,
      repos: [{ url: "https://github.com/enact-ai/enact", description: "Main app" }],
    };
    const user = setupUser();
    render(<RepositoriesTab />, { wrapper: I18nWrapper });
  ...
  it("imports selected GitHub repositories and deduplicates HTTPS against SSH", async () => {
    ...
    githubRepositoriesRef.current = [
      {
        ...
        html_url: "https://github.com/enact-ai/enact",
      ...
      },
      {
        ...
        html_url: "https://github.com/enact-ai/console",
    ...
    await waitFor(() => {
      expect(mockUpdateWorkspace).toHaveBeenCalledWith("workspace-1", {
        repos: [
          { url: "git@github.com:enact-ai/enact.git" },
          {
