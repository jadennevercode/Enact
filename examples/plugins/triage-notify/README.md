...
Demonstrates the hook engine: Enact calling **out** to a plugin's own server,
...
ENACT_SIGNING_SECRET=whsec_… node server/handler.mjs
```
...
Nothing a deployment can configure widens it. `ENACT_PLUGIN_DEV_ORIGINS` lets
...
```bash
export ENACT_PLUGIN_DEV_ORIGINS=https://localhost:8787
```
