...
The reference Enact plugin: one `issue_panel` surface that exercises every
...
- `enact.context.get()` — who is looking and which issue the panel is on
- `enact.issue.get()` — reading the issue behind `issues:read`
- `enact.issue.comment()` — a write that lands as **the user**, marked with
  the plugin (`via_plugin_id`), behind `comments:write`
- `enact.storage.user` — per-member state behind `storage:user`
- `enact.ui.resize()` — asking the host for the height it actually needs

## Running it

Zip this folder — the manifest plus every file it names — and upload it in
**Settings → Plugins**. You need no server of your own: Enact stores the
...
While you are iterating, `ENACT_PLUGIN_DIR` publishes straight from disk
