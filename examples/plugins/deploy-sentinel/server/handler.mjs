// Deploy Sentinel's own server. Enact never runs this — the plugin author does.
//
...
//
// Env: ENACT_SIGNING_SECRET (the whsec_… shown once when the token was issued)
//      PORT (default 8788)
...
// manifest will not install, and ENACT_PLUGIN_DEV_CA only changes WHICH
// certificate Enact trusts — it never turns verification off. Generate one:
//
//   openssl req -x509 -newkey rsa:2048 -nodes -days 365 \
//     -keyout dev-key.pem -out dev-cert.pem \
//     -subj "/CN=127.0.0.1" -addext "subjectAltName=IP:127.0.0.1"
//
// then point ENACT_PLUGIN_DEV_CA at dev-cert.pem.
function tlsOptions() {
...
function verifySignature(rawBody, signature, timestamp) {
  ...
  // Reject a replay before spending time on the comparison. Enact signs
...
const server = createServer(tlsOptions(), async (req, res) => {
  ...
  const verified = verifySignature(
    rawBody,
    req.headers["x-enact-signature"],
    req.headers["x-enact-timestamp"],
  );
  if (!verified.ok) {
  ...
  switch (hookKey) {
    ...
    case "page_oncall": {
      ...
      // would hold a Enact worker for no benefit.
...
server.listen(PORT, () => {
  ...
  if (!SIGNING_SECRET) {
    console.warn("ENACT_SIGNING_SECRET is not set — every request will be refused.");
  }
});
