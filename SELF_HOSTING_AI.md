...
This document is designed for AI agents to execute. Follow these steps exactly to deploy a local Enact instance and connect to it.
...
Wait for the server output `✓ Enact server is running and CLI is ready!` before running `enact setup self-host`.

**Expected result:**
- Frontend at http://localhost:3000
- Backend at http://localhost:8080
- `enact` CLI installed and configured for localhost
...
The `enact setup self-host` command will:
...
2. Run `make selfhost`
3. Run `enact setup self-host --port <PORT> --frontend-port <FRONTEND_PORT>`
...
- **Daemon issues:** `enact daemon logs`
