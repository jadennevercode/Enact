...
TEST_DIR=$(mktemp -d "${TMPDIR:-/tmp}/enact-ios-run.XXXXXX")
...
mkdir -p "$BIN_DIR"
export ENACT_TEST_PNPM_CALLS="$CALLS_FILE"
...
printf '%s\n' "$*" >>"$ENACT_TEST_PNPM_CALLS"

if [ -n "${ENACT_TEST_FAIL_PREBUILD:-}" ]; then
  case "$*" in
    *prebuild*)
...
: >"$CALLS_FILE"
set +e
ENACT_TEST_FAIL_PREBUILD=1 "$SCRIPT_DIR/ios-run.sh" >/dev/null 2>&1
status=$?
set -e
