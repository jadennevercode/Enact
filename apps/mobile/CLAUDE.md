...
- `import type` from `@enact/core/types/*` (zero runtime coupling)
- Pure functions from `@enact/core/`
...
- **Main CI** (`.github/workflows/ci.yml`) excludes mobile via `--filter='!@enact/mobile'`. Mobile failures do NOT block web/desktop PRs.
...
1. **Read the payload.** Find the event in `@enact/core/types/events.ts`. Note the fields; decide if patch is possible (full object) or invalidate is required (just an id).
