  ...
  const onLinkPress = useCallback(
    ({ url }: { url: string }) => {
      ...
      if (url.startsWith("mention://")) {
        ...
        if (id && wsSlug) {
          // Route segments are singular on mobile (`/issue/`) while the
          // mention type and web's route are the same word — keep the
          // mapping explicit so a new type can't silently no-op.
