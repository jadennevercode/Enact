...
export default ({ config }: ConfigContext): ExpoConfig => {
  ...
  return {
    ...
    ios: {
      ...
      // Enact Apple Developer team (and external users self-building a
      ...
      bundleIdentifier: isProd
        ? (process.env.EXPO_BUNDLE_IDENTIFIER_PROD ?? "ai.enact.mobile")
        : isStaging
          ? "ai.enact.mobile.staging"
          : (process.env.EXPO_BUNDLE_IDENTIFIER_DEV ?? "ai.enact.mobile.dev"),
    },
    plugins: [
      "expo-router",
      "expo-secure-store",
      ...
      [
        "expo-image-picker",
        {
          ...
          photosPermission:
            "Allow Enact to access your photos to attach images to issues and comments.",
