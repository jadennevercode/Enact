 ...
 *   enact-desktop-{version}-mac-{arch}.{dmg|zip}
 *   enact-desktop-{version}-windows-{arch}.exe
 *   enact-desktop-{version}-linux-{arch}.{AppImage|deb|rpm}
 *
...
const DESKTOP_ARTIFACT_RE =
  /^enact-desktop-[^-]+-(mac|windows|linux)-([a-z0-9_]+)\.(dmg|zip|exe|AppImage|deb|rpm)$/i;
