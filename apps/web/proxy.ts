...
import { LOCALE_COOKIE } from "@enact/core/i18n";
import {
  ENACT_LOCALE_HEADER,
...
// Forward the resolved locale to RSC layouts via the `x-enact-locale`
...
function nextWithLocale(req: NextRequest): NextResponse {
  ...
  headers.set(ENACT_LOCALE_HEADER, resolveLocale(req));
...
export function proxy(req: NextRequest) {
  ...
  const hasSession = req.cookies.has("enact_logged_in");
