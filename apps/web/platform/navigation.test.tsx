...
import { useNavigation, type NavigationAdapter } from "@enact/views/navigation";

function navigate(path: string) {
  window.dispatchEvent(
    new CustomEvent("enact:navigate", { detail: { path } }),
  );
}
...
describe("WebNavigationProvider internal link bridge", () => {
  ...
  it("ignores an event without a path", () => {
    ...
    window.dispatchEvent(new CustomEvent("enact:navigate", { detail: {} }));
 ...
 * or replaces with a fallback. A wrong `true` walks the user out of Enact,
