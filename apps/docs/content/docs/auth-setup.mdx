...
description: Configure direct email sign-in, Google sign-in, and who can sign up.
---

import { Callout } from "fumadocs-ui/components/callout";

Enact signs users in directly with their email address by default; Google OAuth can be added on top. Existing users can always sign in again — signup restrictions only decide whether new accounts can be created.

## Direct email sign-in

After the user enters an email address and continues, Enact creates or reuses the matching account and immediately issues a sign-in cookie. There is no verification-code step.
