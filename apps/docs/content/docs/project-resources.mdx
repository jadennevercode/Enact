...
When an agent works an issue inside the project, Enact adds the project name, project description, and resource list to the run's context, and writes `.enact/project/resources.json` into the working directory.
...
Beyond the code changes the agent makes, the runtime may also write into the directory the instruction files the current AI coding tool needs, plus `.enact/project/resources.json`. Add them to `.gitignore` if you do not want them under version control.
...
enact project create \
  --title "Agent UX" \
  --repo https://github.com/enact-ai/enact

# List and add resources
enact project resource list <project-id>
enact project resource add <project-id> \
  --type github_repo \
...
enact project resource update <project-id> <resource-id> --execution-mode worktree
enact project resource update <project-id> <resource-id> --execution-mode in_place

# Remove a resource
enact project resource remove <project-id> <resource-id>
```
