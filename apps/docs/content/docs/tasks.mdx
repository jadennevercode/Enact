...
Every time an agent starts working, Enact creates a **task**. It records what triggered the run, which agent it went to, how far it has progressed, and whether it ultimately succeeded.
...
Different entry points provide different context, but execution works the same way: Enact creates a task, a runtime claims it, and the runtime invokes the AI coding tool the agent is configured with.
