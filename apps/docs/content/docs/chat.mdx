...
A chat is **not bound to any issue**: the agent doesn't automatically receive an issue's description and comments, but it can still use the Enact CLI to query workspaces, projects, issues, and skills within its permissions.
...
Direct chats created in Enact are visible only to their creator; other workspace members and admins cannot read them. Conclusions the team needs to see together should be written into an issue, a project description, or a skill.
