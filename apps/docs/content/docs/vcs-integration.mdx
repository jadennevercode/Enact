...
<Callout type="warn">
**Self-hosted Enact only.** This integration is available only when you run
Enact yourself; it is not offered on Enact Cloud. "Self-hosted" here means
**Enact** is self-hosted — typically so it can reach a Git instance on your own
network. A server operator must enable it (`ENACT_VCS_INTEGRATION_ENABLED=true`)
and set `ENACT_VCS_SECRET_KEY`; until then the section does not appear in
Settings → Integrations.
</Callout>

Enact connects to a self-hosted Git provider per workspace: **Forgejo**, **Gitea**, or **GitLab**. Once connected, any pull request (GitLab: merge request) whose branch name, title, or body contains an issue identifier (for example `ENA-123`) is **auto-linked** to that [issue](/issues), appears under **Pull requests** in the issue sidebar, and — when merged with a closing keyword — moves the issue to **Done**. CI for the head commit shows as a checks bar on the card.
...
```sh
ENACT_VCS_INTEGRATION_ENABLED=true
```
...
```sh
ENACT_VCS_SECRET_KEY=<base64 32-byte key>
```
...
Set `ENACT_PUBLIC_URL` to the server's public base URL so Enact can show a ready-to-paste webhook URL. Without it, the UI shows only the webhook path and you prepend your own origin.
...
2. In Enact, open **Settings → Integrations → Git providers**.
3. Choose the **provider**, enter the instance URL (for example `https://forgejo.example.com`) and the access token, then **Connect**. Enact validates the token against the instance before saving.
...
Enact authenticates every delivery against the stored secret, so a webhook without the matching secret is rejected.

## What gets mirrored

- **Pull / merge requests** — open, closed, merged, and draft state, author, branch, and (where the provider supplies them) diff stats.
- **Issue links** — identifiers in the title, body, or branch link the PR to the issue. A closing keyword (`Closes`/`Fixes`/`Resolves ENA-123`) on a merged PR moves the issue to **Done** once no linked PR is still open.
