---
title: 프로젝트 아키텍처
description: Enact의 클라이언트, Go 서비스, 실행 데몬, 공유 프런트엔드 패키지가 함께 작동하는 방식을 알아봅니다.
---

Enact는 Go backend, 여러 클라이언트, 실행 컴퓨터에서 동작하는 데몬으로 구성됩니다. PostgreSQL이 협업 데이터를 저장하고 데몬이 태스크를 가져가 로컬 AI 코딩 도구를 호출합니다.
...
제품 사용자 관점의 설명은 [Enact의 작동 방식](/how-enact-works)을 참고하세요. 이 페이지에서는 코드의 계층 구조를 설명합니다.
...
Mobile은 이 React 페이지를 재사용하지 않습니다. `@enact/core`의 유형과 순수 함수를 import할 수 있지만 UI, query key, 상태, 실시간 구독, 출시 절차는 별도로 소유합니다.
...
Enact에는 서로 다른 두 WebSocket 경로가 있습니다.
