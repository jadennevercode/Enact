...
If you write Enact code, change a translation, or write Chinese product copy, this is the page to reference.
...
| `packages/ui` | nothing | `@enact/core`, business logic |
...
| `apps/mobile/` | types and pure functions from `@enact/core` | Web/Desktop React pages, stores, and platform implementations |
...
`AgentRuntime.name` is the daemon's raw technical name (e.g. `Codex (host)`); the user's alias lives in `custom_name`. User-visible copy must never render `runtime.name` directly — use the shared helpers so aliases and provider stay consistent (ENA-5248, #5260):
...
### The core distinction: everyday noun vs Enact-specific term

Enact's product nouns split into two categories:

- **Everyday noun** — the word a user would say out loud for it. **Translate fully**, whether or not it is a database entity: `issue` → 任务, `workspace` → 工作区, `resource` → 资源.
- **Enact-specific term** — a concept no local word carries (`skill`), or a schema-level identifier a user may have to type or match (`todo`, `in_progress`, `task_id`). Render as **lowercase English** so it reads like a type name.
...
- **Code references and literal commands** stay English: `enact issue ...`, the `/issue` Slack and Lark slash command.
- **`skill`** keeps lowercase English in Chinese text — a Enact-specific concept with no established Chinese term; titles may capitalize as `Skills`.
...
**Why `issue` is translated while `skill` is not**: users file and read issues all day, and "issue" has no meaning in Chinese, Japanese, or Korean outside of dev jargon. The everyday task word is what people already say for this. `skill` is a Enact-specific concept where no local word carries the meaning.
...
| Category | Terms |
| --- | --- |
| Brands | **Enact**, GitHub, Slack, Google, Anthropic, OpenAI, Claude, Codex, Cursor, Linear, Jira |
...
| Knowledge base | **知识库** |
| Code graph | **代码图谱** |
| Subsystem (code graph community) | **子系统** |
| God node | **关键枢纽** |
| Call flow | **调用流** |
...
| Permission | 权限（`owner` / `admin` / `member`）|
| Role | 角色（workspace-defined functional role; grants nothing）|
...
### Permission and role — two different words

The product has two concepts English keeps apart and Chinese must keep apart too:

| Concept | Values | English UI | Chinese UI | Code |
| --- | --- | --- | --- | --- |
| What someone may DO | `owner` / `admin` / `member` | Permission | 权限 | `member.role`, `MemberRole` |
| What kind of judgement someone GIVES | workspace-defined | Role | 角色 | `team_role`, `TeamRole` |

A team role grants no access. It exists so work can be routed to the right
reviewer — the AI-SDLC suite resolves each phase's reviewers from it.

Code keeps `member.role` for the permission because it is a stored column and a
response field installed clients already read; anything about the functional
role carries the `team_role` prefix, so neither concept can be reached by a bare
`role`.

### Permission and status enums (lowercase English, not translated)

These are schema-level identifiers; render as lowercase English even in Chinese context.

- Permissions: `owner` / `admin` / `member`
- Team role keys: workspace-defined, e.g. `architect` / `qa` / `ops`
...
See `auth.json` for the canonical example (the `web` section contains `desktop_handoff.*`).
