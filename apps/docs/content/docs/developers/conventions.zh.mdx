...
| `packages/ui` | 无业务依赖 | `@enact/core`、业务逻辑 |
...
| `apps/mobile/` | `@enact/core` 的类型和纯函数 | Web/Desktop 的 React 页面、store 和平台实现 |
...
`AgentRuntime.name` 是 daemon 的原始技术名（例如 `Codex (host)`），用户别名在 `custom_name`。用户可见文案不得直接渲染 `runtime.name`，要用共享 helper，保证别名和 provider 一致（ENA-5248、#5260）：
...
每个 issue 有人类可读的编号，比如 `ENA-123`：工作区 `issue_prefix`（大写字母和数字，通常 3 个字符，最长 10 个）+ 流水号。工作区管理员可以在**设置 → 通用**中修改前缀；修改会让所有现有 issue 重新编号，外部引用——PR 标题、分支名、文档与聊天里的链接——里的旧前缀会失效。
...
### 核心区分：日常词 vs Enact 专有词

Enact 的产品名词分两类：

- **日常词** —— 用户会直接说出口的词。**完整翻译成中文**，是不是数据库实体都一样：`issue` → 任务、`workspace` → 工作区、`resource` → 资源。
- **Enact 专有词** —— 中文里没有对应词的概念（`skill`），或用户可能要照着敲、照着比对的 schema 标识符（`todo`、`in_progress`、`task_id`）。**用小写英文**呈现，读起来像类型名。
...
- **代码引用和字面命令**保持英文：`enact issue ...`、Slack 与飞书的 `/issue` 斜杠命令。
- **`skill`** 在中文里仍用小写英文 —— Enact 特有概念，没有公认中文译法；标题可大写为 `Skills`。
- **表示「问题」的 issue** 不是这个实体：机器卡片上的 `{{count}} issues` 是 `{{count}} 个异常`，不是 `任务`。

**为什么 `issue` 要译，而 `skill` 不译**：用户每天都在提任务、看任务，而 "issue" 这个词在中文、日文、韩文里除了开发圈黑话之外没有含义，当地人本来就用「任务」在说这件事。`skill` 则是 Enact 特有概念，没有哪个中文词能承载它的意思。
...
导航区块的名字也在这张表里，它们是产品结构的一部分，不是随手写的标题：

| 英 | 中 |
| --- | --- |
| Home | **工作台** |
| Collaboration（导航分组） | **协作** |
| Intelligence（导航分组） | **智能体团队** |
| AI Family（导航项、页面） | **智能体团队** |
| Execution（导航分组） | **运行** |
| Capability Hub（产品区域） | **能力中心** |

### 不翻 —— 品牌名 + 通用缩写

| 类别 | 词 |
| --- | --- |
| 品牌 | **Enact**、GitHub、Slack、Google、Anthropic、OpenAI、Claude、Codex、Cursor、Linear、Jira |
| 缩写 | API、CLI、URL、SDK、OAuth、JWT、SSO、WebSocket、HTTP、JSON、YAML、SQL |
| 一等对象 | **Agent Family**、**Skill**、**MCP**、**Ontology** |

`Agent Family` 曾经按语言翻译过（中文「智能体小组」、韩文、日文各一版）。它现在四种语言统一用英文：
「智能体小组」已经是收纳它的那个页面的名字，两个名字撞在一起，谁都说不清指的是哪个。

`Capability Hub` 是 Enact 自己的能力目录，中文叫**能力中心**。这个区域一度叫「市场 /
Marketplace」，那个词现在不再出现在任何界面里：它把一个部署内部共享能力的地方说成了买卖场所。
英文、日文、韩文界面统一用 `Capability Hub`，只有中文取译名，因为「Hub」在中文界面里不自解释。
它和 Ontology 文案里引用的外部产品 **CapHub** 是两回事，外部引用保持 `CapHub` 不动。
...
| 概念 | 取值 | 英文 UI | 中文 UI | 代码 |
| --- | --- | --- | --- | --- |
| 一个人**能做什么** | `owner` / `admin` / `member` | Permission | 权限 | `member.role`、`MemberRole` |
| 一个人**给出哪类判断** | 工作区自定义 | Role | 角色 | `team_role`、`TeamRole` |

角色不授予任何访问权限。它存在的目的是把工作路由给合适的评审人 —— AI-SDLC
套件就是按角色找到每个阶段的评审人的。

代码里权限仍然叫 `member.role`：它是已经落库的列，也是已安装客户端在读的响应
字段。功能性角色一律带 `team_role` 前缀，这样光写一个 `role` 永远指不到它。
...
- 角色 key：工作区自定义，例如 `architect` / `qa` / `ops`
...
参考 `auth.json`（`web` 段含 `desktop_handoff.*`）。
