...
每条自动化保存一份 Runbook、一个执行方和一个或多个触发器。触发后，Enact 会创建 issue 或直接运行智能体，并保留每一次运行记录。
...
添加 Webhook 触发器后，Enact 会生成一个唯一 URL。向它发送 JSON 对象或数组，即可触发自动化：
...
例如，事件名填写 `workflow_run`，Actions 填写 `completed, failed`，只会接收这两类 `workflow_run` 结果。Enact 会从常见请求头和 payload 中识别事件与 action，包括 GitHub 的 `X-GitHub-Event` 和 body 中的 `action`。
...
Enact 会定期检查近期运行是否持续失败：过去 7 天内已完成或失败的运行不少于 50 次且失败率达到 90% 时，系统会暂停这条自动化并通知创建者；修复原因后可以手动恢复。
