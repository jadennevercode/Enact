...
First work out which layer the problem lives in: the Enact service, the daemon, the runtime, or the AI coding tool. These commands are usually enough to surface the first meaningful error:

```bash
enact version
enact auth status
enact daemon status --output json
enact daemon logs --lines 100
```
...
- The daemon is connected to the wrong Enact service.
...
Self-hosted deployments should also request the API's `/health` from the **executing computer** — testing on the server itself cannot expose DNS, TLS, or firewall problems on the executing computer's side. To change the address, rerun `enact setup self-host`, or check the current profile's `server_url`:
...
If the runtime list is missing an expected tool, first confirm the tool runs and is signed in under the same system account and `PATH`, then run `enact daemon restart`.

### `waiting_local_directory` status

This means another in-flight task is using the same local directory. Enact waits for the directory lock to be released so two agents never modify the same files at once.

This wait only exists in the directory's `in_place` ("Direct") mode. If the directory is a git repository, switching the resource to `worktree` ("Parallel") removes the queue altogether: each task gets its own worktree and hands its work back as a branch, so nothing waits on anything. See [Resources](/resources#how-tasks-share-the-directory).

Otherwise you just wait for the earlier task to finish. If it is stuck, stop it from its execution log, or pick a different local directory for the current agent. This directory mutex lives in the daemon's memory — no lock file is written to disk. If you suspect stale lock state, `enact daemon restart` releases it; there is nothing to delete manually.
...
- **DEV mode**: no email is sent; invitation links are written to the backend log.
...
When `SMTP_HOST` and Resend are both configured, Enact prefers SMTP. For configuration, see [Sign-in and signup](/auth-setup#invitation-email-delivery).
...
If it is another Enact checkout, run `make stop` in that directory first. Otherwise stop the offending program normally, or change the current service's port. Public ports 80/443 are listened on by a reverse proxy such as Caddy or Nginx.

## Log locations

| Component | How to view |
| --- | --- |
| Background daemon | `enact daemon logs --lines 100` |
| Follow daemon logs live | `enact daemon logs --follow` |
| Default profile's log file | `~/.enact/daemon.log` |
| Default profile's startup or crash log | `~/.enact/daemon.err.log` |
| Named profiles | the matching logs under `~/.enact/profiles/<name>/` |
...
If you still cannot pin it down, search existing issues or file a new one on [GitHub Issues](https://github.com/enact-ai/enact/issues).
