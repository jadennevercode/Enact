...
Read and write the [project profile](/workspace-setup) — what this workspace
says its project is. Every agent run starts with it, and the Capability Hub ranks
listings against it:
...
`profile set` replaces the whole document, so read the current one first and
send it back with your changes. `--json-stdin` is required: a bare `set` must
not be able to clear a profile.
...
| `resource` | Manage the workspace's repositories and local directories |
...
| `graph` | Query a repository's [code graph](/code-graph): status, report, subsystems, paths, impact |
...
| --- | --- | --- | --- |
| `issue` | `list` | List issues | `--status`, `--priority`, `--assignee`, `--metadata` (repeatable), `--limit`, `--offset`, `--sort`, `--full-id` |
| | `get <id>` | Show a single issue | |
| | `create` | Create an issue | `--title` (required), `--description` / `--description-stdin` / `--description-file`, `--status`, `--priority`, `--assignee`, `--parent`, `--stage`, `--start-date`, `--due-date`, `--attachment` (repeatable) |
...
| `resource` | `list/add/update/remove` | Manage the workspace's resources | `--type` (`github_repo` / `local_directory`), `--url`, `--ref`, `--local-path`, `--daemon-id`, `--execution-mode` (`in_place` / `worktree` for a local directory) |
...
| `autopilot` | `list/get/create/update/delete` | Manage automations | `create`: `--title`, `--agent`, `--mode` (all required), `--priority`, `--subscriber` (repeatable) |
...
Reads the [code graph](/code-graph) of a repository that has it enabled. Every subcommand takes `--repo <url>`; without it the repository checked out in the current directory (its `origin` remote) is matched against the workspace's resources. When the graph is not enabled, not built yet, or the deployment has no code graph service, the command exits with code 2 and a one-line reason — never a stack trace — so an agent can fall back to its file tools.

| Subcommand | Purpose | Key flags |
| --- | --- | --- |
| `status` | Build state, commit, size, whether the graph is behind the branch head | `--json` |
| `report` | The build report: largest subsystems, most connected nodes, cross-subsystem links | |
| `query "<question>"` | A token-budgeted subgraph for a natural-language question | `--dfs`, `--depth N` (default 3), `--budget N` (default 2000), `--context C` (repeatable) |
| `path "<A>" "<B>"` | Shortest path between two symbols | `--directed` |
| `explain "<node>"` | A symbol's neighbours and the edges touching it | |
| `affected "<node>"` | What depends on a symbol, following call and import edges | `--depth N` (default 2), `--relation R` (repeatable) |
| `communities` | The subsystems with their size and cohesion | `--json` |
