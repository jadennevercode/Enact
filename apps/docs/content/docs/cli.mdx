...
# View and search
enact issue list
enact issue get MUL-123
enact issue search "login failure"

# Create and update
enact issue create --title "Fix login failure"
enact issue status MUL-123 in_progress
enact issue assign MUL-123 --to "Backend Agent"

# Comments and runs
enact issue comment list MUL-123
enact issue comment add MUL-123 --content "Check the regression tests first"
enact issue runs MUL-123
```
...
enact issue comment add MUL-123 --content-stdin < review.md
```

View the messages of a single run, or stop a task:

```bash
enact issue run-messages <task-id> --issue MUL-123
enact issue cancel-task <task-id> --issue MUL-123
```
