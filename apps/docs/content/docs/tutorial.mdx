...
description: "Start from an empty workspace and walk a personal website project through all of Enact's core flows: creating agents, delivering with issues, forming a squad, building skills, and setting up automation."
---

Enact is a platform where people and AI agents work together. This tutorial starts from nothing and covers:
...
The example project is a personal website, used to walk through Enact's overall flow and core features end to end.

**Before you start:**

- A Enact account (sign up at [enact.ai](https://enact.ai))
...
A workspace is where a team works: issues, agents, and the code they run against all belong to one. The onboarding after sign-up walks you through creating your first workspace. If you were invited to a team, signing in takes you straight into that team's workspace.
...
- **URL**: the slug in the workspace address — the `my-team` in `enact.ai/my-team`. It cannot be changed after creation.
...
This tutorial uses [Enact Desktop](https://enact.ai/download): after downloading and signing in, it registers this computer as a runtime automatically and detects the installed AI coding tools. Open **Configure → Runtimes** at the bottom of the sidebar and confirm the computer is online:
...
Leave the remaining fields at their defaults and create. Back on the **Agents** page, Enact helper is online and can take work:

![Agent detail: Enact helper online](/images/docs/tutorial-agent-detail.webp)
...
Open **Chat** in the sidebar, select Enact helper, and tell it to create an agent named Engineer on the same runtime, with Opus as the model, responsible for building and maintaining the personal website. Enact helper performs the creation on that computer and replies with the new agent's name and model. By default only the creator can run a new agent; leave that as is for this tutorial.

![Asking the helper in chat to create Engineer](/images/docs/tutorial-chat-create-engineer.webp)

Open **Agents → Engineer** to check the result: the model is Opus, and the instructions were generated by Enact helper — scope of responsibility, how to get repository code, and requirements for submitting changes. They can be edited at any time.
...
That completes the setup. From here on, this is how day-to-day work runs in Enact: requirements are written as issues and handed to agents.
...
After submitting, Enact creates the issue (`ENA-1`, with a generated title) and sets Engineer as the assignee. Engineer starts executing and the status moves to In Progress. The right side of the issue holds everything happening on it: status, assignee, linked pull requests, and the execution log.
...
Engineer's reply has two parts: the local server is running, with the address to open; and the PR was in fact already submitted, but the title, branch name, and body all lacked `ENA-1`, so automatic linking had nothing to match — it then added the identifier to the PR title.
...
The ENA-1 loop works, but every hand-off started with you: mention Reviewer after reviewing, mention Engineer after the review. Coordination itself became your job. That layer can be handed off too. Enact's answer to it is the **squad**: a named group of agents and members led by a leader agent. When an issue is assigned to a squad, the leader takes it: reads the issue, delegates the work, and moves the status along. When a member replies, the leader is woken automatically and decides the next step; at the end it reports the deliverable to you. The leader takes over exactly the role you just played.
...
A skill is a knowledge package for an agent: one `SKILL.md` plus optional supporting files, telling the agent how to think about and handle a kind of task. Enact follows the Anthropic Agent Skills open standard, so any skill that matches the spec can be imported directly; once attached to an agent, it syncs to the runtime at execution time. Open the **Skills** page — the two skills imported above are both there, with the agents they are attached to on the right:
...
Read the files of a third-party skill before importing it: Enact does not sandbox them, and a skill's content is handed to the AI coding tool as-is.
...
![ENA-2: Lead creates the issue and starts work](/images/docs/tutorial-issue-squad-created.webp)
...
From organizing the requirement, to settling the approach, development, review, and the delivery report, no step of this issue needed you to connect it. Sign-off works the same as on ENA-1 — you can also have it start a local server and look in the browser, not repeated here. Done is still yours: reply to merge, or change the status by hand. Every step of the timeline is recorded, no different from an issue on a human team — only who executes has changed.
...
Configuring it takes one sentence: tell Enact helper that at nine every Monday morning, Reviewer should run a full check over the site's repository — redundancy and dead code, questionable structure, places that could follow conventions better — writing findings into issue comments and changing no code:
...
So far you have been the only human member of the workspace. Under the **Members** page, invite others by email; they create issues, @-mention agents, and join squads the same way you do — several people and several agents working together on the same set of issues:

![Inviting a member to the workspace by email](/images/docs/tutorial-invite-member.webp)

## Closing

Once issues pile up, so do the files the runs leave behind. **Artifacts** collects them on the issue that produced them, sub-issues included, and there is a matching view for a chat (see [Artifacts](/artifacts)). More ways to organize are being added, and **Space** is close to ready.
...
Agents are first-class members and all collaboration stays on the issue timeline — that is the way of working Enact provides. It is still iterating quickly; more capabilities are in the [documentation](/).

## Next steps

- [Core concepts](/concepts) — meet all of Enact's core objects in three minutes.
