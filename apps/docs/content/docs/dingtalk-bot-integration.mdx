...
After connecting an [agent](/agents) to DingTalk, members of the agent's workspace can DM the Bot, @-mention it in a group, or use the `/issue` command to create a Enact [issue](/issues) directly. Conversations and run records are saved to the Enact workspace.

<Callout type="info">
The DingTalk integration is [community-maintained](/community-maintained) and has no official support SLA. Report problems in [GitHub issues](https://github.com/enact-ai/enact/issues).
</Callout>
...
The **Robot Message Sending** permission (`qyapi_robot_sendmsg`) is added by default. To show the Bot's name in Enact, manually add the **DingTalk Group Basic Information Management** permission (`qyapi_chat_manage`).
...
1. Return to Enact and open **Agents → _your agent_ → Capabilities → Integrations**.
2. Click **Connect DingTalk**.
3. Enter the **AppKey** and **AppSecret**, then submit.

## 5. Link your DingTalk and Enact accounts
...
Send `/issue <title>` or `/issue <title>{newline}<detailed description>` to create a new Enact issue and automatically assign it to the agent connected to the Bot. Images in the same message are also added to the issue as attachments. Unlike regular chat messages, the `/issue` command leaves no record in Enact [Chat](/chat).
...
The `/new` command only makes the agent process messages in a fresh context. It does not create a new Enact Chat.
...
Account-linking URLs use `ENACT_APP_URL`, falling back to `FRONTEND_ORIGIN` when it is unset. The URL must point to a Enact address that members can access.
