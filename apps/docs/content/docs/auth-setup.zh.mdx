---
title: 登录与注册
description: 配置邮箱直登、Google 登录和新用户注册范围。
---

import { Callout } from "fumadocs-ui/components/callout";

Enact 默认使用邮箱直接登录，也可以增加 Google OAuth。已经存在的用户始终可以再次登录；注册限制只决定是否允许创建新账号。

## 邮箱直登

用户输入邮箱并继续后，Enact 会创建或复用对应账号，并立即签发登录 cookie，不再有验证码步骤。
