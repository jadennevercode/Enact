...
Enact はワークスペースごとにセルフホストの Git ホスティング、**Forgejo**、**Gitea**、**GitLab** のいずれかへ接続します。接続後、ブランチ名、タイトル、本文にイシュー番号（例: `MUL-123`）を含む Pull Request（GitLab では Merge Request）は、対応する[イシュー](/issues)へ**自動的に紐づけられ**、イシューサイドバーの **Pull requests** に表示されます。終了キーワードを含む PR がマージされると、イシューは「完了」へ移動します。head commit の CI はカード上のチェックバーとして表示されます。
