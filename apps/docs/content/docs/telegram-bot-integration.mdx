---
title: Telegram Bot
description: Connect a Enact agent to your own Telegram Bot for private chats, group mentions, topics, and /issue.
---

import { Callout } from "fumadocs-ui/components/callout";

Enact uses a Bot that you create with Telegram's official [@BotFather](https://t.me/BotFather). One Bot maps to one Enact agent. Create a separate Bot for each agent that needs its own Telegram identity.

Telegram support is [community-maintained](/community-maintained): it ships in every release, but it carries no official support SLA. Report problems in [GitHub issues](https://github.com/enact-ai/enact/issues).

## Before you start

- A Enact workspace owner or admin must connect the Bot.
...
4. Keep **Group Privacy** enabled. Enact only needs commands, explicit @-mentions, and replies to the Bot in groups.
...
1. In Enact, open **Agents**, select an agent, and open **Integrations**.
2. Click **Connect Telegram**.
3. Paste the Bot token and click **Connect**.

Enact calls Telegram to verify the Bot, checks that no outgoing webhook conflicts with long polling, encrypts the token, and starts one supervised `getUpdates` connection. A Bot already connected to another agent or workspace must be disconnected there first.

## First use and account binding

The first time a member messages the Bot, it sends a single-use Enact account-binding link. Open it, sign in to the same Enact workspace, then return to Telegram and send the message again. The link expires after 15 minutes; message the Bot again to get a new link.
...
- `/issue <title>` creates a Enact issue; following lines are used as its optional description. `/issue` without a title returns usage guidance.
...
Open **Settings → Integrations → Telegram** to view connected Bots. Owners and admins can disconnect one. Disconnecting stops long polling and outbound replies but preserves Enact conversations and audit records.
...
Binding links use `ENACT_APP_URL`, falling back to `FRONTEND_ORIGIN`. The resulting address must be reachable by members. The server's network or proxy must allow HTTPS access to `api.telegram.org`; Go honors standard `HTTPS_PROXY` and `NO_PROXY` environment variables.
...
- **409 polling conflict**: another Enact instance or process is polling the same Bot. Stop the other consumer or use a separate Bot per environment.
