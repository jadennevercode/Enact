...
description: Common server, storage, integration, and runtime configuration for self-hosting Enact.
---

import { Callout } from "fumadocs-ui/components/callout";

Enact reads environment variables at process startup. After changing one, restart the affected API, web, or daemon process. Docker Compose's `docker compose restart` does not re-read `.env`; recreate containers with `up -d` for changes to take effect.
...
```dotenv
DATABASE_URL=postgres://user:password@postgres:5432/enact?sslmode=require
JWT_SECRET=<long-random-secret>
APP_ENV=production
FRONTEND_ORIGIN=https://enact.example.com
ENACT_APP_URL=https://enact.example.com
ENACT_PUBLIC_URL=https://api.enact.example.com
```

Email delivery is optional and is only used for workspace invitations. Without it, invitation links are written to the server log.

<Callout type="warning">
`JWT_SECRET` is required in production: with `APP_ENV=production` the backend refuses to boot if it is empty or a known placeholder (generate one with `openssl rand -hex 32`).
</Callout>
...
|---|---|---|
| `DATABASE_URL` | local `enact` database | PostgreSQL connection string |
...
| `ENACT_DATABASE_STARTUP_TIMEOUT` | `3m` | Container-level retry budget shared by migration and API startup; set to `0` for one fail-fast attempt per phase |
| `ENACT_DATABASE_CONNECT_TIMEOUT` | `5s` | Fallback timeout for each startup connection attempt; native pgx `connect_timeout`, `PGCONNECT_TIMEOUT`, or service settings take precedence |
...
| `ENACT_SHUTDOWN_HOLD_DURATION` | `0` | How long to wait after a termination signal before graceful shutdown begins |
| `ENACT_RUNTIME_RECONNECT_GRACE` | `3h` | How long an offline runtime may reconnect before its in-flight tasks fail; values below `150s` are clamped |
...
| `ENACT_APP_URL` | falls back to `FRONTEND_ORIGIN` | User-reachable web URL; CLI sign-in and account-link URLs use it |
| `ENACT_PUBLIC_URL` | empty | Public API URL; used for webhook URLs and runtime connection instructions |
...
| `RESEND_FROM_EMAIL` | `noreply@enact.ai` | Sender address; must belong to a verified domain |
...
When `S3_BUCKET` is unset, Enact uses local disk.
...
| `RATE_LIMIT_AUTH` | `5` | Per-IP requests per minute to sign in by email or start Google sign-in |
...
| `ENACT_TRUSTED_PROXIES` | empty | Trusted proxy CIDRs for automation webhooks and realtime connections |
...
| Lark | `ENACT_LARK_SECRET_KEY` | Base64-encoded 32-byte credential encryption key |
| Slack | `ENACT_SLACK_SECRET_KEY` | Base64-encoded 32-byte token encryption key |
| Telegram | `ENACT_TELEGRAM_SECRET_KEY` | Base64-encoded 32-byte Bot token encryption key |
| Composio | `COMPOSIO_API_KEY` | Enables Composio tool connections |
| Composio | `COMPOSIO_CALLBACK_BASE_URL` | Callback API URL; can fall back to `ENACT_PUBLIC_URL` |
| Composio | `COMPOSIO_STATE_SECRET` | OAuth state signing secret; can be derived from `JWT_SECRET` |
| Self-hosted Git | `ENACT_VCS_INTEGRATION_ENABLED` | Forgejo/Gitea/GitLab integration switch; enabled by default in compose |
| Self-hosted Git | `ENACT_VCS_SECRET_KEY` | Base64-encoded 32-byte encryption key (`openssl rand -base64 32`); the feature is entirely unavailable without it |
...
Optional. The [code graph](/code-graph) service is a separate container (`docker-compose.codegraph.yml`); with these two variables empty the feature is disabled and the checkbox in Resources is greyed out.

| Variable | Default | Description |
|---|---|---|
| `ENACT_CODEGRAPH_SERVICE_URL` | empty | Internal URL of the code graph service, `http://codegraph:8767` in compose |
| `ENACT_CODEGRAPH_SERVICE_KEY` | empty | Shared key sent as `X-Codegraph-Service-Key`; must equal the container's `CODEGRAPH_SERVICE_KEY` |

The container reads `CODEGRAPH_SERVICE_KEY`, `CODEGRAPH_DATA_DIR` (`/data`), `CODEGRAPH_ALLOWED_HOSTS` (`github.com`), `CODEGRAPH_MAX_FILES` (`20000`) and `CODEGRAPH_BUILD_TIMEOUT_S` (`900`); see [Code graph → Deployment](/code-graph#deployment).
...
| Variable | Default | Description |
|---|---|---|
| `ENACT_LLM_API_KEY` | empty | OpenAI-compatible API key |
| `ENACT_LLM_BASE_URL` | empty | OpenAI-compatible endpoint |
| `ENACT_LLM_DEFAULT_MODEL` | `gpt-5.6-luna` | Used when a request does not specify a model |
| `ENACT_LLM_MAX_RETRIES` | `2` | Retry ceiling per call; `0` disables retries, `1`–`5` cap it at N |

`ENACT_LLM_MAX_RETRIES` is the single source for the retry policy. Leave it unset for the default of 2, set `0` to send exactly one request per call, or set 1–5 to cap retries at that many. It is a ceiling, not a quota: only retryable failures consume it, and a success or the caller's own deadline can end the call sooner. Any other value — negative, non-numeric, or above 5 — fails startup instead of being corrected silently. The ceiling is a latency budget: backoff starts at 0.5s and doubles to an 8s cap, so a larger budget would outlast the callers' own deadlines and turn a retryable failure into a timeout. Retries cover connection failures and HTTP 408, 409, 429 and 5xx; all other 4xx responses are returned as-is. The server logs the effective policy at startup as `llm retry policy`, with no credentials in the line.
...
<Callout type="info">
This covers the assist layer only. Running an agent is a separate data path: when an agent answers a chat, your daemon executes that agent's AI coding tool using the tool's own credentials, and does not forward the `ENACT_LLM_*` settings above to it. (The task-scoped Enact connection variables the agent needs are injected by the daemon separately.) Emptying the variables above does not affect that path — govern it through the agent's runtime configuration.
</Callout>
...
| Variable | Default | Description |
|---|---|---|
| `ENACT_SERVER_URL` | `ws://localhost:8080/ws` | Enact API / WebSocket URL; also accepts `http(s)` |
| `ENACT_DAEMON_DEVICE_NAME` | hostname | Device name shown in the runtime list |
| `ENACT_AGENT_RUNTIME_NAME` | `Local Agent` | Runtime display name |
| `ENACT_DAEMON_POLL_INTERVAL` | `30s` | Task polling interval when no wake event arrives |
| `ENACT_DAEMON_HEARTBEAT_INTERVAL` | `15s` | Heartbeat interval |
| `ENACT_DAEMON_MAX_CONCURRENT_TASKS` | `20` | Concurrent task ceiling per daemon |
| `ENACT_AGENT_TIMEOUT` | `0` | Absolute time limit per run; `0` means no limit |
| `ENACT_AGENT_IDLE_WATCHDOG` | `30m` | Silence ceiling with no output and no tool execution |
| `ENACT_AGENT_TOOL_WATCHDOG` | `2h` | Silence ceiling for a single tool call |
| `ENACT_OPENCODE_IDLE_WATCHDOG` | `10m` | OpenCode-specific silence threshold |
| `ENACT_CODEX_SEMANTIC_INACTIVITY_TIMEOUT` | `10m` | Codex semantic-silence threshold |
| `ENACT_CODEX_FIRST_TURN_TIMEOUT` | `0` | Explicit override for the Codex first-turn no-progress ceiling; `0` keeps the default. The effective first-turn wait stays bounded by `ENACT_CODEX_SEMANTIC_INACTIVITY_TIMEOUT` and the overall execution timeout — set `ENACT_CODEX_SEMANTIC_INACTIVITY_TIMEOUT` strictly above this value (with some margin), or the wait is truncated to it and the model-catalog startup retry is skipped. Equal values are not enough: the semantic timer is armed first, so at equal durations the retry can still be lost |
| `ENACT_CODEX_HANDSHAKE_TIMEOUT` | `30s` | Codex app-server startup handshake ceiling |
| `ENACT_DAEMON_AUTO_UPDATE` | Cloud `true`; self-hosted `false` | Whether to check for and apply CLI updates automatically |
| `ENACT_DAEMON_AUTO_UPDATE_INTERVAL` | `6h` | Update check interval |
| `ENACT_DAEMON_AUTO_RELOAD` | `true` | Whether to restart into a `enact` binary replaced on disk out of band (`brew upgrade`, a re-download, a local build). Independent of `ENACT_DAEMON_AUTO_UPDATE` |
| `ENACT_WORKSPACES_ROOT` | `~/enact_workspaces` | Root directory for task working directories |
| `ENACT_AGENT_TEMP_BASE` | `/tmp` (Linux/macOS) | Linux/macOS only. Parent directory for private per-task temp dirs; must be an existing, writable absolute directory, and an invalid value fails task startup instead of falling back to `/tmp`. Pick a short path — child tools may bind AF_UNIX sockets under it, and `sun_path` is limited to 108 bytes on Linux and 104 on macOS |
| `ENACT_KEEP_ENV_AFTER_TASK` | `false` | Keep task directories for debugging |

Each AI coding tool accepts `ENACT_<PROVIDER>_PATH` and `ENACT_<PROVIDER>_MODEL` to override the command path and default model. QwenPaw and MiniMax Code have no model variable because Enact never sends them a model; MiniMax Code's path variable is `ENACT_MCODE_PATH` — see [AI coding tools comparison](/providers#model-sources). DeepSeek Harness supports `ENACT_DSH_PATH` and `ENACT_DSH_MODEL` (a model id from the dsh catalog, e.g. `deepseek-official/deepseek-chat`). Dim supports `ENACT_DIM_PATH` and `ENACT_DIM_MODEL` (a model id from the dim catalog). Machine-wide default arguments via `ENACT_<PROVIDER>_ARGS` are currently supported for five tools: Claude Code, Codex, CodeBuddy, Qwen Code, and QwenPaw. The variables are `ENACT_CLAUDE_ARGS`, `ENACT_CODEX_ARGS`, `ENACT_CODEBUDDY_ARGS`, `ENACT_QWEN_ARGS`, and `ENACT_QWENPAW_ARGS`. For example:

```dotenv
ENACT_CLAUDE_PATH=/opt/bin/claude
ENACT_CLAUDE_ARGS=--max-turns 40
```

Precedence is command-line flag → environment variable → `~/.enact/config.json` → built-in default. For watchdog behavior, see [Daemon and runtimes](/daemon-runtimes).

### Persisting daemon configuration

Common daemon-side settings can also be written to `~/.enact/config.json` instead of relying on shell environment variables; named profiles keep theirs at `~/.enact/profiles/<name>/config.json`:

```bash
enact config set poll_interval 10s
enact config show
```
...
|---|---|---|
| `server_url` | `ws://localhost:8080/ws` | Enact API / WebSocket URL |
...
- Passing an empty string clears a persisted value and falls back to the env var or built-in default, for example `enact config set poll_interval ""`.
