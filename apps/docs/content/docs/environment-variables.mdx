...
`JWT_SECRET` is required in production: with `APP_ENV=production` the backend refuses to boot if it is empty or a known placeholder (generate one with `openssl rand -hex 32`). Do not set `ENACT_DEV_VERIFICATION_CODE`.
</Callout>
...
| `ENACT_DEV_VERIFICATION_CODE` | empty | Fixed 6-digit test code for non-production environments |
