...
| Code hosting | `ENACT_VCS_SECRET_KEY` | Base64-encoded 32-byte key (`openssl rand -base64 32`) that encrypts code-hosting tokens, webhook secrets and enterprise CAs; without it token connections cannot be saved |
