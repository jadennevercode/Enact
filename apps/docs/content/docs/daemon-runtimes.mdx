...
### Using one profile in several workspaces

A custom runtime profile belongs to the person who created it and can be published into several workspaces, instead of being retyped in each one.

- **Publishing** requires being both the profile's owner and an owner or admin of the target workspace. Nobody can pull your profile into their own workspace; you have to hand it over.
- **Editing** stays open to the owners and admins of a workspace while theirs is the only one using the profile, exactly as before. Once it reaches a second workspace, only the profile's owner may change it, because an edit is felt by every team running on it.
- **Disabling** has two switches. Each workspace has its own, which affects only that workspace; the owner has a global one that retires the definition everywhere. A daemon registers the runtime only when both are on.
- **Removing** a profile from a workspace withdraws it from that workspace only; other workspaces keep using it. The definition itself is deleted only when the last workspace using it withdraws.
