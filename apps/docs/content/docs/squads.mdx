...
A squad consists of one leader agent and any number of members. Assigning an issue to a squad does not run every agent at once. Enact wakes the leader first; it reads the context and decides who takes the next step.
...
When a non-Backlog issue is assigned to a squad, Enact immediately enqueues a `task` for the **leader agent** (not for every member). The flow then looks like this:

1. **Leader claims the task.** The agent runtime picks up the task on its next poll, same as any other agent assignment.
2. **Leader is briefed.** On claim, Enact appends three sections to the leader's system prompt — see [What the leader sees on every turn](#what-the-leader-sees-on-every-turn) below.
...
4. **Leader records its evaluation** via `enact squad activity <issue-id> action --reason "..."`. This writes an entry to the issue's activity timeline so humans can see the leader actually evaluated the trigger.
...
- **Squad Instructions** — your custom guidance for this squad (set on the squad detail page or via `enact squad update --instructions`). Use this for routing rules ("send DB work to Alice, frontend to Bob"), escalation policies, or anything else the leader needs to know that isn't already in the issue.
...
| A comment containing only an issue cross-reference (`[ENA-123](mention://issue/...)`) | **Yes** — issue references aren't routing |
...
<Callout type="info">
**Why the leader doesn't trigger when a member posts an `@`-mention.** Once a squad member directly `@`s someone, that comment is a deliberate hand-off — having the leader wake up to "observe" the routing would just produce a no-op turn and clutter the timeline. This also applies to agent-authored results. Mention the leader explicitly when the result needs its decision; a result receipt does not add another routing target. Stage barriers still wait for their configured completion conditions; `in_review` alone does not satisfy a barrier requiring `done` or `cancelled`.
</Callout>
...
@-mentioning a specific member directly hands the work to that member. Enact also dedupes repeated triggers and prevents the leader's comments from re-triggering itself, avoiding execution loops.
...
```bash
enact squad create --name "Product Delivery" --leader delivery-lead
