...
describe("sanitizeTabPath", () => {
  ...
  it("passes through valid workspace-scoped paths", () => {
    ...
    expect(sanitizeTabPath("/my-team/artifacts/abc")).toBe("/my-team/artifacts/abc");
  });
  ...
  it("passes through user slugs that happen to look path-like but aren't reserved", () => {
    ...
    expect(sanitizeTabPath("/project-x/chat")).toBe("/project-x/chat");
  });
  ...
  it("folds the viewer's own surfaces into their Home tab", () => {
    expect(sanitizeTabPath("/acme/inbox")).toBe("/acme/home?tab=inbox");
    expect(sanitizeTabPath("/acme/my-issues")).toBe("/acme/home?tab=my-issues");
  });
  ...
  it("groups v1 flat tabs by workspace slug", () => {
    const v1 = {
      tabs: [
        ...
        { id: "t2", path: "/acme/artifacts", title: "Artifacts", icon: "FolderKanban" },
...
describe("useTabStore actions", () => {
  ...
  it("switchWorkspace without openPath restores the group's last active tab", () => {
    ...
    const acmeArtifactsId = useTabStore.getState().byWorkspace.acme.tabs[1].id;
    store.setActiveTab(acmeArtifactsId);
    ...
    const s = useTabStore.getState();
    expect(s.activeWorkspaceSlug).toBe("acme");
    expect(s.byWorkspace.acme.activeTabId).toBe(acmeArtifactsId);
  });
  ...
  it("openTab dedupes by resourceKey within the active workspace", () => {
    const store = useTabStore.getState();
    store.switchWorkspace("acme");
    const id1 = store.openTab("/acme/artifacts", "Artifacts");
    const id2 = store.openTab("/acme/artifacts", "Artifacts");
    expect(id1).toBe(id2);
    expect(useTabStore.getState().byWorkspace.acme.tabs).toHaveLength(2); // default + artifacts
  });
  ...
  describe("openTab insertion position (ENA-5860)", () => {
    ...
    it("inserts the new tab immediately right of the active tab, not at the end", () => {
      ...
      store.addTab("/acme/autopilots", "C");
      expect(urls()).toEqual(["/acme/issues", "/acme/artifacts", "/acme/autopilots"]);
    ...
    it("a pinned opener's 'right' is the start of the unpinned zone", () => {
      ...
      const s = useTabStore.getState();
      expect(urls()).toEqual(["/acme/issues", "/acme/issues/d", "/acme/artifacts"]);
    ...
    it("the explicit new-tab button (addTab) still appends", () => {
      ...
      store.addTab("/acme/artifacts", "B");
      store.addTab("/acme/autopilots", "C"); // active is still A — must append anyway
      expect(urls()).toEqual(["/acme/issues", "/acme/artifacts", "/acme/autopilots"]);
    });
    ...
    it("a dedupe hit focuses the existing tab without reordering", () => {
      ...
      store.openTab("/acme/autopilots", "C again"); // hits C, at the far end
      const s = useTabStore.getState();
      expect(urls()).toEqual(["/acme/issues", "/acme/artifacts", "/acme/autopilots"]);
      expect(getActiveTab(s)?.url).toBe("/acme/autopilots");
    });
  });
...
describe("navigateActiveSession", () => {
  it("updates url, resourceKey, and pushes onto the virtual history", () => {
    ...
    store.navigateActiveSession("/acme/artifacts?sort=name");

    const active = getActiveTab(useTabStore.getState())!;
    expect(active.url).toBe("/acme/artifacts?sort=name");
    expect(active.resourceKey).toBe("/acme/artifacts");
  ...
  it("replace swaps the current history entry instead of pushing", () => {
    ...
    store.navigateActiveSession("/acme/artifacts?sort=name", { replace: true });
  ...
  it("a push after going back truncates the forward stack", () => {
    ...
    store.navigateActiveSession("/acme/autopilots");

    const active = getActiveTab(useTabStore.getState())!;
    expect(active.history).toEqual({
      stack: ["/acme/issues", "/acme/autopilots"],
      index: 1,
    });
  });
  ...
  it("goBack/goForward project the history stack back into the url", () => {
    ...
    expect(active.url).toBe("/acme/artifacts");
...
describe("commitScrollMemento", () => {
  ...
  it("preserves view-state entries across a scroll REPLACE", () => {
    ...
    store.switchWorkspace("acme");
    const tabId = useTabStore.getState().byWorkspace.acme.tabs[0].id;
    store.commitViewState(tabId, "/acme/autopilots", "highlight:i1", "c1");
    store.commitScrollMemento(tabId, "/acme/autopilots", {
      list: { top: 500, height: 8000 },
    });
    ...
    expect(useTabStore.getState().byWorkspace.acme.tabs[0].memento).toEqual({
      scroll: {},
      view: { "/acme/autopilots::highlight:i1": "c1" },
    });
  });
...
describe("commitViewState", () => {
  ...
  it("clears an entry when the value is undefined, leaving others intact", () => {
    ...
    const tabId = useTabStore.getState().byWorkspace.acme.tabs[0].id;
    store.commitViewState(tabId, "/acme/autopilots", "highlight:i1", "c1");
    store.commitViewState(tabId, "/acme/autopilots", "highlight:i2", "c2");

    store.commitViewState(tabId, "/acme/autopilots", "highlight:i1", undefined);

    expect(useTabStore.getState().byWorkspace.acme.tabs[0].memento.view).toEqual({
      "/acme/autopilots::highlight:i2": "c2",
    });
  });
  ...
  it("skips the store write when the value is unchanged", () => {
    ...
    store.commitViewState(tabId, "/acme/autopilots", "highlight:i1", "c1");
    store.commitViewState(tabId, "/acme/autopilots", "highlight:absent", undefined);
  ...
  it("evicts the oldest entries beyond the cap", () => {
    ...
    const tabId = useTabStore.getState().byWorkspace.acme.tabs[0].id;
    for (let i = 0; i < 101; i++) {
      store.commitViewState(tabId, "/acme/autopilots", `highlight:i${i}`, "c");
    }

    const view = useTabStore.getState().byWorkspace.acme.tabs[0].memento.view;
    expect(Object.keys(view)).toHaveLength(100);
    expect(view["/acme/autopilots::highlight:i0"]).toBeUndefined();
    expect(view["/acme/autopilots::highlight:i100"]).toBe("c");
  });
  ...
  it("preserves scroll entries when a view-state entry commits", () => {
    ...
    expect(useTabStore.getState().byWorkspace.acme.tabs[0].memento).toEqual({
      scroll: { "/acme/autopilots::list": { top: 500, height: 8000 } },
      view: { "/acme/autopilots::highlight:i1": "c1" },
    });
  });
});
...
describe("bulk tab closing", () => {
  it("closes other unpinned tabs, preserves pinned tabs, and activates the target", () => {
    ...
    store.closeOtherTabs(artifactsId);

    const group = useTabStore.getState().byWorkspace.acme;
    expect(group.tabs.map((tab) => tab.id)).toEqual([issuesId, artifactsId]);
    expect(group.activeTabId).toBe(artifactsId);
  });
...
describe("closeTab activation order (ENA-5665)", () => {
  ...
  it("keeps each workspace's visit history to itself", () => {
    ...
    const acmeArtifactsId = store.addTab("/acme/artifacts", "Artifacts");
    store.setActiveTab(acmeArtifactsId);
  ...
  it("reseeding the last tab of a workspace starts a fresh visit history", () => {
    ...
    store.closeTab(artifactsId); // last tab — reseeds the default
...
describe("togglePin", () => {
  ...
  it("returns an unpinned tab to the start of the unpinned zone", () => {
    ...
    const tabs = useTabStore.getState().byWorkspace.acme.tabs;
    expect(tabs.map((t) => t.id)).toEqual([artifactsId, issuesId]);
...
describe("migrateV2ToV3", () => {
  it("adds pinned=false to every persisted tab", () => {
    ...
    expect(v3.byWorkspace.acme.tabs).toEqual([
      ...
      { id: "t2", path: "/acme/artifacts", title: "Artifacts", icon: "FolderKanban", pinned: false },
    ]);
  });
...
describe("mergePersistedTabs (rehydration, ENA-4370)", () => {
  ...
  it("ignores an unknown or corrupted persisted icon", () => {
    const tab = rehydrate(persistedTab("/acme/artifacts", { icon: "NotARealIcon" }));
    expect(tab).not.toHaveProperty("icon");
    expect(tab.url).toBe("/acme/artifacts");
  });
  ...
  it("normalizes a memento persisted without view-state entries", () => {
    const tab = rehydrate(
      persistedTab("/acme/autopilots", {
        memento: { scroll: { "/acme/autopilots::list": { top: 5, height: 100 } } },
      }),
    );
    expect(tab.memento).toEqual({
      scroll: { "/acme/autopilots::list": { top: 5, height: 100 } },
      view: {},
    });
  });
  ...
  it("sanitizes an MRU order carrying dropped, duplicate, or active tab ids", () => {
    const group = rehydrateGroup(
      "t2",
      { t1: "/acme/issues", t2: "/acme/artifacts" },
      ["gone", "t1", "t1", "t2", 7],
    );
