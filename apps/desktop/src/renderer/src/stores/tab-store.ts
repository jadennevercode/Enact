...
import { createPersistStorage, defaultStorage } from "@enact/core/platform";
import { createSafeId } from "@enact/core/utils";
import { isReservedSlug } from "@enact/core/paths";
 ...
 * enact-ai#6405) to invalidate restoration when the content changed
...
// render time via `routeIconForPath` (@enact/views/layout), which shares the
// route → icon map in `@enact/core/paths` with the sidebar nav — so the two
