...
import { CoreProvider } from "@enact/core/platform";
import { pickLocale, type SupportedLocale } from "@enact/core/i18n";
...
import { ThemeProvider } from "@enact/ui/components/common/theme-provider";
...
import { captureEvent } from "@enact/core/analytics";
...
function AppContent() {
  ...
  // can pick the matching CLI profile (server_url from ~/.enact config).
  useEffect(() => {
    if (!runtimeConfig) return;
    window.daemonAPI.setTargetApiUrl(runtimeConfig.apiUrl);
  }, [runtimeConfig]);

  // Listen for invite IDs delivered via deep link (enact://invite/<id>).
  ...
  // Listen for auth token delivered via deep link (enact://auth/callback?token=...).
  ...
  useEffect(() => {
    ...
    const token = localStorage.getItem("enact_token");
    if (!token) return;
    ...
    <div className="flex h-screen items-center justify-center bg-background p-8 text-foreground">
      <div className="max-w-xl rounded-lg border bg-card p-6 shadow-sm">
        ...
        <p className="mt-3 text-body text-muted-foreground">
          Enact Desktop could not load <code>~/.enact/desktop.json</code>. Fix or remove the file and restart the app.
        </p>
