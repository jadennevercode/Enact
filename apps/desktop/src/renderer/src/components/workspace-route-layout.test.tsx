...
vi.mock("@enact/core/workspace/pending-delete", () => ({
  isWorkspaceDeletePending: (id: string) => state.pendingDeletes.has(id),
}));

vi.mock("@enact/core/workspace", async () => {
  const actual = await vi.importActual<typeof import("@enact/core/workspace")>(
    "@enact/core/workspace",
  );
  return {
    ...actual,
...
vi.mock("@enact/views/workspace/use-workspace-seen", () => ({
  useWorkspaceSeen: () => state.workspaceSeen,
}));

vi.mock("@enact/views/workspace/welcome-after-onboarding", () => ({
...
vi.mock("@enact/views/onboarding", () => ({
  SourceBackfillModal: () => {
