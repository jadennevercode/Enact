...
describe("runtime config", () => {
  it("uses cloud defaults without a desktop.json file", () => {
    expect(DEFAULT_RUNTIME_CONFIG).toEqual({
      schemaVersion: 1,
      apiUrl: "https://api.enact.ai",
      wsUrl: "wss://api.enact.ai/ws",
  ...
  it("strips the leading api. label when deriving appUrl", () => {
    expect(
      parseRuntimeConfig(
        JSON.stringify({ schemaVersion: 1, apiUrl: "https://api.enact.ai" }),
      ),
    ).toEqual({
      schemaVersion: 1,
      apiUrl: "https://api.enact.ai",
  ...
  it("rejects non-http api schemes", () => {
    expect(() =>
      parseRuntimeConfig(JSON.stringify({ schemaVersion: 1, apiUrl: "file:///tmp/enact" })),
  ...
  it("derives dev appUrl by stripping the leading api. label", () => {
    ...
    // public web host, not the api host. Enact's convention exposes the
    // api at `api.<web-host>`, so stripping the leading label gives the
    // right web origin without a separate VITE_APP_URL.
    expect(
      runtimeConfigFromDevEnv({ apiUrl: "https://api.test.enact.ai" }),
    ).toEqual({
      schemaVersion: 1,
      apiUrl: "https://api.test.enact.ai",
      wsUrl: "wss://api.test.enact.ai/ws",
      appUrl: "https://test.enact.ai",
    });
  });
