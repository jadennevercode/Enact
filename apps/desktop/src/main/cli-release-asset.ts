const RELEASE_ARCHIVE_PREFIX = "enact-cli-";
...
export function selectPlatformReleaseAssetName(
  ...
  // Prefer the versioned `enact-cli-<v>-<os>-<arch>.<ext>` name; fall
  // back to the legacy `enact_<os>_<arch>.<ext>` so older releases that
  ...
  const legacyName = `enact_${os}_${mappedArch}.${ext}`;
