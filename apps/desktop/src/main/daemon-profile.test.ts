...
const ENACT_DIR = join(homedir(), ".enact");
const DEFAULT_CLI_CONFIG = join(ENACT_DIR, "config.json");

describe("deriveProfileName", () => {
  it("names the profile after the target host", () => {
    expect(deriveProfileName("https://api.enact.ai")).toBe(
      "desktop-api.enact.ai",
    );
  });
...
describe("profile paths", () => {
  it("always resolves under profiles/<name>", () => {
    const dir = join(ENACT_DIR, "profiles", "desktop-api.enact.ai");
    expect(profileDir("desktop-api.enact.ai")).toBe(dir);
    expect(profileConfigPath("desktop-api.enact.ai")).toBe(
      join(dir, "config.json"),
    );
    expect(profileLogPath("desktop-api.enact.ai")).toBe(
      join(dir, "daemon.log"),
    );
    expect(profileUserIdPath("desktop-api.enact.ai")).toBe(
      join(dir, ".desktop-user-id"),
    );
  });

  // Regression: an unresolved profile used to resolve to ~/.enact, so Desktop
  ...
  it("never yields the default CLI config path for any input", () => {
    for (const name of ["desktop-api.enact.ai", "desktop", "x"]) {
...
describe("profileArgs", () => {
  it("selects the Desktop-owned profile", () => {
    expect(profileArgs("desktop-api.enact.ai")).toEqual([
      "--profile",
      "desktop-api.enact.ai",
    ]);
  });
...
describe("healthPortForProfile", () => {
  ...
  it("never derives the default profile's port", () => {
    for (const name of ["desktop-api.enact.ai", "desktop", "x", "a".repeat(50)]) {
      expect(healthPortForProfile(name)).not.toBe(DEFAULT_HEALTH_PORT);
    }
  });

  it("derives a stable per-profile port above the default", () => {
    const port = healthPortForProfile("desktop-api.enact.ai");
    expect(port).toBeGreaterThan(DEFAULT_HEALTH_PORT);
    expect(port).toBe(healthPortForProfile("desktop-api.enact.ai"));
  });
});
