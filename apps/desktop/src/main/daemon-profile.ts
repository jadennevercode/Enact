...
// Keep the Go impl in sync: server/cmd/enact/cmd_daemon.go healthPortForProfile.
export const DEFAULT_HEALTH_PORT = 19514;

/**
 * Desktop owns only `~/.enact/profiles/desktop-<host>/`. The default profile
 * at `~/.enact/` — config, daemon log, and health port 19514 — belongs to the
...
export function profileDir(profile: string): string {
  assertResolvedProfile(profile);
  return join(homedir(), ".enact", "profiles", profile);
}
...
// Sidecar file that records which Enact user the cached PAT in config.json
 ...
 * user's default profile at `~/.enact/config.json`.
 */
