...
const PREFS_PATH = join(homedir(), ".enact", "desktop_prefs.json");
...
// `enact daemon start` blocks until the daemon reports ready, polling /health
// for up to its own startup timeout (45s in server/cmd/enact/cmd_daemon.go) to
 ...
 * a stuck start is an auth problem. Hits the same endpoint `enact auth status`
 ...
 * the user's default CLI profile at `~/.enact/` — neither its files nor its
 * health port.
 */
...
function findCliOnPath(): string | null {
  const candidates = process.platform === "win32" ? ["enact.exe"] : ["enact"];
 ...
 *   to `apps/desktop/resources/bin/enact`. `bundle-cli.mjs` populates this
 *   before dev starts, so iterating on Go changes is "make build → restart".
 * - Packaged: `app.getAppPath()` → `<Enact.app>/Contents/Resources/app.asar`.
...
function bundledCliPath(): string {
  const binName = process.platform === "win32" ? "enact.exe" : "enact";
...
/**
 * Returns a usable `enact` binary path. Priority:
 ...
 *   5. `enact` on PATH (dev convenience / user-installed via brew).
 ...
 * daemon needs a PAT (or `enact_` / `mdt_` token) because JWTs expire in 30
...
async function mintPat(jwt: string): Promise<string> {
  ...
  const res = await fetch(url, {
    ...
    body: JSON.stringify({ name: "Enact Desktop" }),
  });
  if (!res.ok) {
  ...
  if (typeof data.token !== "string" || !data.token.startsWith("enact_")) {
 ...
 * - If the profile already has a cached PAT (`enact_...`) AND the sidecar user
...
async function syncToken(
  ...
  const sameUserWithCachedPat =
    ...
    config.token.startsWith("enact_");

  let finalToken: string;
  if (tokenFromRenderer.startsWith("enact_")) {
...
function desktopSpawnEnv(): NodeJS.ProcessEnv {
  return { ...process.env, ENACT_LAUNCHED_BY: "desktop" };
}

async function startDaemon(): Promise<{ success: boolean; error?: string }> {
  const bin = await resolveCliBinary();
  if (!bin) return { success: false, error: "enact CLI is not installed" };
