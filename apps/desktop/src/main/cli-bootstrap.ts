...
// Desktop prefers the bundled `enact` CLI shipped inside the app for
...
const GITHUB_LATEST_BASE =
  "https://github.com/enact-ai/enact/releases/latest/download";

function binaryName(): string {
  return process.platform === "win32" ? "enact.exe" : "enact";
}
...
async function installFresh(): Promise<string> {
  ...
  const workDir = join(tmpdir(), `enact-cli-${Date.now()}`);
...
/**
 * Returns the path to a usable `enact` binary. If one is already present at
