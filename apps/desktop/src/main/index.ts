...
// enact CLI can find agent binaries like claude/codex/opencode. Must run
...
function installLocaleRefresh(window: BrowserWindow): void {
  // Electron has no dedicated OS-language event. Check whenever any Enact
...
// single-worktree case still lands at "Enact Canary".
const DEV_APP_NAME = process.env.DESKTOP_APP_SUFFIX
  ? `Enact Canary ${process.env.DESKTOP_APP_SUFFIX}`
  : "Enact Canary";
...
if (is.dev) {
  ...
  // to "Enact", but anchoring it here makes WM_CLASS ↔ StartupWMClass
...
if (!gotTheLock) {
  ...
  app.whenReady().then(async () => {
    ...
    electronApp.setAppUserModelId(
      is.dev ? "ai.enact.desktop.dev" : "ai.enact.desktop",
    );
