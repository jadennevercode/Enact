...
// (keyed by the app name "Enact Canary"). The env hooks to override both
...
// "/tmp/enact-3494" is 1189739000, and 1189739000 % 1000 === 0). Range 5174–6173.
...
// the primary checkout keeps the unchanged 5173 / "Enact Canary" defaults.
  ...
  if (log) {
    console.log(
      `[dev:desktop] worktree isolation → renderer port ${env.DESKTOP_RENDERER_PORT}, ` +
        `app "Enact Canary ${env.DESKTOP_APP_SUFFIX}"`,
    );
  }
  return env;
}
