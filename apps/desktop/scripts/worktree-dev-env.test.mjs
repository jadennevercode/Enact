...
describe("worktree-dev-env", () => {
  ...
  it("suffix is '<folder>-<offset>' so it stays recognizable and unique", () => {
    expect(appSuffixForPath("/work/ENA-3724_Desktop")).toBe(
      `ena-3724-desktop-${offsetForPath("/work/ENA-3724_Desktop")}`,
    );
  ...
  it("disambiguates worktrees that share a folder name at different paths", () => {
    // Same basename "enact", different parent dirs → different offsets/suffixes,
    // so each gets its own single-instance lock.
    expect(offsetForPath("/tmp/a/enact")).not.toBe(offsetForPath("/tmp/b/enact"));
    expect(appSuffixForPath("/tmp/a/enact")).not.toBe(
      appSuffixForPath("/tmp/b/enact"),
    );
  });
