...
POSTGRES_DB ?= enact
POSTGRES_USER ?= enact
POSTGRES_PASSWORD ?= enact
POSTGRES_PORT ?= 5432
PORT := $(or $(BACKEND_PORT),$(API_PORT),$(SERVER_PORT),$(PORT),8080)
ifeq ($(origin ENACT_PUBLIC_URL), undefined)
...
ENACT_SERVER_URL ?= ws://localhost:$(PORT)/ws
...
selfhost: ## Create .env if needed, then pull and start the official self-hosted images
	...
	@if [ ! -f .env ]; then \
		...
		if [ "$$(uname)" = "Darwin" ]; then \
			...
			sed -i '' "s#^ENACT_VCS_SECRET_KEY=.*#ENACT_VCS_SECRET_KEY=$$VCSKEY#" .env; \
		else \
			sed -i "s/^JWT_SECRET=.*/JWT_SECRET=$$JWT/" .env; \
			sed -i "s/^POSTGRES_PASSWORD=.*/POSTGRES_PASSWORD=$$PGPASS/" .env; \
			sed -i -E "s#^(DATABASE_URL=postgres://[^:]+:)[^@]*(@.*)#\1$$PGPASS\2#" .env; \
		...
		echo "==> Generated random JWT_SECRET, POSTGRES_PASSWORD, and ENACT_VCS_SECRET_KEY"; \
	fi
	@echo "==> Pulling official Enact images..."
	@if ! $(COMPOSE) -f docker-compose.selfhost.yml pull; then \
		echo ""; \
		echo "Official images for tag '$${ENACT_IMAGE_TAG:-latest}' are not published yet."; \
		echo "If this is before the first GHCR release, build from the current checkout:"; \
		echo "  make selfhost-build"; \
		exit 1; \
	fi
	@echo "==> Starting Enact via Docker Compose..."
...
selfhost-build: ## Build backend/web from the current checkout and start the self-hosted stack
	...
	@echo "==> Building Enact from the current checkout..."
...
daemon: ## Restart the local agent daemon using the CLI's stored auth/session
	@$(MAKE) enact ENACT_ARGS="daemon restart --profile local"

cli: ## Run the enact CLI with ARGS or ENACT_ARGS from source
	@$(MAKE) enact ENACT_ARGS="$(ENACT_ARGS)"

enact: ## Run the enact CLI entrypoint directly from the Go source tree
	cd server && go run -ldflags "-X main.version=$(VERSION) -X main.commit=$(COMMIT) -X main.date=$(DATE)" ./cmd/enact $(ENACT_ARGS)
...
build: ## Build the server, CLI, and migrate binaries into server/bin
	...
	cd server && go build -ldflags "-X main.version=$(VERSION) -X main.commit=$(COMMIT) -X main.date=$(DATE)" -o bin/enact$(EXE) ./cmd/enact
