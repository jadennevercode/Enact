...
This document covers advanced configuration for self-hosted Enact deployments. For the quick start guide, see [SELF_HOSTING.md](SELF_HOSTING.md).
...
|----------|-------------|---------|
| `DATABASE_URL` | PostgreSQL connection string | `postgres://enact:enact@localhost:5432/enact?sslmode=disable` |
...
| `RESEND_FROM_EMAIL` | Sender email address (default: `noreply@enact.ai`) |
...
If the frontend and backend are served from different hostnames, `COOKIE_DOMAIN` is **required**, not optional: the browser must be able to read the `enact_csrf` cookie from the page's own origin to send the `X-CSRF-Token` header, and without it every write request fails with `403 {"error":"CSRF validation failed"}`. Scope it to the narrowest parent domain that covers both hosts — it also shares the `enact_auth` session cookie with every host under that domain. See [Reverse Proxy](#reverse-proxy) for the full split-domain configuration and its trust requirements.
...
| Variable | Default | Description |
|----------|---------|-------------|
| `ENACT_WECOM_TRACE` | empty (off) | `1` records every WeCom frame the backend reads and writes, including the first 120 runes of each message body. Anything else is off. |
...
| Variable | Default | Description |
|----------|---------|-------------|
| `ENACT_SERVER_URL` | `ws://localhost:8080/ws` | WebSocket URL for daemon → server connection |
| `ENACT_APP_URL` | `http://localhost:3000` | Frontend URL for CLI login flow |
| `ENACT_DAEMON_POLL_INTERVAL` | `3s` | How often the daemon polls for tasks |
| `ENACT_DAEMON_HEARTBEAT_INTERVAL` | `15s` | Heartbeat frequency |
...
| `ENACT_COPILOT_PATH` | Custom path to the `copilot` (GitHub Copilot CLI) binary |
...
| `ENACT_GROK_PATH` | Custom path to the `grok` binary |
| `ENACT_GROK_MODEL` | Override the Grok model used (e.g. `grok-4.5`) |

## Database Setup

Enact requires PostgreSQL 17 with the pgvector extension.
...
# Kubernetes
kubectl -n enact exec deploy/enact-backend -- \
  ./backfill_task_usage_hourly --sleep-between-slices=2s
```
...
By default, Enact works on `localhost`. If you access it from another machine on the LAN (e.g. `http://192.168.1.100:3000`), you need to tell the backend to accept that origin:
...
Pin `ENACT_IMAGE_TAG` in `.env` to an exact release like `v0.2.4` if you want to stay on a specific version. Migrations run automatically on backend startup. They are idempotent — running them multiple times has no effect.
