...
cat >"$local_env" <<'EOF'
POSTGRES_DB=enact_feature_123
POSTGRES_USER=enact
DATABASE_URL=postgres://enact:enact@localhost:5432/enact_feature_123?sslmode=disable
EOF
...
require_contains "$docker_log" \
  "compose exec -T postgres dropdb --username enact --maintenance-db postgres --if-exists --force -- enact_feature_123"
require_contains "$output" "Dropped database 'enact_feature_123'."
...
DATABASE_URL=postgres://enact:enact@localhost:5432/postgres
EOF
if printf 'y\n' | PATH="$stub_dir:$PATH" DOCKER_LOG="$docker_log" \
...
cat >"$main_env" <<'EOF'
POSTGRES_DB=enact
DATABASE_URL=postgres://enact:enact@localhost:5432/enact
EOF
if printf 'y\n' | PATH="$stub_dir:$PATH" DOCKER_LOG="$docker_log" \
...
POSTGRES_DB=enact_worktree_456
POSTGRES_USER=enact
DATABASE_URL=postgres://enact:enact@localhost:5432/enact_worktree_456?sslmode=disable
EOF
...
require_contains "$docker_log" \
  "compose exec -T postgres dropdb --username enact --maintenance-db postgres --if-exists --force -- enact_worktree_456"
