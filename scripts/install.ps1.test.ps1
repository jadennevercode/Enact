...
$runnerScript = Join-Path ([System.IO.Path]::GetTempPath()) "enact-install-ps1-case.ps1"
...
if (-not $env:USERPROFILE) { $env:USERPROFILE = [System.IO.Path]::GetTempPath() }
$env:ENACT_INSTALL_DIR = $WorkDir
$env:ENACT_SELFHOST_REF = "main"
...
foreach ($case in $cases) {
    $workDir = Join-Path ([System.IO.Path]::GetTempPath()) ("enact-ps1-" + [guid]::NewGuid().ToString("N"))
