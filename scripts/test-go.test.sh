...
TEST_DIR=$(mktemp -d "${TMPDIR:-/tmp}/enact-test-go.XXXXXX")
...
mkdir -p "$BIN_DIR"
export ENACT_TEST_GO_CALLS="$CALLS_FILE"
...
case "${1:-}" in
  list)
    ...
    printf '%s\n' \
      github.com/enact-ai/enact/server \
      github.com/enact-ai/enact/server/internal/daemon \
      github.com/enact-ai/enact/server/pkg/agent \
      github.com/enact-ai/enact/server/pkg/agent/internal/testutil
    ;;
  test)
    printf '%s\n' "$*" >>"$ENACT_TEST_GO_CALLS"
    ;;
...
expected_calls='test -race github.com/enact-ai/enact/server github.com/enact-ai/enact/server/internal/daemon
