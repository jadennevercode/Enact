...
_setup_sandbox() {
  ...
  cat >"$payload_dir/enact" <<'STUB'
#!/usr/bin/env bash
echo "enact v0.3.2 (commit: test)"
STUB
  chmod +x "$payload_dir/enact"
  tar -czf "$tmp/enact.tar.gz" -C "$payload_dir" enact

  cat >"$stub_bin/curl" <<'STUB'
#!/usr/bin/env bash
if [[ "$*" == *"-sI"* ]]; then
  printf 'HTTP/2 302\r\nlocation: https://github.com/enact-ai/enact/releases/tag/v0.3.2\r\n'
  exit 0
fi
...
_run_installer() {
  ...
  if ! PATH="$tmp/stub-bin:$tmp/install-bin:/usr/bin:/bin" \
    ENACT_BIN_DIR="$tmp/install-bin" \
    ENACT_TEST_ARCHIVE="$tmp/enact.tar.gz" \
  ...
  if [[ ! -x "$tmp/install-bin/enact" ]]; then
    echo "expected fallback binary at $tmp/install-bin/enact" >&2
    cat "$out" >&2 || true
    cat "$err" >&2 || true
    return 1
  fi
...
STUB
  ...
  if ! grep -q "https://enact.ai/settings?tab=tokens" "$tmp/install.out"; then
  ...
  if ! grep -q "enact login --token <YOUR_TOKEN>" "$tmp/install.out"; then
    echo "expected token login command in installer output" >&2
    cat "$tmp/install.out" >&2 || true
    return 1
  fi
  if grep -q "enact config set server_url" "$tmp/install.out"; then
    echo "did not expect default cloud server config command in installer output" >&2
    cat "$tmp/install.out" >&2 || true
    return 1
  fi
  if grep -q "enact config set app_url" "$tmp/install.out"; then
...
_run_with_server() {
  ...
  if ! env -i \
    ...
    HOME="$tmp" \
    ENACT_INSTALL_DIR="$tmp/server" \
    ENACT_SELFHOST_REF="main" \
    ENACT_TEST_CURL_LOG="$tmp/curl.log" \
    "$@" \
