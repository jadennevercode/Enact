# Enact installer for Windows — one command to get started.
#
# Install CLI (default): connects to enact.ai
#   irm https://raw.githubusercontent.com/enact-ai/enact/main/scripts/install.ps1 | iex
#
# Self-host: starts a local Enact server + installs CLI + configures
#   $env:ENACT_MODE="local"; irm https://raw.githubusercontent.com/enact-ai/enact/main/scripts/install.ps1 | iex
#
...
# Configuration
# ---------------------------------------------------------------------------
$RepoUrl       = "https://github.com/enact-ai/enact.git"
$RepoWebUrl    = "https://github.com/enact-ai/enact"
$DefaultInstallDir = Join-Path $env:USERPROFILE ".enact\server"
$InstallDir    = if ($env:ENACT_INSTALL_DIR) { $env:ENACT_INSTALL_DIR } else { $DefaultInstallDir }
...
function Get-LatestVersion {
    try {
        $release = Invoke-RestMethod -Uri "https://api.github.com/repos/enact-ai/enact/releases/latest" -ErrorAction Stop
...
function Get-SelfHostRef {
    if ($env:ENACT_SELFHOST_REF) {
        return $env:ENACT_SELFHOST_REF
    }
...
function Get-InstalledCliVersion {
    try {
        $firstLine = enact version 2>$null | Select-Object -First 1
...
function Install-CliBinary {
    Write-Info "Installing Enact CLI from GitHub Releases..."

    if (-not [Environment]::Is64BitOperatingSystem) {
        Write-Fail "Enact requires a 64-bit Windows installation."
    }
    ...
    $url = "https://github.com/enact-ai/enact/releases/download/$latest/enact-cli-$version-windows-$arch.zip"
    $tmpDir = Join-Path ([System.IO.Path]::GetTempPath()) "enact-install"
    ...
    try {
        Invoke-WebRequest -Uri $url -OutFile (Join-Path $tmpDir "enact.zip") -UseBasicParsing
    } catch {
    ...
    $checksumUrl = "https://github.com/enact-ai/enact/releases/download/$latest/checksums.txt"
    try {
        ...
        $zipFile = Join-Path $tmpDir "enact.zip"
        $actualHash = (Get-FileHash -Path $zipFile -Algorithm SHA256).Hash.ToLower()
        $releaseAsset = "enact-cli-$version-windows-$arch.zip"
        $legacyAsset = "enact_windows_$arch.zip"
    ...
    Expand-Archive -Path (Join-Path $tmpDir "enact.zip") -DestinationPath $tmpDir -Force

    $binDir = Join-Path $env:USERPROFILE ".enact\bin"
    if (-not (Test-Path $binDir)) {
        New-Item -ItemType Directory -Path $binDir -Force | Out-Null
    }

    $exeSrc = Join-Path $tmpDir "enact.exe"
    if (-not (Test-Path $exeSrc)) {
        $exeSrc = Get-ChildItem -Path $tmpDir -Filter "enact.exe" -Recurse | Select-Object -First 1 -ExpandProperty FullName
    }
    if (-not $exeSrc -or -not (Test-Path $exeSrc)) {
        Remove-Item $tmpDir -Recurse -Force
        Write-Fail "enact.exe not found in downloaded archive."
    }

    Copy-Item $exeSrc (Join-Path $binDir "enact.exe") -Force
    Remove-Item $tmpDir -Recurse -Force

    Add-ToUserPath $binDir
    Write-Ok "Enact CLI installed to $binDir\enact.exe"
}
...
function Install-Cli {
    if (Test-CommandExists "enact") {
        ...
        if ($isUpToDate) {
            Write-Ok "Enact CLI is up to date ($currentVer)"
            return
        }

        Write-Info "Enact CLI $currentVer installed, latest is $latestVer - upgrading..."
        Install-CliBinary

        $newVer = Get-InstalledCliVersion
        Write-Ok "Enact CLI upgraded ($currentVer -> $newVer)"
        return
    }

    Install-CliBinary

    if (-not (Test-CommandExists "enact")) {
        Write-Fail "CLI installed but 'enact' not found on PATH. Restart your terminal and try again."
    }
}
...
function Test-Docker {
    if (-not (Test-CommandExists "docker")) {
        Write-Fail @"
Docker is not installed. Enact self-hosting requires Docker and Docker Compose.

Install Docker Desktop for Windows:
  https://docs.docker.com/desktop/install/windows-install/

After installing Docker, re-run this script with `$env:ENACT_MODE="local"`.
"@
    }
...
function Install-Server {
    Write-Info "Setting up Enact server..."
    ...
    if (Test-Path (Join-Path $InstallDir ".git")) {
        ...
        Write-Info "Cloning Enact repository..."
    ...
    Write-Info "Pulling official Enact images..."
    Pull-OfficialSelfHostImages
    Write-Info "Starting Enact services (this may take a few minutes on first run)..."
    ...
    if ($ready) {
        Write-Ok "Enact server is running"
    } else {
...
function Start-DefaultInstall {
    Write-Host ""
    Write-Host "  Enact - Installer" -ForegroundColor White
    Write-Host ""
    ...
    Write-Host "  [OK] Enact CLI is ready!" -ForegroundColor Green
    Write-Host "  ============================================" -ForegroundColor Green
    Write-Host ""
    Write-Host "  Next: configure your environment"
    Write-Host ""
    Write-Host "     enact setup               " -NoNewline; Write-Host "# Connect to Enact Cloud (enact.ai)" -ForegroundColor DarkGray
    Write-Host "     enact setup self-host      " -NoNewline; Write-Host "# Connect to a self-hosted server" -ForegroundColor DarkGray
    Write-Host ""
    Write-Host "  Self-hosting? Install the server first:"
    Write-Host '     $env:ENACT_MODE="with-server"; irm https://raw.githubusercontent.com/enact-ai/enact/main/scripts/install.ps1 | iex'
    Write-Host ""
}
...
function Start-LocalInstall {
    Write-Host ""
    Write-Host "  Enact - Self-Host Installer" -ForegroundColor White
    ...
    Write-Host ""
    Write-Host "  ============================================" -ForegroundColor Green
    Write-Host "  [OK] Enact server is running and CLI is ready!" -ForegroundColor Green
    Write-Host "  ============================================" -ForegroundColor Green
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "     enact setup self-host  " -NoNewline; Write-Host "# Configure + authenticate + start daemon" -ForegroundColor DarkGray
    Write-Host ""
    ...
    Write-Host '     $env:ENACT_MODE="stop"; irm https://raw.githubusercontent.com/enact-ai/enact/main/scripts/install.ps1 | iex'
    Write-Host ""
}
...
function Start-Stop {
    Write-Host ""
    Write-Info "Stopping Enact services..."
    ...
    if (Test-Path $InstallDir) {
        ...
        Write-Warn "No Enact installation found at $InstallDir"
    }

    if (Test-CommandExists "enact") {
        try {
            enact daemon stop 2>$null
...
# Entry point
# ---------------------------------------------------------------------------
$mode = if ($env:ENACT_MODE) { $env:ENACT_MODE.ToLower() } else { "default" }
