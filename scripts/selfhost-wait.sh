...
echo ""
echo "✓ Enact is running!"
echo "  Frontend: ${frontend_url}"
echo "  Backend:  ${backend_url}"
echo ""
if [ "$mode" = "build" ]; then
  echo "Built images locally via docker-compose.selfhost.build.yml."
  echo "Local tags: enact-backend:dev and enact-web:dev."
else
  echo "Images: ${ENACT_BACKEND_IMAGE:-ghcr.io/enact-ai/enact-backend}:${ENACT_IMAGE_TAG:-latest}"
  echo "        ${ENACT_WEB_IMAGE:-ghcr.io/enact-ai/enact-web}:${ENACT_IMAGE_TAG:-latest}"
fi
echo ""
...
echo "  brew install enact-ai/tap/enact"
echo "  enact setup self-host"
