...
- Name: Enact. Mark: a Deloitte-green ring cut twice on the diagonal, holding three slanted bars that read as an E. The source is `design-system/enact/mark-source.svg`; `scripts/generate-brand-mark.mjs` copies it into `docs/assets/logo-light.svg`, `docs/assets/logo-dark.svg`, `apps/web/public/favicon.svg`, `apps/web/public/icons/icon.svg` and the web, mobile and docs components. The colour mark carries one vertical gradient, Deep Green to Deloitte Green, on a transparent ground; the monochrome `currentColor` mark stays for inline use beside text. App icons sit on the light shell colour with the green light, never on a dark ground. Enact keeps its own name and mark in every context.
