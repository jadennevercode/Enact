...
This guide documents the local development workflow for contributors working on the Enact codebase.
...
By submitting a contribution to Enact — a pull request, a patch, or any
other work — you agree to condition 2 of the [Enact License](LICENSE):

- your contribution is submitted under the Enact License as a whole (the
...
- the producer can adjust the Enact License to be more strict or relaxed
  as deemed necessary.
...
- the main checkout usually uses `.env` and `POSTGRES_DB=enact`
...
POSTGRES_DB=enact
POSTGRES_PORT=5432
DATABASE_URL=postgres://enact:enact@localhost:5432/enact?sslmode=disable
PORT=8080
FRONTEND_PORT=3000
```
...
```bash
POSTGRES_DB=enact_my_feature_702
POSTGRES_PORT=5432
PORT=18782
FRONTEND_PORT=13702
DATABASE_URL=postgres://enact:enact@localhost:5432/enact_my_feature_702?sslmode=disable
```
...
```bash
git worktree add ../enact-feature -b feat/my-change main
cd ../enact-feature
make dev
```
...
```bash
make remove-worktree WORKTREE=../enact-feature
```
...
The daemon authenticates using the CLI's stored token (`enact login`).
...
whatever server is configured in `~/.enact/config.json`. That's fine for
...
Example: worktree at `../enact-feat-auth` produces profile
`dev-enact_feat_auth-347`, matching that worktree's port and database
allocation.
...
CONFIG_DIR="$HOME/.enact/profiles/$PROFILE"
mkdir -p "$CONFIG_DIR"
...
local backend. Agent-executed `enact` commands automatically use the same
...
rm -rf "$HOME/.enact/profiles/$PROFILE"
```
...
1. Compiles the `enact` CLI from `server/cmd/enact` into
   `apps/desktop/resources/bin/enact`
...
- `DESKTOP_APP_SUFFIX` = `<folder>-<offset>` — its own single-instance lock /
  `userData`, and an app named `Enact Canary <folder>-<offset>` so it is
  distinguishable in Cmd+Tab. The offset keeps it unique across worktrees that
  share a folder name at different paths.

The primary checkout is left untouched (`5173`, `Enact Canary`). Set either
...
Nothing in this flow touches the system-installed `enact` or the default
`~/.enact/config.json`:

| Resource | System / Production | Local Dev (per-worktree) |
|---|---|---|
| Config | `~/.enact/config.json` | `~/.enact/profiles/dev-<slug>-<hash>/config.json` |
| Daemon PID | `~/.enact/daemon.pid` | `~/.enact/profiles/dev-<slug>-<hash>/daemon.pid` |
| Health port | `19514` | `19514 + 1 + (name_hash % 1000)` |
| Workspaces dir | `~/enact_workspaces/` | `~/enact_workspaces_dev-<slug>-<hash>/` |
| Database | remote / production | local Docker: `enact_<slug>_<hash>` |
| Desktop profile | `desktop-api.enact.ai` | `desktop-localhost-<port>` |
...
```bash
docker compose exec -T postgres psql -U enact -d postgres -At -c "select datname from pg_database order by datname;"
```
...
database `enact` unless `ALLOW_MAIN_DB_DROP=1` is explicitly supplied.
