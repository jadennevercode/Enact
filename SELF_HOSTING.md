...
Deploy Enact on your own infrastructure in minutes.
...
Each user who runs AI agents locally also installs the **`enact` CLI** and runs the **agent daemon** on their own machine.
...
```powershell
# 1. Install CLI + provision the self-host server
$env:ENACT_MODE="with-server"; irm https://raw.githubusercontent.com/enact-ai/enact/main/scripts/install.ps1 | iex
...
This installs the `enact` CLI, checks out the latest self-host assets, pulls the official Enact images from GHCR, and configures everything for localhost.
...
`make selfhost-build` uses local `enact-backend:dev` / `enact-web:dev` tags, so it does not overwrite the pulled `:latest` images.
...
- [DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness) (`dsh` on PATH with the Enact runtime profile installed; set `DEEPSEEK_API_KEY`)
...
If you already run a Kubernetes cluster, you can deploy Enact there instead of Docker Compose using the released OCI Helm chart at `oci://ghcr.io/enact-ai/charts/enact` or the source chart at [`deploy/helm/enact/`](deploy/helm/enact/). It targets a typical k3s / k8s setup with an Ingress controller and a default `ReadWriteOnce` StorageClass — authored against k3s + Traefik + `local-path`, and should work on any cluster with minor tweaks.

The chart creates the following resources in the target namespace:

- `enact-postgres` — `pgvector/pgvector:pg17` backed by a 10Gi PVC
- `enact-backend` — Go API/WS server. Backed by a 5Gi `ReadWriteOnce` uploads PVC by default; set `backend.uploads.persistence.enabled=false` when you have configured S3 (`backend.config.s3Bucket`) and don't want the chart to declare the PVC at all.
- `enact-frontend` — Next.js standalone server
- Two `Ingress` resources: one for the web host, one for the backend host
- `enact-config` ConfigMap (rendered from `values.yaml`)

The `enact-secrets` Secret is **not** managed by the chart — you create it once with `kubectl` so real values never need to land in git.

> **Runtime frontend upstreams:** current `enact-web` images read `REMOTE_API_URL` and `DOCS_URL` when the Next.js server runs, so API/docs upstream changes do not require a web rebuild. The chart defaults `REMOTE_API_URL` to this release's backend Service. `frontend.compatibility.backendAlias` exists only for legacy images that still baked `REMOTE_API_URL=http://backend:8080` at build time.
...
The chart defaults to `enact.dev.lan` (web) and `api.enact.dev.lan` (backend). Pick one of:

- **`/etc/hosts`** on every machine that needs access (developer laptops + the machine running the daemon):

  ```text
  192.168.1.206  enact.dev.lan api.enact.dev.lan
  ```
...
### Step 3 — Create the `enact-secrets` Secret

The chart references this Secret by name. Create it once with random values:

```bash
kubectl -n enact create secret generic enact-secrets \
...
```bash
helm install enact oci://ghcr.io/enact-ai/charts/enact \
...
```bash
helm show values oci://ghcr.io/enact-ai/charts/enact \
...
```bash
helm install enact deploy/helm/enact -n enact
```

Watch the pods come up:

```bash
kubectl -n enact get pods -w
```
...
```bash
curl -H "Host: api.enact.dev.lan" http://<ingress-ip>/healthz
# {"status":"ok","checks":{"db":"ok","migrations":"ok"}}
```

Then open http://enact.dev.lan in your browser.
...
```bash
enact setup self-host \
  --server-url http://api.enact.dev.lan \
  --app-url http://enact.dev.lan
```
...
```bash
kubectl -n enact rollout restart deploy/enact-backend deploy/enact-frontend
```

To upgrade to a specific Enact release, upgrade to the matching chart version. The released chart defaults its app images to the matching Git tag:
...
helm -n enact uninstall enact

# Wipe everything, including PostgreSQL data and uploads
kubectl delete namespace enact
```
...
3. Leave the `pg_cron` extension itself installed unless you are sure no other workload depends on it. The bundled `pgvector/pgvector:pg17` image does **not** ship `pg_cron`, so nothing in Enact's default install needs it; uninstalling `pg_cron` from a custom image that other workloads still use is a separate decision.
...
```bash
curl -fsSL https://raw.githubusercontent.com/enact-ai/enact/main/scripts/install.sh | bash -s -- --stop
```
...
## Switching to Enact Cloud

If you've been self-hosting and want to switch your CLI to [Enact Cloud](https://enact.ai):

```bash
enact setup
```

This reconfigures the CLI for enact.ai, re-authenticates, and restarts the daemon. You will be prompted before overwriting the existing configuration.
...
If you prefer configuring the CLI step by step instead of `enact setup`:
