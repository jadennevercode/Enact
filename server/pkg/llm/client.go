...
// not forward this layer's ENACT_LLM_* settings to it. (It does inject the
// task-scoped Enact connection variables the agent itself needs — see
// mergeEnv in pkg/agent, which drops the daemon's inherited ENACT_* and then
...
// let THIS layer send chat content leaves ENACT_LLM_API_KEY and
// ENACT_LLM_BASE_URL empty; the product stays whole (client-derived chat
...
type Config struct {
	// APIKey authenticates against the upstream. Maps to ENACT_LLM_API_KEY.
	APIKey string
	...
	// ENACT_LLM_DEFAULT_MODEL. When empty, FallbackModel is used.
	DefaultModel string
	// MaxRetries is the transport-level retry budget applied to every request
	// this client makes. Maps to ENACT_LLM_MAX_RETRIES. Build one with
