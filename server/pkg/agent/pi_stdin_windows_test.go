...
const (
	piShimHelperEnv      = "ENACT_PI_SHIM_HELPER"
	piShimHelperRPCEnv   = "ENACT_PI_SHIM_RPC_HELPER"
	piShimHelperArgvFile = "ENACT_PI_SHIM_ARGV_FILE"
	piShimHelperInFile   = "ENACT_PI_SHIM_STDIN_FILE"
)
...
func runPiRPCHelper() {
	...
	for scanner.Scan() {
		...
		case strings.Contains(line, `"get_state"`):
			fmt.Println(`{"id":"enact-state","type":"response","command":"get_state","success":true,"data":{"model":{"id":"reasoning-model","provider":"provider","reasoning":true,"thinkingLevelMap":{"xhigh":"xhigh","max":"max"}},"thinkingLevel":"max"}}`)
		case strings.Contains(line, `"get_available_models"`):
			fmt.Println(`{"id":"enact-models","type":"response","command":"get_available_models","success":true,"data":{"models":[{"id":"reasoning-model","provider":"provider","reasoning":true,"thinkingLevelMap":{"xhigh":"xhigh","max":"max"}}]}}`)
		}
	}
}
...
func assertPiPromptSurvivesShim(t *testing.T) {
	...
	prompt := "ENACT_AGENT_BUILDER_INPUT\n" +
	...
	for _, arg := range strings.Split(strings.TrimSuffix(string(argvRaw), "\n"), "\n") {
		for _, needle := range []string{"ENACT_AGENT_BUILDER_INPUT", "instructions", "-X", "local work"} {
