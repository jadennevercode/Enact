...
func TestBuildPiArgsIgnoresSystemPrompt(t *testing.T) {
	args := buildPiArgs("/tmp/s.jsonl", ExecOptions{
		SystemPrompt: "the entire enact runtime brief",
	}, slog.Default())
	...
	for _, a := range args {
		...
		if a == "the entire enact runtime brief" {
...
func TestStripPiToolCallMarkup(t *testing.T) {
	tests := map[string]string{
		...
		`before response:bash{command:<|"|>enact issue comment list issue-id --all --output json<|"|>} after`: "before  after",
...
// slash (`claude/claude-opus-5` under provider `enact-anthropic`). Splitting
...
func TestBuildPiArgsPassesQualifiedSelectorWhole(t *testing.T) {
	args := buildPiArgs("/tmp/s.jsonl", ExecOptions{Model: "enact-anthropic/claude/claude-opus-5"}, slog.Default())

	joined := strings.Join(args, " ")
	if !strings.Contains(joined, "--model enact-anthropic/claude/claude-opus-5") {
