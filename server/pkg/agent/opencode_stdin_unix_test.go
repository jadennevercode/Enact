...
func TestOpencodeExecuteSendsPromptOnStdinNotArgv(t *testing.T) {
	...
	for _, arg := range argv {
		for _, needle := range []string{"ENACT_AGENT_BUILDER_INPUT", "user_request", "-X", "inspect local"} {
...
func TestOpencodeExecutePromptExceedingWindowsCommandLineLimit(t *testing.T) {
	...
	prompt := strings.Repeat("enact opencode workspace skill catalogue 0123456789\n", 800)
	if len(prompt) <= 32767 {
...
func TestOpencodeExecuteLargePromptDoesNotDeadlock(t *testing.T) {
	...
	prompt := strings.Repeat("enact opencode stdin payload 0123456789\n", 16_384)
