...
// dshBackend drives the Enact DSH bundle over its versioned JSONL
...
// this package only translates those events into Enact's Backend contract.
...
func (b *dshBackend) Execute(ctx context.Context, prompt string, opts ExecOptions) (*Session, error) {
	...
	if requestID == "" {
		requestID = "enact-" + strconv.FormatInt(time.Now().UnixNano(), 10)
	}
