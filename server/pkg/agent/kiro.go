...
func (b *kiroBackend) Execute(ctx context.Context, prompt string, opts ExecOptions) (*Session, error) {
	...
	// (goal_complete or `enact issue comment add`). We record the status
	...
	c := &hermesClient{
		...
		onMessage: func(msg Message) {
			...
			if msg.Type == MessageToolUse {
				...
				// Recognize `enact issue comment add` by its command
...
// `enact issue comment add` invocation. It keys purely off the command
...
func isKiroIssueCommentAddCommand(command string) bool {
	...
	// `sh -c "enact issue comment add ..."`; unwrap a single such layer so
	...
	if executable != "enact" && !strings.HasSuffix(executable, "/enact") {
		return false
	}
...
// like `ENACT_TOKEN=x enact issue comment add ...` is still recognized.
