...
// server into the snake_case identifiers the Enact UI expects.
//
...
func selectReasonixPermissionOption(params json.RawMessage) (optionID string, grant bool, ok bool, blockedQuestion string) {
	...
	if reasonixPermissionIsQuestion(p) {
		reason := "Reasonix requested interactive user input, which is unavailable in an unattended Enact task"
