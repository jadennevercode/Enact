...
// model and consumes API quota. It exercises the complete Enact backend,
...
func TestDshRealRuntimeSmoke(t *testing.T) {
	if os.Getenv("ENACT_RUN_REAL_AGENT_SMOKE") != "1" {
		t.Skip("set ENACT_RUN_REAL_AGENT_SMOKE=1 to run the DSH integration smoke")
	}
	path := os.Getenv("ENACT_DSH_PATH")
	if path == "" {
		t.Fatal("ENACT_DSH_PATH is required")
	}
	...
	defer cancel()
	cwd := t.TempDir()
	session, err := b.Execute(ctx, "Remember the code word PINEAPPLE. Reply with exactly DSH_ENACT_OK and nothing else.", ExecOptions{
	...
	if strings.TrimSpace(result.Output) != "DSH_ENACT_OK" {
