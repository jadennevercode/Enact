...
// through this package's own backend with a Enact-shaped agent.mcp_config
...
func TestKimiRealMcpConfigReachesSessionSmoke(t *testing.T) {
	...
	mcpConfig := fmt.Sprintf(`{"mcpServers":{"enactprobe":{"command":%q,"args":[],"env":{}}}}`, serverPath)

	ctx, cancel := context.WithTimeout(context.Background(), 240*time.Second)
	defer cancel()
	session, err := backend.Execute(ctx,
		"Call the enact_probe_ping tool and reply with exactly what it returned. Do nothing else.",
		ExecOptions{
	...
	if !strings.Contains(result.Output, "ENACT_MCP_OK") {
...
while IFS= read -r line; do
  ...
  case "$line" in
    *'"method":"initialize"'*)
      printf '{"jsonrpc":"2.0","id":%s,"result":{"protocolVersion":"2025-06-18","capabilities":{"tools":{"listChanged":false}},"serverInfo":{"name":"enact-probe","version":"1.0.0"}}}\n' "$id"
      ;;
    *'"method":"tools/list"'*)
      printf '{"jsonrpc":"2.0","id":%s,"result":{"tools":[{"name":"enact_probe_ping","description":"Returns ENACT_MCP_OK.","inputSchema":{"type":"object","properties":{},"additionalProperties":false}}]}}\n' "$id"
      ;;
    *'"method":"tools/call"'*)
      printf '{"jsonrpc":"2.0","id":%s,"result":{"content":[{"type":"text","text":"ENACT_MCP_OK"}],"isError":false}}\n' "$id"
      ;;
