...
// the enact CLI version reported by the daemon at registration time. The
...
// binding, no-retry semantics on `enact issue create` failure — see PR
...
var (
	ErrCLIVersionMissing = errors.New("enact CLI version not reported by daemon")
	ErrCLIVersionTooOld  = errors.New("enact CLI version is below required minimum")
)
