...
func TestMergeEnvFiltersClaudeCodeVars(t *testing.T) {
	...
	env := mergeEnv([]string{
		...
		"ENACT_SERVER_URL=https://daemon.example",
	}, map[string]string{
		"FOO":                "bar",
	...
	if !found["ENACT_SERVER_URL=https://task.example"] || !found["ENACT_TOKEN=mat_task"] {
		t.Fatalf("expected explicit task ENACT_* values to be appended, got %v", env)
	}
}
...
func TestResumeWasRejected(t *testing.T) {
	...
	cases := []struct {
		...
		{
			// Verbatim from enact-ai/enact#5704 (Claude Code 2.1.207,
...
func TestBuildClaudeArgsManagedSkillSettingsWins(t *testing.T) {
	args := buildClaudeArgs(ExecOptions{
		ClaudeSettingsPath: "/tmp/enact-claude-settings.json",
	...
	if !strings.Contains(joined, "--settings /tmp/enact-claude-settings.json") {
