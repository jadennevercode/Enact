...
const (
	...
	// while keeping the fast-fail value (ENA-5542).
...
// stop handing the same oversized thread to the next task (ENA-5722).
...
// downgrading a user's isolation opt-in (ENA-4957).
...
const (
	enactCodexMcpBeginMarker = "# BEGIN enact-managed mcp_servers (do not edit; regenerated by daemon)"
	enactCodexMcpEndMarker   = "# END enact-managed mcp_servers"
)

var codexMcpBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(enactCodexMcpBeginMarker) +
		`.*?^` + regexp.QuoteMeta(enactCodexMcpEndMarker) + `\n*`)
...
func ensureCodexMcpConfig(configPath string, mcpConfig json.RawMessage, logger *slog.Logger) error {
	...
	if managed {
		...
		if block == "" {
			block = enactCodexMcpBeginMarker + "\n" + enactCodexMcpEndMarker + "\n"
		}
		if stripped == "" {
			updated = block
		} else {
...
func renderCodexMcpServersBlock(raw json.RawMessage) (string, bool, error) {
	...
	sb.WriteString(enactCodexMcpBeginMarker)
	sb.WriteString("\n")
	...
	sb.WriteString(enactCodexMcpEndMarker)
	sb.WriteString("\n")
...
func isEnactMcpSelectorKey(k string) bool {
	switch k {
...
func (b *codexBackend) Execute(ctx context.Context, prompt string, opts ExecOptions) (*Session, error) {
	...
	go func() {
		...
		for attempt := 1; attempt <= 2; attempt++ {
			...
			// thread that never produced a turn (ENA-5110). The first non-pin
...
func (b *codexBackend) executeOnce(ctx context.Context, prompt string, opts ExecOptions, attempt int) (*Session, error) {
	...
	go func() {
		...
		if err := scanner.Err(); err != nil {
			...
			// resume rather than a crash (ENA-5722).
	...
	go func() {
		...
		_, err := c.request(runCtx, "initialize", map[string]any{
			"clientInfo": map[string]any{
				"name":    "enact-agent-sdk",
				"title":   "Enact Agent SDK",
				"version": "0.2.0",
			},
		...
		// this covers the ones only the live resume reveals (ENA-4424).
		//
		...
		// still readable, which this package cannot see (ENA-5722).
		...
		// ENA-2339 — Trump's constraint that the three injection points
		...
		// there rather than in the shared ~/.codex/sessions (ENA-4424).
...
// the conversation is still readable (ENA-5722).
...
func (c *codexClient) startOrResumeThread(ctx context.Context, opts ExecOptions, logger *slog.Logger) (string, bool, error) {
	if priorThreadID := opts.ResumeSessionID; priorThreadID != "" {
		...
		// agent's thinking_level since. See ENA-2339 — Elon flagged that
...
// (per Trump's ENA-2339 review constraint).
//
...
//
// An explicit configured override (ENACT_CODEX_FIRST_TURN_TIMEOUT) is honored
...
// retry re-execs codex, which is the only way back (ENA-5722).
//
