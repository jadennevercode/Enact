...
func (b *claudeBackend) Execute(ctx context.Context, prompt string, opts ExecOptions) (*Session, error) {
	...
	go func() {
		...
		if sawAsyncLaunch {
			completionGuardError = "claude launched an async background task; Enact-managed runs require foreground execution"
		}
...
var resumeRejectedPhrases = []string{
	...
	// Reported verbatim in enact-ai/enact#5704 against Claude Code
...
func claudeRootSudoPreflight(args, env []string) error {
	...
	return fmt.Errorf("Claude Code refuses bypassPermissions under root/sudo privileges. Run the Enact daemon as a non-root user, or set IS_SANDBOX=1 if running in a genuine container/sandbox")
}
...
func mergeEnv(base []string, extra map[string]string) []string {
	...
	for _, entry := range base {
		...
		// ENACT_* in the daemon's own environment is not task context. Drop
		// the inherited namespace for every backend and append only the values
		// daemon.go explicitly assembled for this task below.
		if isFilteredChildEnvKey(key) || strings.HasPrefix(strings.ToUpper(key), "ENACT_") {
			continue
		}
...
func writeMcpConfigToTemp(raw json.RawMessage) (string, error) {
	dir, err := os.MkdirTemp("", "enact-mcp-*")
	if err != nil {
...
func cleanupMcpConfigTemp(path string) {
	...
	if strings.HasPrefix(filepath.Base(dir), "enact-mcp-") {
		_ = os.RemoveAll(dir)
		return
	}
	_ = os.Remove(path)
