...
// write (GitHub enact#5300). It prefers a session-scoped grant over a
...
func TestHermesClientAutoApprovesPermissionRequest(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			// exactly "allow_once"; this is the enact#5300 regression
...
// failures from GitHub enact#5862.
//
...
//     completed the requested work, then Enact suddenly flips the same
...
// second pollution shape from GitHub enact#5862. Hermes echoes a
...
// enact#1952 scenario: the upstream LLM returns HTTP 429 (rate
...
// fix for GitHub enact#1952: a hermes run that hits a 429 (or any
...
// GitHub enact#4010: session/resume succeeds and echoes back the
...
// fix for GitHub enact#4010: when a resumed session turns out to be
...
// regression GPT-Boy flagged on the enact#1952 fix: a per-attempt
...
func TestHermesClientEmitsToolUseWhenStartFrameCarriesCommand(t *testing.T) {
	...
	c.handleLine(`{"jsonrpc":"2.0","method":"session/update","params":{"sessionId":"ses_1","update":{"content":[{"content":{"text":"$ enact issue get 7473e16c --output json","type":"text"},"type":"content"}],"kind":"execute","locations":[],"title":"terminal: enact issue get 7473e16c --output json","toolCallId":"tc-305462f5d67d","sessionUpdate":"tool_call"}}}`)
	...
	if text, _ := got[0].Input["text"].(string); text != "$ enact issue get 7473e16c --output json" {
