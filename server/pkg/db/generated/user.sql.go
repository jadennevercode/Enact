...
func (q *Queries) CreatePasswordUser(ctx context.Context, arg CreatePasswordUserParams) (User, error) {
	row := q.db.QueryRow(ctx, createPasswordUser, arg.Name, arg.Email, arg.PasswordHash)
	var i User
	err := row.Scan(
		&i.ID,
		&i.Name,
...
const setInitialUserPassword = `-- name: SetInitialUserPassword :one
UPDATE "user" SET
    name = $2,
    password_hash = $3,
    updated_at = now()
...
type SetInitialUserPasswordParams struct {
	ID           pgtype.UUID `json:"id"`
	Name         string      `json:"name"`
	PasswordHash pgtype.Text `json:"password_hash"`
}

func (q *Queries) SetInitialUserPassword(ctx context.Context, arg SetInitialUserPasswordParams) (User, error) {
	row := q.db.QueryRow(ctx, setInitialUserPassword, arg.ID, arg.Name, arg.PasswordHash)
	var i User
	err := row.Scan(
		&i.ID,
		&i.Name,
