...
// ~/.enact/profiles/<name>/daemon.log while ~/.enact/daemon.log may still
...
func TestDaemonLogSourcePathResolvesPerProfile(t *testing.T) {
	t.Run("default profile", func(t *testing.T) {
		...
		want := filepath.Join(home, ".enact", "daemon.log")
		if got != want {
	...
	t.Run("named profile", func(t *testing.T) {
		home := mkProfiles(t, "desktop-api.enact.ai")

		got, err := daemonLogSourcePath("desktop-api.enact.ai")
		if err != nil {
			t.Fatalf("daemonLogSourcePath = %v, want nil", err)
		}
		want := filepath.Join(home, ".enact", "profiles", "desktop-api.enact.ai", "daemon.log")
		if got != want {
			t.Fatalf("daemonLogSourcePath = %q, want %q", got, want)
		}
	})
...
func TestRunDaemonLogsAnnouncesResolvedPath(t *testing.T) {
	cases := []struct {
		...
		{"named profile", "desktop-api.enact.ai", "desktop-api.enact.ai", 50, false},
		{"named profile following", "desktop-api.enact.ai", "desktop-api.enact.ai", 50, true},
	}
...
// The announcement goes to stderr, so `enact daemon logs | grep ...` and a
...
func TestRunDaemonLogsMissingFileNamesProfilePath(t *testing.T) {
	...
	seedDaemonLog(t, filepath.Join(home, ".enact", "daemon.log"))

	cmd, errOut := daemonLogsCmdFor(t, "desktop-api.enact.ai", 50, false)
	rec := stubTailLog(t, errOut)
