...
func init() {
	...
	// `enact agent env set <id>` — that path admits the agent owner or a
...
func newAPIClient(cmd *cobra.Command) (*cli.APIClient, error) {
	...
	if taskContext && !strings.HasPrefix(token, "mat_") {
		// When the ONLY daemon signal is a workdir marker (no ENACT_AGENT_ID /
		// ENACT_TASK_ID / ENACT_DAEMON_PORT), the likeliest cause outside a
		...
		if markerPath := leftoverDaemonTaskMarkerPath(); markerPath != "" {
			return nil, fmt.Errorf("agent execution context requires ENACT_TOKEN to be a task-scoped mat_ token%s", leftoverMarkerSuffix(markerPath))
		}
		return nil, fmt.Errorf("agent execution context requires ENACT_TOKEN to be a task-scoped mat_ token%s", daemonPortOnlyContextHint())
	}

	serverURL := resolveServerURL(cmd)
	workspaceID := resolveWorkspaceID(cmd)
	if serverURL == "" {
		return nil, fmt.Errorf("server URL not set: use --server-url flag, ENACT_SERVER_URL env, or 'enact config set server_url <url>'")
	}

	client := cli.NewAPIClient(serverURL, workspaceID, token)
	// When running inside a daemon task, attribute actions to the agent.
	if agentID := os.Getenv("ENACT_AGENT_ID"); agentID != "" {
		client.AgentID = agentID
	}
	if taskID := os.Getenv("ENACT_TASK_ID"); taskID != "" {
...
const (
	defaultCloudServerURL = "https://api.enact.ai"
	defaultCloudAppURL    = "https://enact.ai"
)
...
// ENACT_DAEMON_PORT in a host/container environment must not hide the human
...
func tryResolveExplicitServerURL(cmd *cobra.Command) string {
	val := cli.FlagOrEnv(cmd, "server-url", "ENACT_SERVER_URL", "")
	if val == "" {
		return ""
	}
...
func missingServerConfigMessage() string {
	return fmt.Sprintf("No server configured. Run 'enact setup' first%s.", daemonPortOnlyContextHint())
}
...
func resolveHumanServerURL(cmd *cobra.Command) string {
	...
	fmt.Fprintln(os.Stderr, "No server configured. Run 'enact setup' first.")
	os.Exit(1)
	return "" // unreachable
}
...
func inAgentExecutionContext() bool {
	return os.Getenv("ENACT_AGENT_ID") != "" || os.Getenv("ENACT_TASK_ID") != ""
}

// inDaemonManagedExecutionContext reports whether the CLI is being invoked
// from inside a daemon-managed agent task. ENACT_DAEMON_PORT is included as
// a defense-in-depth marker for subprocesses that lose ENACT_AGENT_ID or
// ENACT_TASK_ID but still run under the daemon environment. In this context
// workspace and token must come from daemon-provided env; falling back to
// user-global ~/.enact/config.json can make agent writes land as a member.
func inDaemonManagedExecutionContext() bool {
	return inAgentExecutionContext() || os.Getenv("ENACT_DAEMON_PORT") != "" || hasDaemonTaskContextMarker()
}

// inDaemonTaskIdentityContext reports strong evidence that the current process
// belongs to a daemon-managed task. ENACT_DAEMON_PORT is deliberately not
...
func daemonPortOnlyContextHint() string {
	if strings.TrimSpace(os.Getenv("ENACT_DAEMON_PORT")) == "" || inDaemonTaskIdentityContext() {
		return ""
	}
	return "; ENACT_DAEMON_PORT is set without task identity — if this is a host or container startup shell, remove that variable and retry"
}

// requireTaskLocalConfigRoot prevents daemon-managed subprocesses that lost
// part of their injected environment from silently resolving Enact state
...
func requireTaskLocalConfigRoot() error {
	...
	if strings.TrimSpace(os.Getenv(cli.TaskConfigRootEnv)) == "" {
		...
		if markerPath := leftoverDaemonTaskMarkerPath(); markerPath != "" {
			return fmt.Errorf("daemon-managed task requires a task-local Enact config root in %s%s", cli.TaskConfigRootEnv, leftoverMarkerSuffix(markerPath))
		}
		return fmt.Errorf("daemon-managed task requires a task-local Enact config root in %s%s", cli.TaskConfigRootEnv, daemonPortOnlyContextHint())
	}
	return nil
}
...
func resolveWorkspaceID(cmd *cobra.Command) string {
	val := cli.FlagOrEnv(cmd, "workspace-id", "ENACT_WORKSPACE_ID", "")
	if val != "" {
		return val
	}
...
func requireWorkspaceID(cmd *cobra.Command) (string, error) {
	...
	if id == "" {
		if inDaemonManagedExecutionContext() {
			return "", fmt.Errorf("workspace_id is required: ENACT_WORKSPACE_ID must be set by the daemon in agent execution context (no fallback to user config)")
		}
		return "", fmt.Errorf("workspace_id is required: use --workspace-id flag, set ENACT_WORKSPACE_ID env, or run 'enact config set workspace_id <id>'")
	}
	return id, nil
}
...
func runAgentUpdate(cmd *cobra.Command, args []string) error {
	...
	if len(body) == 0 {
		return fmt.Errorf("no fields to update; use --name, --description, --instructions, --runtime-id, --runtime-config, --model, --thinking-level, --service-tier, --custom-args, --mcp-config, --visibility, --status, or --max-concurrent-tasks (env vars now live behind `enact agent env set <id>`)")
	}
