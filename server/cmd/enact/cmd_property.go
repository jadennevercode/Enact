...
// enact property {list|get|create|update|archive|unarchive} — workspace
// custom property definitions, and enact issue property {list|set|unset} —
...
flags:
  enact property create --name Severity --type select \
...
The actor types hold workspace members; they take no options:
  enact property create --name Reviewer --type actor`,
	Args: exactArgs(0),
