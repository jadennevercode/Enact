...
var authCmd = &cobra.Command{
	Use:   "auth",
	Short: "Authenticate enact with Enact",
}
...
func resolveToken(cmd *cobra.Command) string {
	if v := strings.TrimSpace(os.Getenv("ENACT_TOKEN")); v != "" {
		return v
	}
	...
	// inDaemonManagedExecutionContext already covers the ENACT_DAEMON_PORT
	// signal for subprocesses that lost ENACT_AGENT_ID / ENACT_TASK_ID.
...
func resolveAppURL(cmd *cobra.Command) string {
	...
	fmt.Fprintln(os.Stderr, "No app URL configured. Run 'enact setup' first.")
	os.Exit(1)
	return "" // unreachable
}
...
func runAuthLoginBrowser(cmd *cobra.Command) error {
	...
	if err != nil {
		return cli.WithUserMessage("Sign-in did not complete: the server could not issue an access token for the CLI. Run `enact login` again.", err)
	}
	...
	if err := patClient.GetJSON(ctx, "/api/me", &me); err != nil {
		return cli.WithUserMessage("Sign-in did not complete: the server did not accept the new credential. Run `enact login` again.", err)
	}
...
func runAuthLoginToken(cmd *cobra.Command, providedToken string) error {
	...
	if err := client.GetJSON(ctx, "/api/me", &me); err != nil {
		return cli.WithUserMessage("Could not sign in with that token — make sure it is valid and not expired, then run `enact login --token <token>` again.", err)
	}
...
func runAuthStatus(cmd *cobra.Command, _ []string) error {
	...
	if taskContext && !strings.HasPrefix(token, "mat_") {
		return fmt.Errorf("agent execution context requires ENACT_TOKEN to be a task-scoped mat_ token")
	}
	serverURL := resolveServerURL(cmd)

	if token == "" {
		fmt.Fprintln(os.Stderr, "Not authenticated. Run 'enact login' to authenticate.")
		return nil
	}
...
<title>Enact — Authenticated</title>
<style>
