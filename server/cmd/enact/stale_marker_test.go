...
func TestLeftoverMarkerNotReportedUnderRealTaskIdentity(t *testing.T) {
	for _, tc := range []struct{ name, env, value string }{
		{name: "agent id", env: "ENACT_AGENT_ID", value: "agent-1"},
		{name: "task id", env: "ENACT_TASK_ID", value: "task-1"},
		{name: "task config root", env: cli.TaskConfigRootEnv, value: "/tmp/task-enact"},
		{name: "daemon port", env: "ENACT_DAEMON_PORT", value: "20032"},
	} {
		t.Run(tc.name, func(t *testing.T) {
			...
			// ENACT_DAEMON_PORT alone is not task identity, so it does not by
