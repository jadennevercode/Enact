...
// scratch paths like /tmp are not. ENA-4252 traced a cross-environment context
...
var issueReorderCmd = &cobra.Command{
	...
	Long: "Reposition an issue inside its current status column by computing a new\n" +
		...
		"different column, change its status first with `enact issue status`.",
	Args: exactArgs(1),
...
// workspace. Since ENA-6243 a workspace may define additional custom statuses,
...
//
// Since ENA-6243 a workspace can define custom statuses, so the CLI cannot know
...
func init() {
	...
	issueCreateCmd.Flags().Bool("allow-external-file", false, "Allow --description-file / --attachment to read a path outside the current working directory. Off by default so a stale file from another run/environment can't be picked up (ENA-4252).")
	...
	issueUpdateCmd.Flags().Bool("allow-external-file", false, "Allow --description-file to read a path outside the current working directory. Off by default so a stale temp file from another run/environment can't be picked up (ENA-4252).")
	...
	issueCommentListCmd.Flags().String("before", "", "Cursor (RFC3339Nano timestamp). With --recent: thread cursor (last_activity_at). With --thread + --tail: reply cursor (reply created_at). Read from the X-Enact-Next-Before response header, printed on stderr as \"Next thread cursor\" / \"Next reply cursor\"; must be paired with --before-id.")
	issueCommentListCmd.Flags().String("before-id", "", "Cursor UUID. With --recent: thread root UUID. With --thread + --tail: oldest reply UUID. Read from the X-Enact-Next-Before-Id response header; must be paired with --before.")
...
// built-in statuses correctly. (ENA-6243)
...
// --description-file / --content-file (ENA-4252) to a local --attachment path.
...
// only accepts local paths). Each remaining path is run through the ENA-4252
...
func quickCreateAttachmentIDsFromEnv() ([]string, error) {
	raw := strings.TrimSpace(os.Getenv("ENACT_QUICK_CREATE_ATTACHMENT_IDS"))
	if raw == "" {
		return nil, nil
	}
	var ids []string
	if err := json.Unmarshal([]byte(raw), &ids); err != nil {
		return nil, fmt.Errorf("parse ENACT_QUICK_CREATE_ATTACHMENT_IDS: %w", err)
	}
...
func runIssueCreate(cmd *cobra.Command, _ []string) error {
	...
	if hasDesc {
		if err := guardLocalPathLinks(desc, "issue description",
			"Deliver the file itself with `enact issue create --attachment <path>` (repeatable) and drop the link."); err != nil {
			return err
		}
	...
	// Quick-create stamp: when the daemon sets ENACT_QUICK_CREATE_TASK_ID
	// before invoking the agent, the agent's `enact issue create` call
	...
	if taskID := os.Getenv("ENACT_QUICK_CREATE_TASK_ID"); taskID != "" {
...
func runIssueUpdate(cmd *cobra.Command, args []string) error {
	...
	if cmd.Flags().Changed("description") || cmd.Flags().Changed("description-stdin") || cmd.Flags().Changed("description-file") {
		...
		if err := guardLocalPathLinks(desc, "issue description",
			"`enact issue update` cannot carry files — deliver the file with `enact issue comment add <issue-id> --attachment <path>` instead, and drop the link."); err != nil {
			return err
		}
		body["description"] = desc
	}
...
func reorderTargetNotInColumnError(ctx context.Context, client *cli.APIClient, otherRef, issueRef resolvedID, status string) error {
	if other, err := fetchIssue(ctx, client, otherRef.ID); err == nil {
		if otherStatus := strVal(other, "status"); otherStatus != "" && otherStatus != status {
			return fmt.Errorf("issue %s is in the %q column but %s is in %q; move one with `enact issue status` first, or pick a target in the same column", otherRef.Display, otherStatus, issueRef.Display, status)
		}
	}
...
func runIssueCommentList(cmd *cobra.Command, args []string) error {
	...
	if nb := respHeaders.Get("X-Enact-Next-Before"); nb != "" {
		if nbid := respHeaders.Get("X-Enact-Next-Before-Id"); nbid != "" {
...
func runIssueCommentAdd(cmd *cobra.Command, args []string) error {
	...
	if err := guardLocalPathLinks(content, "comment body",
		"Deliver the file itself with `enact issue comment add <issue-id> --attachment <path>` (repeatable) and drop the link."); err != nil {
		return err
	}
	...
	// the comment is never posted (which would duplicate on retry — ENA-4252).
	pending, err := collectLocalAttachments(cmd, attachments)
	if err != nil {
		return err
	}
...
var (
	...
	// Actor property values are members only (ENA-6286).
...
func resolveAssignee(ctx context.Context, client *cli.APIClient, name string, kinds assigneeKinds) (string, string, error) {
	if client.WorkspaceID == "" {
		return "", "", fmt.Errorf("workspace ID is required to resolve assignees; use --workspace-id or set ENACT_WORKSPACE_ID")
	}
...
// from `enact workspace member list --output json`) should use this instead of
