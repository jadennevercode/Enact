...
func TestResolveAppURL(t *testing.T) {
	...
	t.Run("prefers ENACT_APP_URL", func(t *testing.T) {
		t.Setenv("ENACT_APP_URL", "http://localhost:14000")
...
func TestBrowserLoginInstructionsSSHRemoteHint(t *testing.T) {
	const loginURL = "https://enact.ai/login?cli_callback=http%3A%2F%2Flocalhost%3A43689%2Fcallback"
...
func TestLoginTokenFlagWiring(t *testing.T) {
	...
	if tokenFlag.NoOptDefVal != tokenPromptSentinel {
		t.Fatalf("loginCmd --token NoOptDefVal = %q, want %q (legacy `enact login --token` prompt mode would break)", tokenFlag.NoOptDefVal, tokenPromptSentinel)
	}
}

// TestLoginTokenHelpOutputRendersCleanly renders loginCmd's flag help through
// the same pflag path `enact login -h` uses (FlagUsagesWrapped) and locks the
...
// right downstream branch is taken: `--token enact_xxx` and `--token=enact_xxx`
...
func TestLoginTokenFlagParsing(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			argv: []string{"--token", "enact_xxx"},
			want: want{changed: true, resolvedToken: "enact_xxx"},
		},
		{
			name: "equals-separated value",
			argv: []string{"--token=enact_yyy"},
			want: want{changed: true, resolvedToken: "enact_yyy"},
		},
		{
...
func TestRunAuthStatusTaskContextDoesNotPrintCredential(t *testing.T) {
	...
	t.Setenv("ENACT_TOKEN", fakeTaskToken)
...
func TestRunAuthStatusTaskContextRequiresTaskToken(t *testing.T) {
	...
	ownerPath := filepath.Join(ownerHome, ".enact", "config.json")
	...
	for _, tc := range []struct {
		...
		{name: "human token", token: "enact_owner_sentinel"},
	} {
		t.Run(tc.name, func(t *testing.T) {
			t.Setenv("ENACT_TOKEN", tc.token)
			requestCount = 0
			stderr := captureStderr(t)
...
func TestHumanAuthCommandsFailClosedInTaskContext(t *testing.T) {
	...
	t.Setenv("ENACT_AGENT_ID", "agent-test")
	t.Setenv("ENACT_TASK_ID", "task-test")
	t.Setenv("ENACT_TOKEN", "mat_task_sentinel")
	t.Setenv("ENACT_SERVER_URL", "https://task.invalid")
	t.Setenv("ENACT_TASK_CONFIG_ROOT", filepath.Join(t.TempDir(), "task-enact"))
	...
	loginCmd := testCmd()
	loginCmd.Flags().String("token", "enact_fake_login", "")
	_ = loginCmd.Flags().Set("token", "enact_fake_login")
...
// `enact login --token`. The original implementation hardcoded `enact_`
// only, which rejected legitimate Enact Cloud Node PATs (`mcn_`) at
...
func TestValidateLoginTokenPrefix(t *testing.T) {
	cases := []struct {
		...
		{name: "enact_ PAT", token: "enact_abc123", wantErr: false},
		...
		{name: "case-sensitive: ENA_ rejected", token: "ENA_abc123", wantErr: true},
		{name: "leading whitespace not allowed (callers TrimSpace first)", token: " enact_abc", wantErr: true},
	}
	for _, tc := range cases {
		tc := tc
		t.Run(tc.name, func(t *testing.T) {
	...
	for _, p := range []string{"enact_", "mcn_"} {
