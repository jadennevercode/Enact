...
func TestRunWorkspaceCreatePostsWorkspaceAndDoesNotSwitchDefault(t *testing.T) {
	...
	t.Setenv("ENACT_WORKSPACE_ID", "existing-workspace")
...
func TestRunWorkspaceSwitch(t *testing.T) {
	...
	// Isolate HOME so the test never touches the developer's ~/.enact.
	t.Setenv("HOME", t.TempDir())
	t.Setenv("ENACT_SERVER_URL", srv.URL)
	t.Setenv("ENACT_TOKEN", "test-token")
	...
	t.Run("isolates by profile", func(t *testing.T) {
		...
		wantSuffix := filepath.Join(".enact", "profiles", "staging", "config.json")
...
func TestRunWorkspaceSwitchFailsClosedInTaskContext(t *testing.T) {
	...
	t.Setenv("ENACT_WORKSPACE_ID", "task-workspace")
...
func TestFetchWorkspacesExplainsPortOnlyFailClosedContext(t *testing.T) {
	...
	if err == nil || !strings.Contains(err.Error(), "ENACT_DAEMON_PORT") || !strings.Contains(err.Error(), "remove") {
