...
//
// Any ENACT_* task identity in the environment means the daemon really did
// launch this process, so the marker is doing its job. ENACT_DAEMON_PORT
...
func leftoverDaemonTaskMarkerPath() string {
	if inAgentExecutionContext() ||
		...
		strings.TrimSpace(os.Getenv("ENACT_DAEMON_PORT")) != "" {
		return ""
	}
