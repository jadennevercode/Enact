...
func TestHumanLocalCommandDistinguishesPortHintFromTaskIdentity(t *testing.T) {
	...
	t.Setenv(cli.TaskConfigRootEnv, filepath.Join(t.TempDir(), "task-enact"))
...
func TestHumanLocalCommandRejectsExplicitTaskIdentity(t *testing.T) {
	for _, tc := range []struct {
		...
		{name: "agent ID blocks setup", envName: "ENACT_AGENT_ID", command: "setup"},
		{name: "task ID blocks daemon stop", envName: "ENACT_TASK_ID", command: "daemon stop"},
	} {
...
func TestMissingServerConfigMessageExplainsPortOnlyContext(t *testing.T) {
	...
	if !strings.Contains(message, "ENACT_DAEMON_PORT") || !strings.Contains(message, "remove") {
...
// impersonation escape: a sandbox fault strips every ENACT_* env var from
// an agent subprocess, which then runs `enact` from the *parent* directory
...
func TestNewAPIClient_WorkdirParentEscapeFailsClosed(t *testing.T) {
	...
	// Daemon-owned tree: {root}/.enact marker + {root}/{ws}/{task}/workdir
	...
	t.Setenv("ENACT_SERVER_URL", "http://127.0.0.1:8080")
...
// agent task (ENACT_AGENT_ID / ENACT_TASK_ID set), the CLI must NOT
// silently read the user-global ~/.enact/config.json to recover a missing
...
func TestResolveWorkspaceID_AgentContextSkipsConfig(t *testing.T) {
	...
	t.Run("agent context with explicit env uses env", func(t *testing.T) {
		t.Setenv("ENACT_AGENT_ID", "agent-123")
		t.Setenv("ENACT_TASK_ID", "task-456")
		t.Setenv("ENACT_DAEMON_PORT", "")
		t.Setenv("ENACT_WORKSPACE_ID", "env-ws")
...
func TestResolveToken_AgentContextSkipsConfig(t *testing.T) {
	...
	t.Run("agent context without env never reads config", func(t *testing.T) {
		...
		if got := resolveToken(testCmd()); got != "" {
			t.Fatalf("resolveToken() = %q, want empty in agent context without ENACT_TOKEN", got)
		}
	})
	...
	t.Run("daemon port marker without env never reads config", func(t *testing.T) {
		...
		if got := resolveToken(testCmd()); got != "" {
			t.Fatalf("resolveToken() = %q, want empty in daemon-managed context without ENACT_TOKEN", got)
		}
	})
	...
	t.Run("agent context uses explicit task token env", func(t *testing.T) {
		...
		t.Setenv("ENACT_TASK_ID", "task-456")
		t.Setenv("ENACT_DAEMON_PORT", "")
		t.Setenv("ENACT_TOKEN", "mat_task_token")

		if got := resolveToken(testCmd()); got != "mat_task_token" {
			t.Fatalf("resolveToken() = %q, want ENACT_TOKEN", got)
		}
	})
	...
	t.Run("daemon port set without agent context avoids config fallback", func(t *testing.T) {
		...
		t.Setenv("ENACT_TASK_ID", "")
		t.Setenv("ENACT_TOKEN", "")
		t.Setenv("ENACT_DAEMON_PORT", "19514")
	...
	t.Run("daemon port set with explicit task token uses task token", func(t *testing.T) {
		...
		if got := resolveToken(testCmd()); got != "mat_task_token" {
			t.Fatalf("resolveToken() = %q, want ENACT_TOKEN (task token wins over daemon signal)", got)
		}
	})

	// ENACT_SERVER_URL is a user-facing env var that may be set in a
	// normal shell. It is NOT a daemon identity signal — only
	// ENACT_DAEMON_PORT is. The config fallback must still work when
	// SERVER_URL is set but no daemon signal is present.
	t.Run("ENACT_SERVER_URL alone does not block config fallback", func(t *testing.T) {
		t.Setenv("ENACT_AGENT_ID", "")
		t.Setenv("ENACT_TASK_ID", "")
		t.Setenv("ENACT_TOKEN", "")
		t.Setenv("ENACT_DAEMON_PORT", "")
...
func TestNewAPIClient_AgentContextRequiresTaskToken(t *testing.T) {
	t.Setenv("ENACT_SERVER_URL", "http://127.0.0.1:8080")
	t.Setenv("ENACT_WORKSPACE_ID", "workspace-123")
	t.Setenv("ENACT_AGENT_ID", "agent-123")
	t.Setenv("ENACT_TASK_ID", "task-456")
...
func TestNewAPIClient_DaemonPortRequiresTaskToken(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "ENACT_DAEMON_PORT") || !strings.Contains(err.Error(), "remove") {
...
func TestAgentUpdateNoFieldsErrorPointsAtEnvCommand(t *testing.T) {
	t.Setenv("HOME", t.TempDir())
	t.Setenv("ENACT_SERVER_URL", "http://127.0.0.1:0")
	t.Setenv("ENACT_WORKSPACE_ID", "test-ws")
	...
	msg := err.Error()
	if !strings.Contains(msg, "enact agent env set") {
		t.Fatalf("no-fields error must direct users to `enact agent env set`; got: %q", msg)
	}
}
