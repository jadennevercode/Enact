...
func TestFindLocalPathLinksAllowsLegitimateContent(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			body: "see [ENA-1](/acme/issues/ENA-1) and [inbox](/issues/123)",
		},
		{
			name: "external http link",
			body: "the PR is at [#42](https://github.com/enact-ai/enact/pull/42)",
		},
		{
			name: "mention link",
...
func TestGuardLocalPathLinksOnlyFiresInAgentContext(t *testing.T) {
	...
	t.Run("human PAT context is never linted", func(t *testing.T) {
		// No ENACT_AGENT_ID / ENACT_TASK_ID: a person running the CLI. Their
	...
	t.Run("agent context with a clean body passes", func(t *testing.T) {
		withAgentContext(t)
		if err := guardLocalPathLinks("all good — see [ENA-1](/acme/issues/ENA-1)", "comment body", "hint"); err != nil {
...
func TestGuardLocalPathLinksHintIsPerCommand(t *testing.T) {
	...
	err := guardLocalPathLinks(
		"[screenshot]("+shot+")",
		"issue description",
		"`enact issue update` cannot carry files — deliver the file with `enact issue comment add <issue-id> --attachment <path>` instead, and drop the link.",
	)
	if err == nil {
		t.Fatal("expected a hard failure")
	}
	msg := err.Error()
	if !strings.Contains(msg, "`enact issue update` cannot carry files") {
		t.Errorf("update hint missing, got: %v", msg)
	}
	if !strings.Contains(msg, "enact issue comment add <issue-id> --attachment <path>") {
		t.Errorf("update hint must redirect to comment add, got: %v", msg)
	}
	if strings.Contains(msg, "enact issue update --attachment") {
