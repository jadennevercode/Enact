...
const blockingChildEnv = "ENACT_TEST_BLOCKING_CHILD"
...
// carries no platform build tag, so `go test ./cmd/enact` has to pass there
...
func TestDaemonStatusInTaskContextReportsNamedProfileHost(t *testing.T) {
	setup := func(t *testing.T) {
		...
		t.Setenv("ENACT_DAEMON_PORT", strconv.Itoa(stub.port))
	}
