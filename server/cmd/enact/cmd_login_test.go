...
func TestRunLoginTokenAutoWatchesDiscoveredWorkspaces(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.Header.Get("Authorization") != "Bearer enact_test_token" {
	...
	cmd := newLoginTestCmd()
	if err := cmd.Flags().Set("token", "enact_test_token"); err != nil {
	...
	if cfg.Token != "enact_test_token" || cfg.ServerURL != srv.URL || cfg.WorkspaceID != "ws-1" {
