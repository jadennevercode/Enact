...
func TestRunIssueCommentAddRejectsExternalAttachmentWithZeroUploads(t *testing.T) {
	...
	t.Setenv("ENACT_TOKEN", "mat_test-token")
...
func TestRunIssueCreateSendsExistingAttachmentIDs(t *testing.T) {
	...
	t.Setenv("ENACT_WORKSPACE_ID", "ws-1")
	t.Setenv("ENACT_TOKEN", "test-token")
	t.Setenv("ENACT_QUICK_CREATE_ATTACHMENT_IDS", `["att-env","att-shared"]`)
...
// enact-ai/enact#1620: when one name is a substring of another (e.g.
...
// enact-ai/enact#1620: passing a full UUID or its 8-char prefix must
...
func TestRunIssueCommentList_ThreadTailPassesThroughAndPrintsReplyCursor(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if r.Method == http.MethodGet && strings.HasSuffix(r.URL.Path, "/comments") {
			...
			w.Header().Set("X-Enact-Next-Before", "2026-01-01T00:00:00.000000001Z")
			w.Header().Set("X-Enact-Next-Before-Id", "00000000-0000-0000-0000-000000000999")
			w.Write([]byte("[]"))
			return
		}
		t.Errorf("unexpected request: %s %s", r.Method, r.URL.String())
...
func TestRunIssueCommentList_RecentStillLabelsCursorAsThread(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("X-Enact-Next-Before", "2026-01-01T00:00:00.000000001Z")
		w.Header().Set("X-Enact-Next-Before-Id", "00000000-0000-0000-0000-000000000777")
		w.Write([]byte("[]"))
	}))
	defer srv.Close()
...
func TestRunIssueUpdateNoStartSendsSuppressRun(t *testing.T) {
	...
	defer srv.Close()
	setCLITestServerEnv(t, srv.URL)
	t.Setenv("ENACT_TASK_CONFIG_ROOT", t.TempDir())
	t.Setenv("ENACT_TOKEN", "mat_test-token")
...
func TestIssueReadCommandsUseInjectedTaskToken(t *testing.T) {
	...
	t.Setenv("ENACT_WORKSPACE_ID", "ws-task")
	t.Setenv("ENACT_TASK_CONFIG_ROOT", t.TempDir())
...
func TestIssueReadCommandsFailClosedWithoutTaskToken(t *testing.T) {
	...
	t.Setenv("ENACT_SERVER_URL", "http://127.0.0.1:1")
	t.Setenv("ENACT_WORKSPACE_ID", "ws-task")
	t.Setenv("ENACT_TASK_CONFIG_ROOT", t.TempDir())
