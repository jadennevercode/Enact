...
import (
	...
	logger_pkg "github.com/enact-ai/enact/server/internal/logger"
...
var daemonDiskUsageCmd = &cobra.Command{
	...
	Long: "Walks the daemon's workspaces root and reports per-task or per-workspace disk usage.\n" +
		...
		"root — the default root plus each ~/.enact/profiles/* root, including the Desktop app's dedicated\n" +
		...
		"overridable via ENACT_GC_ARTIFACT_PATTERNS) so the report stays in sync with what the GC reclaims.\n" +
...
func init() {
	...
	f.String("daemon-id", "", "Unique daemon identifier (env: ENACT_DAEMON_ID)")
	f.String("device-name", "", "Human-readable device name (env: ENACT_DAEMON_DEVICE_NAME)")
	f.String("runtime-name", "", "Runtime display name (env: ENACT_AGENT_RUNTIME_NAME)")
	f.String("workspaces-root", "", "Base directory for task workspaces (env: ENACT_WORKSPACES_ROOT)")
	f.Duration("poll-interval", 0, "Task poll interval (env: ENACT_DAEMON_POLL_INTERVAL)")
	f.Duration("heartbeat-interval", 0, "Heartbeat interval (env: ENACT_DAEMON_HEARTBEAT_INTERVAL)")
	f.Duration("agent-timeout", 0, "Absolute per-task wall-clock cap; 0 = no cap, rely on the watchdogs (env: ENACT_AGENT_TIMEOUT)")
	f.Duration("codex-semantic-inactivity-timeout", 0, "Codex semantic inactivity timeout (env: ENACT_CODEX_SEMANTIC_INACTIVITY_TIMEOUT)")
	f.Duration("codex-handshake-timeout", 0, "Codex app-server startup RPC timeout (env: ENACT_CODEX_HANDSHAKE_TIMEOUT)")
	f.Int("max-concurrent-tasks", 0, "Max tasks running in parallel (env: ENACT_DAEMON_MAX_CONCURRENT_TASKS)")
	f.Bool("no-auto-update", false, "Disable periodic CLI self-update (env: ENACT_DAEMON_AUTO_UPDATE=false)")
	f.Duration("auto-update-interval", 0, "How often to poll GitHub for a newer release (env: ENACT_DAEMON_AUTO_UPDATE_INTERVAL)")
	f.Bool("no-auto-reload", false, "Disable restarting when the enact binary on disk changes version (env: ENACT_DAEMON_AUTO_RELOAD=false)")
	...
	rf.String("daemon-id", "", "Unique daemon identifier (env: ENACT_DAEMON_ID)")
	rf.String("device-name", "", "Human-readable device name (env: ENACT_DAEMON_DEVICE_NAME)")
	rf.String("runtime-name", "", "Runtime display name (env: ENACT_AGENT_RUNTIME_NAME)")
	rf.String("workspaces-root", "", "Base directory for task workspaces (env: ENACT_WORKSPACES_ROOT)")
	rf.Duration("poll-interval", 0, "Task poll interval (env: ENACT_DAEMON_POLL_INTERVAL)")
	rf.Duration("heartbeat-interval", 0, "Heartbeat interval (env: ENACT_DAEMON_HEARTBEAT_INTERVAL)")
	rf.Duration("agent-timeout", 0, "Absolute per-task wall-clock cap; 0 = no cap, rely on the watchdogs (env: ENACT_AGENT_TIMEOUT)")
	rf.Duration("codex-semantic-inactivity-timeout", 0, "Codex semantic inactivity timeout (env: ENACT_CODEX_SEMANTIC_INACTIVITY_TIMEOUT)")
	rf.Duration("codex-handshake-timeout", 0, "Codex app-server startup RPC timeout (env: ENACT_CODEX_HANDSHAKE_TIMEOUT)")
	rf.Int("max-concurrent-tasks", 0, "Max tasks running in parallel (env: ENACT_DAEMON_MAX_CONCURRENT_TASKS)")
	rf.Bool("no-auto-update", false, "Disable periodic CLI self-update (env: ENACT_DAEMON_AUTO_UPDATE=false)")
	rf.Duration("auto-update-interval", 0, "How often to poll GitHub for a newer release (env: ENACT_DAEMON_AUTO_UPDATE_INTERVAL)")
	rf.Bool("no-auto-reload", false, "Disable restarting when the enact binary on disk changes version (env: ENACT_DAEMON_AUTO_RELOAD=false)")
	...
	df.Bool("all-profiles", false, "Scan every workspace root (default root + all ~/.enact/profiles/* roots, incl. the Desktop app's) and report a combined total")
...
// Empty profile → ~/.enact/, named profile → ~/.enact/profiles/<name>/.
...
func newDaemonLogRotator(logPath string) *lumberjack.Logger {
	return &lumberjack.Logger{
		Filename:   logPath,
		MaxSize:    envPositiveIntOrDefault("ENACT_DAEMON_LOG_MAX_SIZE_MB", defaultDaemonLogMaxSizeMB),
		MaxBackups: envPositiveIntOrDefault("ENACT_DAEMON_LOG_MAX_BACKUPS", defaultDaemonLogMaxBackups),
		MaxAge:     envPositiveIntOrDefault("ENACT_DAEMON_LOG_MAX_AGE_DAYS", defaultDaemonLogMaxAgeDays),
		Compress:   true,
	}
}
...
// directory under ~/.enact/profiles. It carries the known profile names so
...
func (e *unknownProfileError) Error() string {
	if len(e.Known) == 0 {
		...
		return fmt.Sprintf("unknown profile %q: no named profiles exist yet.\nCreate one with: enact login --profile %s", e.Profile, shellQuoteArg(e.Profile))
	}
...
// may contain separators: `enact --profile team/dev config set ...` creates
// ~/.enact/profiles/team/dev. Matching against top-level entries would
...
// one-glance fix.
//
// The default profile is never validated: it owns ~/.enact directly, has no
...
// requireDaemonAuth fails fast when the user never ran `enact login`. The
...
// authenticates via the stored config token, never ENACT_TOKEN.
func requireDaemonAuth(profile string) error {
	...
	if cfg.Token == "" {
		...
		if profile != "" {
			loginHint = fmt.Sprintf("enact login --profile %s", profile)
		}
...
// ENACT_SERVER_URL env, then the stored CLI config for the profile.
func resolveDaemonServerURL(cmd *cobra.Command, profile string) string {
	serverURL := cli.FlagOrEnv(cmd, "server-url", "ENACT_SERVER_URL", "")
	if serverURL == "" {
...
func daemonStartupFailureError(logs daemonStartupLogs, waitErr error, profile, serverURL string) error {
	...
	case strings.Contains(joined, "connection refused") ||
		...
		target := "the Enact server"
		if serverURL != "" {
			target += " at " + serverURL
		}
		return fmt.Errorf("daemon failed to start: cannot reach %s.\nMake sure the server is running and reachable, then rerun 'enact daemon start'.\nFull log: %s", target, logs.logPath)
	}
...
func buildDaemonStartArgs(cmd *cobra.Command) []string {
	...
	// (= no cap), so it can override an environment ENACT_AGENT_TIMEOUT.
...
func runDaemonForeground(cmd *cobra.Command) error {
	...
	//
	//   --flag  >  ENACT_… env  >  config.json  >  built-in default
	//
	...
	pollOverride, err := resolveDaemonDurationOverride(pollFlag, "ENACT_DAEMON_POLL_INTERVAL", fileCfg.PollInterval)
	if err != nil {
		return err
	}
	if pollOverride > 0 {
	...
	heartbeatOverride, err := resolveDaemonDurationOverride(heartbeatFlag, "ENACT_DAEMON_HEARTBEAT_INTERVAL", fileCfg.HeartbeatInterval)
	if err != nil {
		return err
	}
	...
	agentTimeoutOverride, err := resolveDaemonAgentTimeoutOverride(cmd, "ENACT_AGENT_TIMEOUT", fileCfg.AgentTimeout)
	if err != nil {
		return err
	}
	...
	semanticOverride, err := resolveDaemonDurationOverride(semanticFlag, "ENACT_CODEX_SEMANTIC_INACTIVITY_TIMEOUT", fileCfg.CodexSemanticInactivityTimeout)
	if err != nil {
		return err
	}
	...
	handshakeOverride, err := resolveDaemonDurationOverride(handshakeFlag, "ENACT_CODEX_HANDSHAKE_TIMEOUT", fileCfg.CodexHandshakeTimeout)
	if err != nil {
		return err
	}
	...
	if n := resolveDaemonIntOverride(maxFlag, "ENACT_DAEMON_MAX_CONCURRENT_TASKS", fileCfg.MaxConcurrentTasks); n > 0 {
		overrides.MaxConcurrentTasks = n
	}
	// --no-auto-update / ENACT_DAEMON_AUTO_UPDATE=false / config.json
	...
	if resolveDaemonDisableSignal(noAutoUpdateFlag, "ENACT_DAEMON_AUTO_UPDATE", fileCfg.DisableAutoUpdate) {
	...
	if resolveDaemonDisableSignal(noAutoReloadFlag, "ENACT_DAEMON_AUTO_RELOAD", fileCfg.DisableAutoReload) {
		overrides.DisableAutoReload = true
	}
	autoUpdateFlag, _ := cmd.Flags().GetDuration("auto-update-interval")
	autoUpdateOverride, err := resolveDaemonDurationOverride(autoUpdateFlag, "ENACT_DAEMON_AUTO_UPDATE_INTERVAL", fileCfg.AutoUpdateCheckInterval)
	if err != nil {
		return err
	}
	...
	cfg.LaunchedBy = os.Getenv("ENACT_LAUNCHED_BY")
...
// `enact auth status`) and refuse to touch the running daemon on failure.
func requireDaemonRestartPreflight(cmd *cobra.Command, profile string) error {
	...
	if err := cli.NewAPIClient(baseURL, "", cfg.Token).GetJSON(ctx, "/api/me", nil); err != nil {
		...
		return fmt.Errorf("refusing to restart: cannot reach the Enact server at %s (%w); the running daemon was left untouched.\nMake sure the server is running and reachable, then rerun 'enact daemon restart'", baseURL, err)
	}
	return nil
}
...
// ENACT_DAEMON_PORT remains in the host environment. Inside a managed task it
...
func daemonStatusHealthPort(cmd *cobra.Command) (int, error) {
	...
	raw := strings.TrimSpace(os.Getenv("ENACT_DAEMON_PORT"))
	if raw == "" {
		return 0, fmt.Errorf("daemon status inside a daemon-managed task requires the daemon-injected ENACT_DAEMON_PORT")
	}
	port, err := strconv.Atoi(raw)
	if err != nil || port <= 0 {
		return 0, fmt.Errorf("invalid ENACT_DAEMON_PORT %q inside a daemon-managed task", raw)
	}
	return port, nil
}
...
func describeDaemonManager(launchedBy string) string {
	if launchedBy == "desktop" {
		return "Enact Desktop app (start and stop it from the app)"
	}
	return launchedBy
}
...
// ~/.enact/daemon.log for the default profile,
// ~/.enact/profiles/<name>/daemon.log for a named one — and guarantees it is
...
//  2. ENACT_AGENT_TIMEOUT is set -> return nil so LoadConfig reads the
...
//  2. ENACT_DAEMON_AUTO_UPDATE explicitly set to a falsy value ->
...
// boundary: --all-profiles enumerates ~/.enact/profiles and discloses the
...
// each ~/.enact/profiles/* root) and renders a per-root breakdown with a
...
// orientation even when empty), then each ~/.enact/profiles/* root that
// exists on disk, sorted by profile name. Roots that resolve to the same path
// (e.g. when ENACT_WORKSPACES_ROOT pins every profile to one directory) are
...
func printRepoCacheLine(w io.Writer, report daemon.DiskUsageReport) {
	...
	fmt.Fprintf(w, "Repo cache (.repos): %s across %d repo(s), not included above. Evicted once a repo is unused for ENACT_GC_REPO_TTL and no longer attached to a workspace.\n",
...
func printDiskUsageOtherRootsHint(w io.Writer, report daemon.DiskUsageReport, profile, rootOverride string, taskContext bool) {
	...
	// The suggestions are built by listing ~/.enact/profiles, so printing
	...
	fmt.Fprintln(w, "Run 'enact daemon disk-usage --all-profiles' for a combined total across all roots.")
}
...
func diskUsageProfileSuggestions(currentProfile, currentRoot string) []diskUsageProfileSuggestion {
		...
		if root, err := resolveWorkspacesRootForProfile("", ""); err == nil && !samePath(root, currentRoot) {
			if taskCount := countDiskUsageTaskDirs(root); taskCount > 0 {
				out = append(out, diskUsageProfileSuggestion{
					Profile:   "",
					Command:   "enact daemon disk-usage",
					Root:      root,
					TaskCount: taskCount,
				})
			}
	...
	for _, entry := range entries {
		...
		out = append(out, diskUsageProfileSuggestion{
			Profile:   profile,
			Command:   "enact --profile " + shellQuoteArg(profile) + " daemon disk-usage",
			Root:      root,
			TaskCount: taskCount,
		})
	}
...
func profilesRootDir() (string, error) {
	...
	return filepath.Join(home, ".enact", "profiles"), nil
}
