...
func TestRunConfigSetPersistsSupportedKeysInProfile(t *testing.T) {
	...
	workspacesRoot := filepath.Join(t.TempDir(), "enact-dev")
...
func TestRunConfigCommandsUseTaskLocalConfigWithoutTouchingOwner(t *testing.T) {
	...
	for _, forbidden := range []string{ownerHome, "https://owner.invalid", "owner-workspace-sentinel", "enact_owner_sentinel"} {
...
func TestApplyConfigSetSupportsDaemonKeys(t *testing.T) {
	...
	cfg := cli.CLIConfig{}
	workspacesRoot := filepath.Join(t.TempDir(), "enact")
...
func TestApplyConfigSetNormalizesWorkspacesRoot(t *testing.T) {
	...
	cfg := cli.CLIConfig{}
	if err := applyConfigSet(&cfg, "workspaces_root", filepath.Join("data", "enact")); err != nil {
		t.Fatalf("applyConfigSet: %v", err)
	}
	want := filepath.Join(cwd, "data", "enact")
...
func TestApplyConfigSetPositiveDurationRoundTripsToDaemonResolver(t *testing.T) {
	const envName = "TEST_ENACT_PERSISTED_DURATION"
	t.Setenv(envName, "")
