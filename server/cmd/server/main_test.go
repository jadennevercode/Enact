...
func TestRedisClientName(t *testing.T) {
	tests := []struct {
		...
		{"empty_suffix_returns_existing", "enact-api:store", "", "enact-api:store"},
		{"empty_existing_uses_default_prefix", "", "store", "enact-api:store"},
...
func TestNewNamedRedisClient_SetsClientName(t *testing.T) {
	...
	opts := client.Options()
	if opts.ClientName != "enact-api:store" {
		t.Errorf("ClientName = %q, want %q", opts.ClientName, "enact-api:store")
	}
}
...
func TestNewNamedRedisClient_DisableClientName_InvalidValue(t *testing.T) {
	...
	if opts.ClientName != "enact-api:store" {
		t.Errorf("ClientName = %q, want %q (invalid env should fall back to naming enabled)", opts.ClientName, "enact-api:store")
	}
}
...
func TestJWTSecretBootError(t *testing.T) {
	...
	tests := []struct {
		...
		{"production_with_code_default_is_rejected", "enact-dev-secret-change-in-production", "production", true},
