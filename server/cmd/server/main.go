...
import (
	...
	"github.com/enact-ai/enact/server/internal/profiling"
	"github.com/enact-ai/enact/server/internal/realtime"
	"github.com/enact-ai/enact/server/internal/scheduler"
...
// maxLLMRetriesLimit caps ENACT_LLM_MAX_RETRIES. The ceiling is a latency
...
// parseLLMMaxRetries turns the raw ENACT_LLM_MAX_RETRIES value into the
...
func main() {
	...
	shutdownHoldDuration := envNonNegativeDuration("ENACT_SHUTDOWN_HOLD_DURATION", 0)

	// Feature flags: loaded once at startup from ENACT_FEATURE_FLAGS_FILE
	...
	llmMaxRetries, err := parseLLMMaxRetries(os.Getenv("ENACT_LLM_MAX_RETRIES"))
	if err != nil {
		slog.Error("invalid ENACT_LLM_MAX_RETRIES", "error", err)
		os.Exit(1)
	}
	...
	runtimeReconnectGrace := envDuration("ENACT_RUNTIME_RECONNECT_GRACE", defaultRuntimeReconnectGrace)
