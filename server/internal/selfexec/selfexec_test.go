...
func TestResolveWithFallsBackToArgv0(t *testing.T) {
	...
	t.Run("absolute path", func(t *testing.T) {
		want := writeTestExecutable(t, t.TempDir(), "enact")
	...
	t.Run("relative path", func(t *testing.T) {
		dir := t.TempDir()
		want := writeTestExecutable(t, dir, "enact")
		t.Chdir(dir)
	...
	t.Run("PATH command", func(t *testing.T) {
		...
		got, err := resolveWith(failExecutable, []string{"enact"})
		if err != nil {
			t.Fatalf("resolveWith() error = %v", err)
		}
		if got != want {
...
func TestResolveWithRejectsInvalidFallback(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "missing executable", args: []string{"enact-does-not-exist"}, want: "enact-does-not-exist"},
	}
	...
	if runtime.GOOS != "windows" {
		t.Run("non-executable file", func(t *testing.T) {
			path := filepath.Join(t.TempDir(), "enact")
