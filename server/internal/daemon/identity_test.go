...
func TestEnsureDaemonID_Persists(t *testing.T) {
	...
	path := filepath.Join(home, ".enact", "daemon.id")
...
func TestEnsureDaemonID_SharedAcrossProfiles(t *testing.T) {
	...
	// only source of truth is ~/.enact/daemon.id.
	profileFile := filepath.Join(home, ".enact", "profiles", "staging", "daemon.id")
...
func TestEnsureDaemonID_PromotesPreChangeProfileFile(t *testing.T) {
	...
	profileDir := filepath.Join(home, ".enact", "profiles", "staging")
	...
	data, err := os.ReadFile(filepath.Join(home, ".enact", "daemon.id"))
	if err != nil {
...
func TestLegacyDaemonUUIDs_ScansProfileDirs(t *testing.T) {
	...
	for name, id := range map[string]string{"prod": uuidA, "desktop-enact": uuidB} {
		dir := filepath.Join(home, ".enact", "profiles", name)
		if err := os.MkdirAll(dir, 0o755); err != nil {
	...
	corruptDir := filepath.Join(home, ".enact", "profiles", "corrupt")
