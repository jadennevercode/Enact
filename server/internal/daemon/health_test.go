...
// a enact version change but the daemon was busy at the barrier check, the
...
func TestHealthHandlerReportsDeferredReload(t *testing.T) {
	...
	t.Run("explains a deferred restart", func(t *testing.T) {
		...
		d.setReloadPending("enact binary on disk reports 0.3.8, running 0.3.7")
...
func TestHealthHandlerReportsProfileIdentity(t *testing.T) {
	...
	t.Run("named profile reports its name", func(t *testing.T) {
		t.Parallel()
		raw := rawHealth(t, Config{Profile: "desktop-api.enact.ai", LaunchedBy: "desktop"})
		if got, want := raw["profile"], "desktop-api.enact.ai"; got != want {
