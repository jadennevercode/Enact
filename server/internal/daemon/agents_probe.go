...
// /health.agents reports and what `enact daemon probe-runtimes` prints.
//
...
// the daemon is already running gets picked up without a restart (ENA-5439).
// Everything it reads is process-external (PATH, ENACT_*_PATH, ENACT_*_MODEL),
...
var probeAgentCLIs = func() map[string]AgentEntry {
	...
	// pinned to an explicit ENACT_*_PATH) must not pay the cost of
	...
	// ~/.local/bin via ~/.zshrc) be discovered without a restart (ENA-5439).
	...
	probe := func(envVar, defaultCmd, modelEnv string) (AgentEntry, bool) {
		...
		// who pinned ENACT_*_PATH to an absolute or relative path that
	...
	if e, ok := probe("ENACT_CLAUDE_PATH", "claude", "ENACT_CLAUDE_MODEL"); ok {
		agents["claude"] = e
	}
	if e, ok := probe("ENACT_CODEX_PATH", "codex", "ENACT_CODEX_MODEL"); ok {
		agents["codex"] = e
	}
	if e, ok := probe("ENACT_OPENCODE_PATH", "opencode", "ENACT_OPENCODE_MODEL"); ok {
		agents["opencode"] = e
	}
	if e, ok := probe("ENACT_DEVECO_PATH", "deveco", "ENACT_DEVECO_MODEL"); ok {
		agents["deveco"] = e
	}
	if e, ok := probe("ENACT_OPENCLAW_PATH", "openclaw", "ENACT_OPENCLAW_MODEL"); ok {
		agents["openclaw"] = e
	}
	if e, ok := probe("ENACT_HERMES_PATH", "hermes", "ENACT_HERMES_MODEL"); ok {
		agents["hermes"] = e
	}
	if e, ok := probe("ENACT_PI_PATH", "pi", "ENACT_PI_MODEL"); ok {
		agents["pi"] = e
	}
	...
	if e, ok := probe("ENACT_CURSOR_PATH", "cursor-agent", "ENACT_CURSOR_MODEL"); ok {
		agents["cursor"] = e
	}
	if e, ok := probe("ENACT_COPILOT_PATH", "copilot", "ENACT_COPILOT_MODEL"); ok {
		agents["copilot"] = e
	}
	if e, ok := probe("ENACT_KIMI_PATH", "kimi", "ENACT_KIMI_MODEL"); ok {
		agents["kimi"] = e
	}
	if e, ok := probe("ENACT_REASONIX_PATH", "reasonix", "ENACT_REASONIX_MODEL"); ok {
		agents["reasonix"] = e
	}
	// DSH is registered only when its Enact runtime profile is installed.
	...
	if e, ok := probe("ENACT_DSH_PATH", "dsh", "ENACT_DSH_MODEL"); ok && probeDshEnactProfile(e.Path) {
		agents["dsh"] = e
	}
	if e, ok := probe("ENACT_KIRO_PATH", "kiro-cli", "ENACT_KIRO_MODEL"); ok {
		agents["kiro"] = e
	}
	if e, ok := probe("ENACT_CODEBUDDY_PATH", "codebuddy", "ENACT_CODEBUDDY_MODEL"); ok {
		agents["codebuddy"] = e
	}
	// agy 1.0.6 added a `--model` flag (ENA-3125), so Antigravity now takes a
	// model env like every other backend. ENACT_ANTIGRAVITY_MODEL seeds the
	...
	if e, ok := probe("ENACT_ANTIGRAVITY_PATH", "agy", "ENACT_ANTIGRAVITY_MODEL"); ok {
	...
	// qodercli install stayed invisible across restarts (ENA-5524).
	if e, ok := probe("ENACT_QODER_PATH", "qodercli", "ENACT_QODER_MODEL"); ok {
		agents["qoder"] = e
	}
	...
	if e, ok := probe("ENACT_QODERCLICN_PATH", "qoderclicn", "ENACT_QODERCLICN_MODEL"); ok {
		agents["qoderclicn"] = e
	}
	// ByteDance official TRAE CLI (the `traecli` binary from https://docs.trae.cn/cli),
	// driven over ACP via `traecli acp serve --yolo`. ENACT_TRAECLI_MODEL seeds
	// the daemon-wide default model (a model id from the user's logged-in traecli
	// catalog).
	if e, ok := probe("ENACT_TRAECLI_PATH", "traecli", "ENACT_TRAECLI_MODEL"); ok {
		agents["traecli"] = e
	}
	// xAI Grok Build CLI (`grok`), driven over ACP via
	// `grok agent --always-approve stdio`. ENACT_GROK_MODEL seeds the
	// daemon-wide default (e.g. grok-4.5).
	if e, ok := probe("ENACT_GROK_PATH", "grok", "ENACT_GROK_MODEL"); ok {
		agents["grok"] = e
	}
	...
	if e, ok := probe("ENACT_QWEN_PATH", "qwen", "ENACT_QWEN_MODEL"); ok {
		agents["qwen"] = e
	}
	...
	if e, ok := probe("ENACT_QWENPAW_PATH", "qwenpaw", ""); ok {
		agents["qwenpaw"] = e
	}
	// Dim (`dim`) is the DimCode CLI agent, driven over ACP via `dim acp`.
	// ENACT_DIM_MODEL seeds the daemon-wide default (a model id from the
	// user's logged-in dim catalog).
	if e, ok := probe("ENACT_DIM_PATH", "dim", "ENACT_DIM_MODEL"); ok {
		agents["dim"] = e
	}
	...
	if e, ok := probe("ENACT_MCODE_PATH", "mcode", ""); ok {
		agents["mcode"] = e
	}
	return agents
}

func probeDshEnactProfile(executablePath string) bool {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	cmd := exec.CommandContext(ctx, executablePath, "--profile", "enact", "--probe")
