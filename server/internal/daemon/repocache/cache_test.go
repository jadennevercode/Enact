...
func TestCreateWorktreeContextCancelsRunningGitProcessTree(t *testing.T) {
	...
	body := "#!/bin/sh\n(sleep 0.5; echo leaked > \"$ENACT_TEST_GIT_MARKER\") &\nwait\n"
	if err := os.WriteFile(script, []byte(body), 0o755); err != nil {
		t.Fatal(err)
	}
	t.Setenv("PATH", binDir+string(os.PathListSeparator)+os.Getenv("PATH"))
	t.Setenv("ENACT_TEST_GIT_MARKER", marker)
...
// invocation Windows Codex tasks now use (enact-ai/enact#6449) and proves
...
// for the Enact Agent to every commit made in the worktree.
func TestCreateWorktreeInstallsCoAuthoredByHook(t *testing.T) {
	...
	expectedTrailer := "Co-authored-by: enact-agent <github@enact.ai>"
...
// path: a bare cache that already carries the Enact prepare-commit-msg hook
...
func TestCreateWorktreeRemovesCoAuthoredByHookWhenDisabled(t *testing.T) {
	...
	commitMsg := string(out)
	if strings.Contains(commitMsg, "Co-authored-by: enact-agent") {
...
// prepare-commit-msg hook that does NOT include the enactHookMarker
...
func TestCreateWorktreeRemovesLegacyCoAuthoredByHook(t *testing.T) {
	...
	// previous daemon release (no enactHookMarker line). Keeping a
...
`
	...
	if commitMsg := string(out); strings.Contains(commitMsg, "Co-authored-by: enact-agent") {
...
// without the Enact marker (e.g. one a user added manually) must be left
