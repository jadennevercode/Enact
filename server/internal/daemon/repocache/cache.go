...
type Cache struct {
	root   string // base directory for all caches (e.g. ~/enact_workspaces/.repos)
	logger *slog.Logger
...
const lastUsedFile = ".enact_last_used"
...
type WorktreeParams struct {
	...
	// listed as a writable root — on Linux (enact-ai/enact#2925) and on the
	// Windows native sandbox (enact-ai/enact#6449).
...
const (
	isolatedCheckoutConfigKey   = "enact.checkout-mode"
	isolatedCheckoutConfigValue = "isolated"
	isolatedCacheRemoteName     = "enact-cache"
)
...
func (c *Cache) createOrUpdateIsolatedCheckoutContext(ctx context.Context, barePath, repoURL, checkoutPath, branchName, baseRef string) (string, error) {
	...
	if _, err := os.Stat(checkoutPath); err == nil {
		return "", fmt.Errorf("checkout path already exists and is not a Enact isolated checkout: %s", checkoutPath)
...
// deleteStaleAgentBranches prunes branches left by earlier Enact tasks while
...
// enactHookMarker is a sentinel comment embedded in every prepare-commit-msg
...
const enactHookMarker = "# enact:prepare-commit-msg:co-authored-by"
...
// treats a hook as Enact-owned if its content contains ANY of these
// substrings. The list deliberately includes the legacy comment that the
// daemon used before enactHookMarker existed, so disabling the toggle on
...
var daemonInstalledHookSignatures = []string{
	enactHookMarker,
	"# Installed by the Enact daemon.",
}

// prepareCommitMsgHook is the prepare-commit-msg hook script that appends a
// Co-authored-by trailer for the Enact Agent to every commit message.
const prepareCommitMsgHook = `#!/bin/sh
# enact:prepare-commit-msg:co-authored-by
# Enact: add Co-authored-by trailer for the Enact Agent.
# Installed by the Enact daemon. Do not edit — it will be overwritten.
...
TRAILER="Co-authored-by: enact-agent <github@enact.ai>"
...
// a Co-authored-by trailer for the Enact Agent. The hook is installed in the
...
// installed by the Enact daemon (current or any previously released
