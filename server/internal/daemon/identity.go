...
// same machine shares one UUID stored at `~/.enact/daemon.id`. Profile
...
// LegacyDaemonUUIDs scans `~/.enact/profiles/*/daemon.id` and returns every
...
// explicitly pins ENACT_DAEMON_ID to the hostname itself, there's nothing
