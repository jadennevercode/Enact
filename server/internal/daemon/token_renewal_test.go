...
func TestTryRenewToken_SurfacesReloginWarningOn401(t *testing.T) {
	...
	if !strings.Contains(out, "enact login") {
		t.Fatalf("401 warning must tell the user to run 'enact login', got: %s", out)
	}
}
...
func TestPreflightAuth_RenewsBeforeWorkspaceSyncOnExpiredToken(t *testing.T) {
	...
	if !strings.Contains(out, "enact login") {
		t.Fatalf("expected the actionable 'run enact login' hint in the WARN, got: %s", out)
	}
}
