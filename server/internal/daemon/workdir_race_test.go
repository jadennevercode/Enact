...
// /enact_workspaces/{ws}/{short-id}/workdir from the running signal
...
// TestTaskTempBaseDir covers the ENACT_AGENT_TEMP_BASE validation contract:
...
func TestTaskTempBaseDir(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Setenv("ENACT_AGENT_TEMP_BASE", `C:\configured-but-ignored`)
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			t.Setenv("ENACT_AGENT_TEMP_BASE", tc.value)
			if !tc.set {
				if err := os.Unsetenv("ENACT_AGENT_TEMP_BASE"); err != nil {
					t.Fatalf("unset ENACT_AGENT_TEMP_BASE: %v", err)
				}
			}
			got, configured, err := taskTempBaseDir()
			...
			if tc.wantErr {
				...
				if !strings.Contains(err.Error(), "ENACT_AGENT_TEMP_BASE") {
					t.Fatalf("error %q does not mention ENACT_AGENT_TEMP_BASE", err)
				}
				return
			}
			if err != nil {
	...
	t.Run("configured base creates private 0700 task dir", func(t *testing.T) {
		t.Setenv("ENACT_AGENT_TEMP_BASE", validBase)
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			t.Setenv("ENACT_AGENT_TEMP_BASE", tc.base)
			dir, lock, err := ensureTaskTempDir("root", "ws", "task")
			if err == nil {
				execenv.ReleaseTaskTempLock(lock)
				_ = os.RemoveAll(dir)
				...
				t.Fatalf("ensureTaskTempDir() = %q with unusable ENACT_AGENT_TEMP_BASE, want error", dir)
			}
			if !strings.Contains(err.Error(), "ENACT_AGENT_TEMP_BASE") {
				t.Fatalf("error %q does not mention ENACT_AGENT_TEMP_BASE", err)
			}
...
// TestRunTask_TaskTempBaseOverride is the ENACT_AGENT_TEMP_BASE counterpart
...
func TestRunTask_TaskTempBaseOverride(t *testing.T) {
	...
	tempBase := t.TempDir()
	t.Setenv("ENACT_AGENT_TEMP_BASE", tempBase)
...
// ENACT_AGENT_TEMP_BASE fails the task with a message naming the variable,
...
func TestRunTask_TaskTempBaseInvalidFailsStartup(t *testing.T) {
	...
	t.Setenv("ENACT_AGENT_TEMP_BASE", missingBase)
...
`
	...
	if err == nil {
		t.Fatal("runTask() succeeded with an unusable ENACT_AGENT_TEMP_BASE, want failure")
	}
	if !strings.Contains(err.Error(), "ENACT_AGENT_TEMP_BASE") {
		t.Fatalf("runTask() error = %v, want it to name ENACT_AGENT_TEMP_BASE", err)
	}
