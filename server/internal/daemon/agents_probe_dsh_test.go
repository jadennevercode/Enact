...
func TestProbeDshEnactProfile(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if got := probeDshEnactProfile(path); got != tc.want {
				t.Fatalf("probeDshEnactProfile() = %v, want %v", got, tc.want)
			}
		})
	}
}

func TestProbeAgentCLIsRequiresDshEnactProfile(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("shell fixture")
	}
	...
	for _, tc := range []struct {
		...
		{name: "profile missing", body: `printf '%s\n' 'missing enact profile'; exit 1`, want: false},
	} {
		t.Run(tc.name, func(t *testing.T) {
			fakeDir := t.TempDir()
