...
//
// A tool call does NOT go to the plugin from here. It goes back to Enact,
...
// pluginHookInvoker performs one hook call against the Enact server.
...
func (s *pluginHookMCPServer) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	case "initialize":
		writePluginHookMCPResult(w, request.ID, map[string]any{
			...
			"serverInfo":      map[string]any{"name": "enact-plugins", "version": "1"},
		})
