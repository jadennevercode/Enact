...
// subagents. The Enact daemon currently models only the parent Codex
...
// lifecycle state with drain/cancel primitives, or the Enact runtime
...
//
// Users who explicitly want Codex native subagents inside a Enact task
// (and accept the lifecycle risk) can keep the feature enabled by setting
// `ENACT_CODEX_MULTI_AGENT=1` in the daemon environment.
//
// Layout note
//
...
// EnactCodexMultiAgentEnv is the env var users can set to keep Codex
...
const EnactCodexMultiAgentEnv = "ENACT_CODEX_MULTI_AGENT"

// enactMultiAgentBeginMarker / enactMultiAgentEndMarker delimit the
// multi-agent-specific managed block. Kept separate from the sandbox
// block so each can evolve and migrate independently.
const (
	enactMultiAgentBeginMarker = "# BEGIN enact-managed multi-agent (do not edit; regenerated by daemon)"
	enactMultiAgentEndMarker   = "# END enact-managed multi-agent"
)
...
var multiAgentBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(enactMultiAgentBeginMarker) +
		`.*?^` + regexp.QuoteMeta(enactMultiAgentEndMarker) + `\n*`)
...
func codexMultiAgentEnabled() bool {
	raw := strings.TrimSpace(os.Getenv(EnactCodexMultiAgentEnv))
...
// renderEnactMultiAgentBlock returns the daemon-managed multi-agent
// block. The body uses `multi_agent = false` when injected inside a
// `[features]` table, and `features.multi_agent = false` otherwise.
func renderEnactMultiAgentBlock(inFeaturesTable bool) string {
	var b strings.Builder
	b.WriteString(enactMultiAgentBeginMarker)
	b.WriteString("\n")
	if inFeaturesTable {
	...
	b.WriteString(enactMultiAgentEndMarker)
	b.WriteString("\n")
	return b.String()
}
...
func injectManagedBlockIntoFeaturesTable(content string) string {
	block := renderEnactMultiAgentBlock(true)
...
//
// When ENACT_CODEX_MULTI_AGENT is set to a truthy value, the function is
...
func ensureCodexMultiAgentConfig(configPath string, logger *slog.Logger) error {
	if codexMultiAgentEnabled() {
		if logger != nil {
			logger.Info("codex multi-agent: leaving Codex native multi-agent untouched per ENACT_CODEX_MULTI_AGENT",
	...
	if hasRootFeaturesTable(existing) {
		...
		block := renderEnactMultiAgentBlock(false)
		if existing == "" {
			updated = block
		} else {
			updated = block + "\n" + existing
