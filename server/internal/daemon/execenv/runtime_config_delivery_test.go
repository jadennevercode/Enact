...
func TestBriefSurfaceDeliveryPolicy(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"comment": {
			mustHave: []string{"`--attachment <path>` to `enact issue comment add`"},
			mustNot:  []string{"enact attachment upload"},
		},
		"assignment": {
			mustHave: []string{"`--attachment <path>` to `enact issue comment add`"},
			mustNot:  []string{"enact attachment upload"},
		...
		"chat_direct": {
			mustHave: []string{"`enact attachment upload <local-path>`"},
		...
		"quickcreate": {
			mustHave: []string{"your stdout is text-only", "`enact issue create` call itself via `--attachment <path>`"},
			mustNot:  []string{"enact attachment upload"},
		},
	}
