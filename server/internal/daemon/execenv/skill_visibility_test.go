...
Hidden body.`,
	...
	// The brief is now the only Enact-rendered skill listing, for every kind.
...
// writeSkillFiles puts the second in `a-b-enact`. The second skill then has
...
func TestBatchSlugAllocationCountsHiddenSkills(t *testing.T) {
	...
	// written — as `a-b-enact`.
	if visible[0].Name != "a-b-enact" {
		t.Errorf("visible skill listed as %q, want %q", visible[0].Name, "a-b-enact")
	}
	if _, err := os.Stat(filepath.Join(dir, "a-b-enact", "SKILL.md")); err != nil {
