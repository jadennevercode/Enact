...
func TestPrepareOpenclawSkillWriteMatchesScanPath(t *testing.T) {
	...
	for _, s := range skills {
		...
		if _, err := os.Stat(want); err != nil {
			t.Errorf("openclaw scan target %s missing — Enact's write path and the openclaw scanner are out of sync: %v", want, err)
		}
	}
}
...
//
// When a enact agent is configured for gateway-mode openclaw and the
