...
func TestWorktreeModeDeliversBranchWithoutSidecars(t *testing.T) {
	...
	for _, sidecar := range []string{".agent_context", ".enact", "CLAUDE.md"} {
...
// An in_place task on the same directory leaves .agent_context/ and .enact/
...
func TestPrepareLocalWorktreeSkipsEnactSidecars(t *testing.T) {
	repo := newTestRepo(t)
	writeFile(t, filepath.Join(repo, ".agent_context", "issue_context.md"), "OTHER issue's brief\n")
	writeFile(t, filepath.Join(repo, ".enact", "project", "resources.json"), "{}\n")
	...
	writeFile(t, filepath.Join(repo, "services", "api", ".enact", "task.json"), "{}\n")
	...
	if _, err := os.Stat(filepath.Join(wt.Path, ".enact")); !os.IsNotExist(err) {
		t.Error("another task's .enact was copied into this worktree")
	}
	...
	if _, err := os.Stat(filepath.Join(wt.Path, "services", "api", ".enact")); !os.IsNotExist(err) {
		t.Error("a subdirectory task's .enact was copied into this worktree")
	}
...
// fails, committing anyway would deliver a branch whose content is Enact's
