...
const sidecarManifestFile = ".enact_sidecar_manifest.json"
...
// down a Enact skill without colliding with a user-installed skill
...
// collision we append `-enact`, then `-enact-2`, `-enact-3`,
...
// baseSlug: the bare slug first, then `-enact`, then numbered variants.
//
...
func skillSlugCandidate(baseSlug string, attempt int) string {
	...
	case attempt <= 0:
		return baseSlug
	case attempt == 1:
		return baseSlug + "-enact"
	default:
		return fmt.Sprintf("%s-enact-%d", baseSlug, attempt)
	}
}
...
// (.agent_context/, .enact/, .claude/skills/, .github/skills/,
...
// issue-review-enact. A managed skill directory is platform-owned — the
