...
// must still exist and the Enact-written sibling must be gone.
...
func TestCleanupSidecarsLeavesUserContentInTrackedDirIntact(t *testing.T) {
	...
	// Imagine Prepare wrote .enact/sidecar.txt and created
	// .enact/ + .enact/project/, then exited. Between Prepare
	// and Cleanup the user dropped their own file under .enact/.
	managedDir := filepath.Join(workDir, ".enact")
	...
	// .enact still holds user-notes.txt, so rmdir must have been
...
// collision-free sibling (which lives under .../issue-review-enact/)
// is fully cleaned up.
//
...
// provider, seed a user skill at the exact slug Enact would use
// (`.claude/skills/issue-review/SKILL.md` etc.), run the full
// Prepare → Inject → Cleanup cycle with a Enact skill of the same
// name, and assert the workdir snapshot is byte-identical to the
// seed. The user's SKILL.md must not be touched, and the Enact
// sibling (which lives at `<slug>-enact`) must be fully removed by
// CleanupSidecars.
...
// — a Enact-only namespace file. If the user already has a file at
...
func TestPrepareThenCleanupSidecarsWorkspaceResourcesCollisionPerProvider(t *testing.T) {
	...
	for _, provider := range allFileBasedProviders {
		...
		t.Run(provider, func(t *testing.T) {
			...
			if err := os.MkdirAll(filepath.Join(workDir, ".enact", "project"), 0o755); err != nil {
				t.Fatalf("seed dir: %v", err)
			}
			userBody := `{"user":"owns this file"}`
			userPath := filepath.Join(workDir, ".enact", "project", "resources.json")
			if err := os.WriteFile(userPath, []byte(userBody), 0o644); err != nil {
				t.Fatalf("seed user file: %v", err)
			}
...
// first try the natural slug, then `-enact`, then `-enact-2`,
// `-enact-3`, … The PR-review concern is "Enact skill must still
...
func TestAllocateCollisionFreeSkillDir(t *testing.T) {
	...
	// 2) Pre-existing user dir at the base slug → bump to `-enact`.
	...
	if slug != "issue-review-enact" {
		t.Errorf("second allocation should bump to `-enact`; got %q", slug)
	}
	if dir != filepath.Join(parent, "issue-review-enact") {
		t.Errorf("second allocation path = %q, want under parent", dir)
	}

	// 3) Pre-existing collision at the bumped slug too → bump again.
	if err := os.MkdirAll(filepath.Join(parent, "issue-review-enact"), 0o755); err != nil {
	...
	if slug != "issue-review-enact-2" {
		t.Errorf("third allocation should be `-enact-2`; got %q", slug)
	}
	if dir != filepath.Join(parent, "issue-review-enact-2") {
...
// `Issue Review` skill, the Enact sibling must land at a different
// slug (so the agent still sees it), AND Cleanup must remove the
// Enact sibling entirely without touching the user's.
func TestPrepareThenCleanupSidecarsMultiSkillCollisionFreeAllocation(t *testing.T) {
	...
	// Run only the inject side first — verify the Enact skill
	...
	if err := writeContextFiles(workDir, "claude", TaskContextForEnv{
		IssueID: "11111111-2222-3333-4444-555555555555",
		AgentSkills: []SkillContextForEnv{
			{Name: "Issue Review", Content: "Enact's version\n"},
		},
	}, manifest); err != nil {
		t.Fatalf("writeContextFiles: %v", err)
	}

	enactDir := filepath.Join(workDir, ".claude", "skills", "issue-review-enact")
	if _, err := os.Stat(filepath.Join(enactDir, "SKILL.md")); err != nil {
		t.Errorf("Enact sibling skill should exist at %s: %v", enactDir, err)
	}
	got, err := os.ReadFile(userFile)
	if err != nil {
	...
	// Enact sibling is gone; user's path survives.
	if err := writeSidecarManifest(envRoot, manifest); err != nil {
		t.Fatalf("write manifest: %v", err)
	}
	if err := CleanupSidecars(envRoot); err != nil {
	...
	if _, err := os.Stat(enactDir); !os.IsNotExist(err) {
		t.Errorf("Enact sibling should be removed by Cleanup; stat err=%v", err)
	}
	got, err = os.ReadFile(userFile)
	if err != nil {
