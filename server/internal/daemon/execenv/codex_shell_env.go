...
// daemon-spawned Codex process can have ENACT_TOKEN while `enact issue ...`
...
const (
	...
	enactShellEnvBeginMarker  = "# BEGIN enact-managed shell-environment (do not edit; regenerated by daemon)"
	enactShellEnvEndMarker    = "# END enact-managed shell-environment"
)

var shellEnvBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(enactShellEnvBeginMarker) +
		`.*?^` + regexp.QuoteMeta(enactShellEnvEndMarker) + `\n*`)
...
// custom_env. Inherited ENACT_* variables are always dropped because they
// belong to the daemon process, not necessarily to this task. Explicit
// ENACT_* values are safe to include because daemon.go blocklists that
...
func renderEnactShellEnvBlock(includeOnly []string) (string, error) {
	...
	b.WriteString(enactShellEnvBeginMarker)
	b.WriteByte('\n')
	b.WriteString("[")
	...
	b.WriteByte('\n')
	b.WriteString(enactShellEnvEndMarker)
	b.WriteByte('\n')
	return b.String(), nil
}
...
func EnsureCodexShellEnvPolicyConfig(configPath string, includeOnly []string, logger *slog.Logger) error {
	...
	block, err := renderEnactShellEnvBlock(includeOnly)
	if err != nil {
		return err
	}
