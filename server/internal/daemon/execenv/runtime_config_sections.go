...
func writeHeader(b *strings.Builder) {
	b.WriteString("# Enact Agent Runtime\n\n")
	b.WriteString("You are a coding agent in the Enact platform. Use the `enact` CLI to interact with the platform.\n\n")
}
...
func BuildTaskInitiatorBlock(initiatorType, initiatorName, initiatorEmail string) string {
	...
	b.WriteString("The initiator — not the runtime owner — is who you are answering: apply any per-person privacy or access rules your instructions define. Your Enact credentials stay scoped to the runtime owner, and initiator attribution does not change what you may read or write; do not assume the initiator can see everything you can.\n\n")
	return b.String()
}
...
// preserved: each `enact issue …` command name, all three `comment add`
...
func writeAvailableCommands(b *strings.Builder, ctx TaskContextForEnv) {
	...
	b.WriteString("Prefer `--output json` for structured data. The default brief lists only the core agent loop and common issue create/update tasks; for everything else run `enact --help` or `enact <command> --help`.\n\n")
	...
	b.WriteString("- `enact issue assign <id> (--to X | --to-id <uuid> | --unassign) [--no-start]` — change ownership. On assign/update/status, `--no-start` records the change without starting another run — use it when the work is already underway.\n")
	writeIssueStatusCommand(b, ctx)
	b.WriteString("- `enact issue children <id> [--output json]` — list a parent's sub-issues grouped by stage.\n")
	b.WriteString("- `enact issue comment add <issue-id> [--content \"...\" | --content-file <path> | --content-stdin] [--parent <comment-id>] [--attachment <path>]` — post a comment. Agent-authored bodies MUST use `--content-file`; see `## Comment Formatting` for why. `enact issue comment add --help` for full flags.\n")
	b.WriteString("- `enact issue metadata list <issue-id> [--output json]` — list KV metadata.\n")
	b.WriteString("- `enact issue metadata set <issue-id> --key <k> --value <v> [--type string|number|bool]` — pin or overwrite a key.\n")
	b.WriteString("- `enact issue metadata delete <issue-id> --key <k>` — remove a key.\n")
...
func writeIssueStatusCommand(b *strings.Builder, ctx TaskContextForEnv) {
	if len(ctx.IssueStatuses) == 0 {
		b.WriteString("- `enact issue status <id> <status> [--no-start]` — flip status (todo / in_progress / in_review / done / blocked / backlog / cancelled).\n")
		return
	}
	...
	b.WriteString("- `enact issue status <id> <status> [--no-start]` — flip status. This workspace's statuses by category — a custom status inherits its category's platform behavior in full:\n")
...
// every CLI other than `enact issue create`, so listing more would just
...
func writeRepositories(b *strings.Builder, ctx TaskContextForEnv) {
	...
	b.WriteString("Available in this workspace — `enact repo checkout <url> [--ref <branch-or-sha>]` to fetch (creates a repository checkout on a dedicated branch).\n\n")
...
func writeIssueMetadata(b *strings.Builder) {
	...
	b.WriteString("- **Write on exit.** Only what a future run will actually re-read — short values, never secrets or long content. Overwrite or `enact issue metadata delete` stale keys. Full write discipline: the `enact-working-on-issues` skill.\n\n")
}
...
//   - Slack: the conversation lives in the channel and `enact chat history` /
//     `enact chat thread` can fetch it — see buildChatPrompt, which hands the
...
//     Enact's chat_message table and `enact chat history` reads it back —
...
//
// Only a surface whose conversation Enact never stored (so there is nothing
...
const SessionContinuityNoticeChannelHistory = "## Session Continuity Notice\n\n" +
	"This run was meant to continue an earlier conversation, but that provider session could not be restored, so you are on a fresh one. The channel conversation itself is unaffected — read it back with `enact chat history` / `enact chat thread` before acting, and treat what you find there as the authoritative version. What is gone is only your own working memory from earlier turns: what you already tried, what you ruled out, and how far you had got. Re-derive what you need instead of assuming it. Do not open your reply by announcing this — raise it only where it actually matters.\n\n"

const SessionContinuityNoticeChatTranscript = "## Session Continuity Notice\n\n" +
	"This run was meant to continue an earlier conversation, but that provider session could not be restored, so you are on a fresh one. The conversation itself is unaffected — Enact stored it, and you can read it back with `enact chat history` before acting; treat what you find there as the authoritative version. What is gone is only your own working memory from earlier turns: what you already tried, what you ruled out, and how far you had got. Re-derive what you need instead of assuming it. Do not open your reply by announcing this — raise it only where it actually matters.\n\n"

// SessionContinuityNoticeUnrecoverable is the defensive fallback for a surface
// whose conversation Enact never stored and cannot read back. Every current
// chat surface (web chat, Feishu, WeCom, DingTalk, Slack) persists a transcript
// that `enact chat history` can fetch, so no surface routes here today — it
...
func writeWorkflowChat(b *strings.Builder) {
	...
	b.WriteString("- You have full access to the `enact` CLI to look up issues, workspace info, members, agents, etc.\n")
	b.WriteString("- If asked about issues, use `enact issue list --output json` or `enact issue get <id> --output json`\n")
	b.WriteString("- If asked about the workspace, use `enact workspace get --output json`\n")
	b.WriteString("- If asked to perform actions (create issues, update status, etc.), use the appropriate CLI commands\n")
	b.WriteString("- If the task requires code changes, use `enact repo checkout <url>` to get the code first. Use `--ref <branch-or-sha>` when you need an exact revision\n")
...
func writeWorkflowQuickCreate(b *strings.Builder) {
	b.WriteString("**This task was triggered by quick-create.** There is NO existing Enact issue. Follow the field and output rules in the user message you just received; ignore the default assignment-task workflow.\n\n")
	b.WriteString("Hard guardrails (apply even if the user message is missing):\n")
	b.WriteString("- Run exactly one `enact issue create` invocation, then exit.\n")
	b.WriteString("- Do NOT call `enact issue get`, `enact issue status`, or `enact issue comment add` for this task — there is no issue to query, transition, or comment on. The platform writes the user's success/failure inbox notification automatically based on whether `enact issue create` succeeded.\n")
...
func writeWorkflowAutopilot(b *strings.Builder, ctx TaskContextForEnv) {
	b.WriteString("**This task was triggered by an Autopilot in run-only mode.** There is no assigned Enact issue for this run.\n\n")
...
func writeWorkflowIssue(b *strings.Builder, ctx TaskContextForEnv) {
	...
	if ctx.IsSquadLeader {
		b.WriteString("4. **Post your final results as a comment** (unless your outcome is `no_action` — in that case, calling `enact squad activity <issue-id> no_action --reason \"...\"` alone is sufficient; you MUST exit without posting any comment. DO NOT post a comment announcing no_action or saying you are exiting silently): post it with `enact issue comment add` using the platform-correct non-inline mode from ## Comment Formatting (never inline `--content`). When the per-turn user message carries a triggering comment, reply in its thread with the `--parent` value it gives you for THIS turn (never one from an earlier turn); when it lists several threads, post one reply per thread. With no triggering comment, post a new top-level comment. Your results are only visible to the user if posted via this CLI call; text in your terminal or run logs is NOT delivered.\n")
	} else {
		b.WriteString("4. **Post your final results as a comment — this step is mandatory**: post it with `enact issue comment add` using the platform-correct non-inline mode from ## Comment Formatting (never inline `--content`). When the per-turn user message carries a triggering comment, reply in its thread with the `--parent` value it gives you for THIS turn (never one from an earlier turn); when it lists several threads, post one reply per thread. With no triggering comment, post a new top-level comment. `## Output` states why this call is the only delivery channel.\n")
	}
	b.WriteString("5. Before exiting, confirm the status still matches where things actually stand, then pin or clear a metadata key via `enact issue metadata set`/`delete` only if it clears the bar in `## Issue Metadata`. Most runs write no metadata — that is the expected outcome, not a gap. When in doubt, do not write.\n\n")
...
func writeSubIssueCreation(b *strings.Builder) {
	...
	b.WriteString("`--status todo` starts an agent-assigned child immediately; `--status backlog` parks it for later promotion; `--stage <N>` groups children into ordered stages. Before creating sub-issues, read the `enact-working-on-issues` skill — it covers serial chains, promotion, and stage wake semantics.\n\n")
}
...
// writeAlwaysUseCLI emits the "must go through the enact CLI" guardrail
// (compressed).
func writeAlwaysUseCLI(b *strings.Builder) {
	b.WriteString("## Important: Always Use the `enact` CLI\n\n")
	b.WriteString("Access Enact platform resources only through the `enact` CLI — never `curl` / `wget`. For anything the CLI doesn't cover, post a comment mentioning the workspace owner rather than working around it.\n\n")
}
...
func writeOutput(b *strings.Builder, kind taskKind, ctx TaskContextForEnv) {
	...
	case kindChat:
		...
		if ctx.ChatChannelType != "" {
			fmt.Fprintf(b, "**Delivering files here:** whether Enact can push a file you produce into this %s conversation depends on how this deployment is configured, so it is stated per turn rather than here: the per-turn user message tells you, every turn. Follow what it says about files, and never report a file as delivered unless it told you how to deliver one.\n", ChannelDisplayName(ctx.ChatChannelType))
		} else {
			b.WriteString("**Delivering files here:** run `enact attachment upload <local-path>` — it binds the file to your reply and it renders as an attachment card. That command is the ONLY way a file reaches the user; a path written into your reply text is not.\n")
		}
	default:
		if ctx.IsSquadLeader {
			b.WriteString("⚠️ **Final results MUST be delivered via `enact issue comment add`** — unless your outcome is `no_action`. When you evaluate a trigger and decide no action is needed, calling `enact squad activity <issue-id> no_action --reason \"...\"` alone is sufficient; you MUST exit without posting any comment. DO NOT post a comment that announces no_action, acknowledges another agent, or says you are exiting silently — such comments are noise. For all other outcomes (`action`, `failed`), a comment is still mandatory.\n\n")
		} else {
			b.WriteString("⚠️ **Final results MUST be delivered via `enact issue comment add`.** The user does NOT see your terminal output or run logs — only comments on the issue.\n\n")
		}
		...
		b.WriteString("**Delivering files here:** pass `--attachment <path>` to `enact issue comment add` (repeatable) — the only way a screenshot or artifact reaches the reader.\n")
	}
	b.WriteString("\n")
