...
// runtimeMarkerBegin and runtimeMarkerEnd delimit the Enact-managed brief
...
const (
	runtimeMarkerBegin = "<!-- BEGIN ENACT-RUNTIME (auto-managed; do not edit) -->"
	runtimeMarkerEnd   = "<!-- END ENACT-RUNTIME -->"
...
// writeRuntimeConfigFile writes the Enact runtime brief to path without
...
// locateMarkerBlock finds the [start, end) byte range of the Enact marker
...
//
//   - User content carries a stray `<!-- END ENACT-RUNTIME -->` (e.g. a
...
// CleanupRuntimeConfig excises the Enact marker block from the runtime
...
// of Enact runs without ever touching a single non-managed byte.
//
...
// the user inside the same directory after a Enact task would pick up
...
// agent about the Enact runtime environment and available CLI tools.
//
