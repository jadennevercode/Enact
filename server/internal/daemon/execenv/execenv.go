...
// checked out on demand by the agent via `enact repo checkout`.
package execenv
...
type PrepareParams struct {
	WorkspacesRoot string // base path for all envs (e.g., ~/enact_workspaces)
	...
	// Enact profile dir could not be resolved.
	...
	// state.db task-local — no agent or conversation to key on, or the Enact
...
type Environment struct {
	...
	LocalDirectory bool
	// EnactConfigRoot is the private per-task config directory exported to
	// child CLI invocations. It prevents implicit discovery of the daemon
	// owner's ~/.enact profile without changing the provider-facing HOME.
	EnactConfigRoot string
...
// on demand via `enact repo checkout <url>`.
func Prepare(params PrepareParams, logger *slog.Logger) (*Environment, error) {
	...
	// that lose all ENACT_* env vars AND escape above the workdir. Non-fatal:
	...
	enactConfigRoot := filepath.Join(envRoot, "enact-config")
	if err := os.MkdirAll(enactConfigRoot, 0o700); err != nil {
		return nil, fmt.Errorf("execenv: create task-local Enact config directory: %w", err)
	}
	if err := os.Chmod(enactConfigRoot, 0o700); err != nil {
		return nil, fmt.Errorf("execenv: restrict task-local Enact config directory: %w", err)
	}
	...
	// belongs to the user, where a leftover marker disables every enact
...
func Reuse(params ReuseParams, logger *slog.Logger) *Environment {
	...
	if env.RootDir != "" {
		env.EnactConfigRoot = filepath.Join(env.RootDir, "enact-config")
		if err := os.MkdirAll(env.EnactConfigRoot, 0o700); err != nil {
			logger.Warn("execenv: restore task-local Enact config directory failed; forcing fresh prepare", "error", err)
			return nil
		}
		if err := os.Chmod(env.EnactConfigRoot, 0o700); err != nil {
			logger.Warn("execenv: restrict task-local Enact config directory failed; forcing fresh prepare", "error", err)
			return nil
		}
	}
	...
	// a collision-free sibling (issue-review, issue-review-enact,
	// issue-review-enact-2, …), accumulating a fresh duplicate on every
...
const ManagedEnvProvenanceManagedBy = "enact-daemon-managed-env"
