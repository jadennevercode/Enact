...
`,
			...
			removed: []string{"legal quoted table", `FOO = "bar"`, `ENACT_TOKEN = "stale"`},
		},
		{
...
`,
	...
	includeOnly := []string{"CUSTOM_FLAG", "ENACT_SERVER_URL", "ENACT_TOKEN", "PATH", "SystemRoot"}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if strings.Contains(s, `"ENACT_*"`) {
				t.Fatalf("managed policy must not use a ENACT_* wildcard:\n%s", s)
			}
...
func TestEnsureCodexShellEnvPolicyConfigRejectsInvalidInputWithoutWriting(t *testing.T) {
	...
	if err := EnsureCodexShellEnvPolicyConfig(configPath, []string{"ENACT_TOKEN", "PATH"}, testLogger()); err == nil {
...
func TestEnsureCodexShellEnvPolicyConfigIsIdempotent(t *testing.T) {
	t.Parallel()
	configPath := filepath.Join(t.TempDir(), "config.toml")
	includeOnly := []string{"ENACT_SERVER_URL", "ENACT_TOKEN", "PATH"}
	...
	s := string(data)
	assertValidToml(t, s)
	if n := strings.Count(s, enactShellEnvBeginMarker); n != 1 {
