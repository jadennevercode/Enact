...
func TestPrepareDirectoryMode(t *testing.T) {
	...
	for _, sub := range []string{"workdir", "output", "logs", "enact-config"} {
		path := filepath.Join(env.RootDir, sub)
		if _, err := os.Stat(path); os.IsNotExist(err) {
			t.Fatalf("expected %s to exist", path)
		}
	}
	if env.EnactConfigRoot != filepath.Join(env.RootDir, "enact-config") {
		t.Fatalf("EnactConfigRoot = %q, want task-local config directory", env.EnactConfigRoot)
	}
	info, err := os.Stat(env.EnactConfigRoot)
	if err != nil {
		t.Fatalf("stat EnactConfigRoot: %v", err)
	}
	if got := info.Mode().Perm(); got != 0o700 {
		t.Fatalf("EnactConfigRoot mode = %o, want 700", got)
	}
...
func TestPrepareWithProjectResources(t *testing.T) {
		...
		ProjectResources: []ProjectResourceForEnv{
			{
				...
				ResourceRef:  json.RawMessage(`{"url":"https://github.com/enact-ai/enact","ref":"release/v2","default_branch_hint":"main"}`),
			},
		},
	}
	env, err := Prepare(PrepareParams{
	...
	resourcesPath := filepath.Join(env.WorkDir, ".enact", "project", "resources.json")
...
func TestPrepareWithRepoContext(t *testing.T) {
	...
	for _, e := range entries {
		name := e.Name()
		if name != ".agent_context" && name != ".enact" && name != "CLAUDE.md" && name != ".claude" {
...
func TestWriteContextFilesAutopilotRunOnly(t *testing.T) {
	...
	for _, want := range []string{
		...
		"enact autopilot get autopilot-1 --output json",
		"no assigned issue",
	} {
		if !strings.Contains(s, want) {
	...
	if strings.Contains(s, "Run `enact issue get") {
...
// collision-free duplicates (issue-review, issue-review-enact,
// issue-review-enact-2, …). Reuse rolls back the prior dispatch's writes
...
func TestReuseRefreshesSkillsWithoutDuplicating(t *testing.T) {
	...
	if len(names) != 1 || names[0] != "issue-review" {
		t.Fatalf("after re-dispatch the skills dir = %v, want exactly [issue-review] with no -enact duplicates", names)
	}
...
// next refresh dodges to issue-review-enact. Reuse must reclaim the
...
func TestReuseReclaimsManagedSkillDirWithStrayAgentFile(t *testing.T) {
	...
	if len(names) != 1 || names[0] != "issue-review" {
		t.Fatalf("after reuse with a stray agent file the skills dir = %v, want exactly [issue-review] with no -enact duplicate", names)
	}
...
func TestInjectRuntimeConfigAvailableCommandsCoreOnly(t *testing.T) {
	...
	for _, want := range []string{
		...
		"enact issue create --title",
		"enact issue update <id>",
		"enact issue assign <id>",
		"--no-start",
		"--description-file <path>",
		"--parent \"\"",
		"enact repo checkout <url>",
		"enact issue status <id> <status> [--no-start]",
		"enact issue comment add <issue-id>",
		"enact issue comment add --help",
	} {
		if !strings.Contains(s, want) {
	...
	for _, want := range []string{
		"### Squad maintenance",
		"enact squad member set-role <squad-id>",
	} {
	...
	for _, banned := range []string{
		"enact issue list [--status",
		"enact issue label list",
		"enact issue subscriber list",
		"enact label list",
		"enact workspace member list",
		"enact agent list",
		"enact squad list",
		"enact issue runs",
		"enact issue run-messages",
		"enact attachment download",
		"enact autopilot list",
		"enact autopilot create",
		"enact autopilot update",
		"enact autopilot trigger",
		"enact autopilot delete",
		"enact project get",
		"enact project resource list",
		"enact issue label add",
		"enact issue label remove",
		"enact issue subscriber add",
		"enact issue subscriber remove",
		"enact issue comment delete",
		"enact label create",
	} {
		if strings.Contains(s, banned) {
...
func TestInjectRuntimeConfigNoSkills(t *testing.T) {
	...
	s := string(content)
	if !strings.Contains(s, "enact issue get") {
		t.Error("should reference enact CLI even without skills")
	}
...
// user-installed skill already sitting at the natural slug pushes Enact's
// copy to `<slug>-enact`; the frontmatter has to follow it there, or the
...
func TestWriteContextFilesFrontmatterNameFollowsCollisionSlug(t *testing.T) {
	...
	// Enact's copy landed at the fallback slug and names itself after it.
	moved, err := os.ReadFile(filepath.Join(dir, ".opencode", "skills", "review-enact", "SKILL.md"))
	if err != nil {
...
func TestInjectRuntimeConfigQoderCN(t *testing.T) {
	...
	if !strings.Contains(string(content), "Enact Agent Runtime") {
		t.Error("AGENTS.md missing meta skill header")
	}
}
...
func TestPrepareWithRepoContextOpencode(t *testing.T) {
	...
	for _, e := range entries {
		name := e.Name()
		if name != ".agent_context" && name != ".enact" && name != "AGENTS.md" {
			t.Errorf("unexpected entry in workdir: %s", name)
		}
	}
...
func TestInjectRuntimeConfigRequiresExplicitCommentPost(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			for _, want := range []string{
				"Final results MUST be delivered via `enact issue comment add`",
...
// non-ASCII as `?` before reaching `enact.exe` (#2198 / #2236 / #2376).
//
// Not parallel: mutates the package-level runtimeGOOS.
...
`
	...
	for _, marker := range []string{
		enactManagedBeginMarker,
		enactMultiAgentBeginMarker,
		enactMemoryFeatureBeginMarker,
		enactMemoryConfigBeginMarker,
	} {
...
func TestEnsureCodexSandboxConfigCreatesDefaultLinux(t *testing.T) {
	...
	s := string(data)
	if !strings.Contains(s, enactManagedBeginMarker) || !strings.Contains(s, enactManagedEndMarker) {
...
func TestEnsureCodexSandboxConfigIsIdempotent(t *testing.T) {
	...
	if n := strings.Count(string(data), enactManagedBeginMarker); n != 1 {
...
`
	...
	beginIdx := strings.Index(s, enactManagedBeginMarker)
	endIdx := strings.Index(s, enactManagedEndMarker)
	tableIdx := strings.Index(s, "[permissions.enact]")
	...
	if !(beginIdx < endIdx && endIdx < tableIdx) {
		t.Errorf("managed block must be hoisted above [permissions.enact]; got begin=%d end=%d table=%d:\n%s", beginIdx, endIdx, tableIdx, s)
	}
	...
	if !strings.Contains(s, `trust = "always"`) {
		t.Error("lost user permissions.enact content")
	}
	...
	if n := strings.Count(string(data2), enactManagedBeginMarker); n != 1 {
...
`
	...
	if beginIdx < 0 || tableIdx < 0 || beginIdx > tableIdx {
		t.Errorf("expected managed block to be hoisted above [permissions.enact], got:\n%s", s)
	}
	if strings.Count(s, enactManagedBeginMarker) != 1 {
...
func TestPrepareCodexHomeWritesManagedSandboxBlock(t *testing.T) {
	...
	s := string(data)
	if !strings.Contains(s, enactManagedBeginMarker) {
...
func TestReuseRestoresCodexHome(t *testing.T) {
	...
	if reused.EnactConfigRoot != filepath.Join(reused.RootDir, "enact-config") {
		t.Fatalf("EnactConfigRoot = %q, want restored task-local config directory", reused.EnactConfigRoot)
	}
	if info, err := os.Stat(reused.EnactConfigRoot); err != nil {
		t.Fatalf("stat restored EnactConfigRoot: %v", err)
	} else if got := info.Mode().Perm(); got != 0o700 {
		t.Fatalf("restored EnactConfigRoot mode = %o, want 700", got)
	}
	...
	if !strings.Contains(string(data), enactManagedBeginMarker) {
		t.Error("reused config.toml missing enact-managed block")
	}
}
...
// inside a Enact task, despite the daemon redirecting CODEX_HOME to a
// per-task directory.
...
func TestInjectRuntimeConfigAssignmentTriggerScansRootsFirst(t *testing.T) {
	...
	for _, banned := range []string{
		"enact issue comment list issue-1 --output json",
...
func TestInjectRuntimeConfigCatchUpScansRootsFirst(t *testing.T) {
	...
	if strings.Contains(s, "enact issue comment list issue-1 --recent") {
...
func TestInjectRuntimeConfigIssueMetadataSectionScope(t *testing.T) {
	...
	coreDiscoveryLines := []string{
		...
		"enact issue metadata set <issue-id> --key <k> --value <v> [--type string|number|bool]",
		"enact issue metadata delete <issue-id> --key <k>",
	}
	...
	withSection := wantSection{
		present: []string{
			...
			// enact-working-on-issues skill, pinned by
			...
			"the `enact-working-on-issues` skill",
		},
	}
	withoutSection := wantSection{
		// We can't simply require `enact issue metadata list` absent
	...
	cases := []struct {
	...
	}{
		{
			...
			workflowStepPresent: []string{
				...
				"enact issue metadata set",
				"enact issue metadata delete",
				"Before exiting",
			},
			workflowAbsent: []string{
				// The redundant standalone read must not come back (#7016).
				"Read the metadata bag (`enact issue metadata list`)",
			},
			want: withSection,
		},
		{
...
func TestInjectRuntimeConfigIssueMetadataCodexFormattingUnchanged(t *testing.T) {
	...
	t.Run("linux_content_file", func(t *testing.T) {
		...
		if strings.Contains(s, "Read the metadata bag (`enact issue metadata list`)") {
