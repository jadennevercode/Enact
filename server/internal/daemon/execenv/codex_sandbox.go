...
// `no such host` errors when calling out (for example, `enact issue get`
// hitting the Enact API). See upstream issue openai/codex#10390.
//
...
//     rejects non-safe mutation commands "by policy" — e.g. `enact issue
...
//     danger-full-access so the Enact CLI can reach the API.
...
// enactManagedBeginMarker / enactManagedEndMarker delimit the block the
...
const (
	enactManagedBeginMarker = "# BEGIN enact-managed (do not edit; regenerated by daemon)"
	enactManagedEndMarker   = "# END enact-managed"
)

// renderEnactManagedBlock produces the managed block for the given policy.
//
...
//     other table (e.g. `[permissions.enact]`), a bare `sandbox_mode = ...`
...
// at the top of the file (see upsertEnactManagedBlock), avoids both traps.
func renderEnactManagedBlock(policy codexSandboxPolicy) string {
	var b strings.Builder
	b.WriteString(enactManagedBeginMarker)
	b.WriteString("\n")
	b.WriteString(fmt.Sprintf("sandbox_mode = %q\n", policy.Mode))
	if policy.Mode == "workspace-write" {
		b.WriteString(fmt.Sprintf("sandbox_workspace_write.network_access = %t\n", policy.NetworkAccess))
	}
	b.WriteString(enactManagedEndMarker)
	b.WriteString("\n")
	return b.String()
}
...
var managedBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(enactManagedBeginMarker) +
		`.*?^` + regexp.QuoteMeta(enactManagedEndMarker) + `\n*`)

// upsertEnactManagedBlock returns the config content with the enact-managed
...
// `[permissions.enact]` or `[profiles.foo]`. Combined with the dotted-key
// form used by renderEnactManagedBlock, this means the managed block neither
// leaks into nor inherits from any surrounding table scope.
func upsertEnactManagedBlock(content string, policy codexSandboxPolicy) string {
	// Drop any previously written managed block (wherever it sits).
	content = managedBlockRe.ReplaceAllString(content, "")
	block := renderEnactManagedBlock(policy)
...
// ensureCodexSandboxConfig writes the enact-managed sandbox block into the
...
func ensureCodexSandboxConfig(configPath string, policy codexSandboxPolicy, detectedVersion string, logger *slog.Logger) error {
	...
	updated := upsertEnactManagedBlock(existing, policy)
