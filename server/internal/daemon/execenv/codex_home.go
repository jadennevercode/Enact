...
func prepareCodexHomeWithOpts(codexHome string, opts CodexHomeOptions, logger *slog.Logger) error {
	...
	// `missing field path` and bails out of `thread/start`. Enact writes the
	...
	// so cross-task and cross-workspace context leaks (enact#3130) cannot
...
// enumerated by a plain `codex` run, keeping Enact task history out of the
// user's own thread list.
const codexSessionStoreRoot = "enact-sessions"
...
// shared home's enact-sessions root that have not been touched within
