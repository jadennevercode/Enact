...
// — users cannot audit or edit the contents from any Enact UI.
//
// This conflicts with the Enact daemon's context model. Enact
...
//     unrelated host-project context into Enact tasks. The reproduction
//     in github.com/enact-ai/enact#3130 saw Raw Memories from
//     `D:\Project\MoHaYu\WowChat` (a host-local project) injected into a
//     brand-new Enact issue's first Codex turn.
//
...
//
// Users who explicitly want Codex native memory inside a Enact task
// (and accept the leak risk) can keep the feature enabled by setting
// `ENACT_CODEX_MEMORY=1` in the daemon environment. The long-term
// answer for durable agent context is a Enact-owned, user-visible,
...
// EnactCodexMemoryEnv is the env var users can set to keep Codex
...
const EnactCodexMemoryEnv = "ENACT_CODEX_MEMORY"

const (
	enactMemoryFeatureBeginMarker = "# BEGIN enact-managed memory-feature (do not edit; regenerated by daemon)"
	enactMemoryFeatureEndMarker   = "# END enact-managed memory-feature"
	enactMemoryConfigBeginMarker  = "# BEGIN enact-managed memory-config (do not edit; regenerated by daemon)"
	enactMemoryConfigEndMarker    = "# END enact-managed memory-config"
)
...
var memoryFeatureBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(enactMemoryFeatureBeginMarker) +
		`.*?^` + regexp.QuoteMeta(enactMemoryFeatureEndMarker) + `\n*`)

var memoryConfigBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(enactMemoryConfigBeginMarker) +
		`.*?^` + regexp.QuoteMeta(enactMemoryConfigEndMarker) + `\n*`)
...
func codexMemoryEnabled() bool {
	raw := strings.TrimSpace(os.Getenv(EnactCodexMemoryEnv))
...
// renderEnactMemoryFeatureBlock returns the daemon-managed memory-feature
// block. The body uses `memories = false` when injected inside a
// `[features]` table, and `features.memories = false` otherwise.
func renderEnactMemoryFeatureBlock(inFeaturesTable bool) string {
	var b strings.Builder
	b.WriteString(enactMemoryFeatureBeginMarker)
	b.WriteString("\n")
	if inFeaturesTable {
	...
	b.WriteString(enactMemoryFeatureEndMarker)
	b.WriteString("\n")
	return b.String()
}

// renderEnactMemoryConfigBlock returns the daemon-managed memory-config
// block. The body uses bare keys when injected inside a `[memories]` table
// and dotted-key form otherwise.
func renderEnactMemoryConfigBlock(inMemoriesTable bool) string {
	var b strings.Builder
	b.WriteString(enactMemoryConfigBeginMarker)
	b.WriteString("\n")
	if inMemoriesTable {
	...
	b.WriteString(enactMemoryConfigEndMarker)
	b.WriteString("\n")
	return b.String()
}
...
func injectMemoryFeatureBlockIntoFeaturesTable(content string) string {
	return injectAfterHeader(content, rootFeaturesTableHeaderRe, renderEnactMemoryFeatureBlock(true))
}
...
func injectMemoryConfigBlockIntoMemoriesTable(content string) string {
	return injectAfterHeader(content, rootMemoriesTableHeaderRe, renderEnactMemoryConfigBlock(true))
}
...
//
// When ENACT_CODEX_MEMORY is set to a truthy value, the function is a
...
func ensureCodexMemoryConfig(configPath string, logger *slog.Logger) error {
	if codexMemoryEnabled() {
		if logger != nil {
			logger.Info("codex memory: leaving Codex native memory untouched per ENACT_CODEX_MEMORY",
	...
	if !hasFeatures {
		prepend.WriteString(renderEnactMemoryFeatureBlock(false))
	}
	if !hasMemories {
		prepend.WriteString(renderEnactMemoryConfigBlock(false))
	}
