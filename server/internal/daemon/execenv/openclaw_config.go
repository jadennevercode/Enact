...
// ENACT_OPENCLAW_CLI_TIMEOUT (or backends.openclaw.cli_timeout in the CLI
...
const OpenclawCLITimeoutEnv = "ENACT_OPENCLAW_CLI_TIMEOUT"
...
// explicit (tests) > ENACT_OPENCLAW_CLI_TIMEOUT > openclawCLITimeout.
...
type OpenclawConfigPrep struct {
	...
	// falls back to the ENACT_OPENCLAW_CLI_TIMEOUT override, then to
	// openclawCLITimeout.
...
func buildPerTaskOpenclawConfig(activePath string, exists bool, snapshotPath string, resolvedList []any, agentsFromRegistry bool, workDir string, managedMcp map[string]any, hasManagedMcp bool, gateway OpenclawGatewayPin) map[string]any {
	...
	// fields the enact admin explicitly populated — zero strings/ints
...
//
// Input shape mirrors the rest of Enact: Claude-style
