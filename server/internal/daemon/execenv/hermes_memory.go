...
// agent writes back) in `<HERMES_HOME>/memories/`. Enact has to take over
...
//
// This file makes memory an agent-scoped store Enact owns:
//
//	<enact profile dir>/hermes-state/<agent>/<hermes profile>/memories
//
...
// hermesMemoryStoreRoot is the directory under the daemon's Enact profile dir
...
// agent to key on, or the Enact profile dir cannot be resolved. The daemon
// marks the returned path in-use for the task's duration so
// PruneHermesMemoryStores never reclaims it mid-mount.
//
// daemonProfile namespaces by Enact profile for free: profile dirs are already
...
//     filesystems — the workspaces root and the Enact profile dir can be on
...
// Enact profile dir), so it must actively replace a link left by a previous
