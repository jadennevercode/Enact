...
func newSelfReloadTestDaemon(t *testing.T, runningVersion string) (*Daemon, *atomic.Int32) {
	...
	binary := filepath.Join(t.TempDir(), "enact")
...
// stubSelfVersion makes the `enact --version` probe report version (or fail).
...
func TestParseSelfVersion(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			raw:  "enact 0.3.7 (commit: abc1234, built: 2026-07-29T10:00:00Z)\ngo: go1.26.1, os/arch: darwin/arm64\n",
			want: "0.3.7",
		},
		{
			name: "dev build from git describe",
			raw:  "enact v0.3.7-42-gabcdef0-dirty (commit: abcdef0, built: unknown)\n",
