...
func sessionContinuityNoticeFor(task Task) string {
	...
	// WeCom, DingTalk) reads it back via `enact chat history`; Slack alone
...
func buildActiveSiblingRunsBlock(currentIssueID string, runs []ActiveSiblingRunData) string {
	...
	fmt.Fprintf(&b, " (`enact issue comment list %s --roots-only --summary --compact --output json`)", currentIssueID)
	b.WriteString(" and inspect relevant siblings with the `run-messages` commands below — coordinate with existing work instead of opening a second PR. For writes that only record ownership or status of work already underway, use `--no-start` on `enact issue assign`/`update`/`status`.\n\n")
	for _, run := range runs {
		...
		fmt.Fprintf(&b, "; inspect: `enact issue run-messages %s`\n", run.TaskID)
	}
	b.WriteString("\n")
	return b.String()
}
...
// the agent's job is to translate it into one `enact issue create` CLI
...
// call `enact issue get` or attempt to comment — there's nothing to read
// or reply to.
func buildQuickCreatePrompt(task Task) string {
	var b strings.Builder
	b.WriteString("You are running as a quick-create assistant for a Enact workspace.\n\n")
	b.WriteString("A user captured the following input via the quick-create modal. There is NO existing issue. Your job is to create a well-formed issue from this input with a single `enact issue create` command.\n\n")
	...
	b.WriteString("    - When the user names someone (\"assign to X\" / \"@X\"), call `enact workspace member list --output json`, `enact agent list --output json`, and `enact squad list --output json` and find the matching entity by display name. Squads are first-class assignees too — a squad name (e.g. \"Super Human\") routes work to the squad leader, who then delegates. On a clean unambiguous match, prefer `--assignee-id <uuid>` using the `user_id` (member) or `id` (agent or squad) from that JSON — UUID matching is exact and robust to name collisions in workspaces with overlapping names. `--assignee <name>` (fuzzy) is acceptable as a fallback when names are unambiguous. On no match or ambiguous match, do NOT pass either flag — instead append a final line to the description: `Unrecognized assignee: X`.\n")
	...
	if task.TriggerCommentContent != "" {
		...
		if len(task.CoalescedComments) > 0 {
			...
			if task.NewCommentsSince != "" {
				fmt.Fprintf(&b, "Start with `enact issue comment list %s --since %s --compact --output json`. Treat that as a candidate window, not a guarantee — it also carries unrelated comments, and a retried run can carry ids older than the window. Check every id above against the result.\n\n",
					task.IssueID, task.NewCommentsSince)
			}
			fmt.Fprintf(&b, "Fetch each id you still need directly: `enact issue comment list %s --thread <comment-id> --tail 30 --compact --output json`. `--thread` accepts a reply id, not just a thread root, so you do not need to know which thread the comment lives in. If it is older than those 30 replies, page back with the `Next reply cursor` values (`--before` / `--before-id`) until it appears. Do not finish this turn until every id above is accounted for.\n\n",
				task.IssueID)
		}
...
func buildChatPrompt(task Task) string {
	...
	if task.ChatIntro {
		var b strings.Builder
		b.WriteString("You are running as a chat assistant for a Enact workspace.\n")
	...
	// history is the Enact chat_session the agent already resumes.
	//
	// The history half: `enact chat history` is served by handler/chat_history.go,
	...
	// "its history lives in the channel, NOT in Enact" for every channel type
	// contradicted the very next line on a transcript surface, which tells the
	// agent Enact stored it and hands it the command to read it back. An agent
	...
	if task.ChatChannelType != "" {
		...
		fmt.Fprintf(&b, "You are operating inside a %s conversation — not the Enact web app. Never look in Enact issues or comments for this conversation.\n", platform)
		if task.ChatChannelType == execenv.ChannelTypeSlack {
			fmt.Fprintf(&b, "This conversation and its history live in %s, NOT in Enact. The message below may be only what triggered you. Read the conversation with:\n", platform)
			b.WriteString("- `enact chat history --output json` — the channel overview: recent top-level messages, each thread tagged with a `thread_id` and `reply_count`. It does NOT expand thread contents.\n")
			b.WriteString("- `enact chat thread [<thread_id>] --output json` — read one thread's messages; omit the id to read the thread you are in, or pass a `thread_id` from the overview to read a specific thread.\n")
			if task.ChatInThread {
				b.WriteString("You were @mentioned inside a thread: start with `enact chat thread` to read it; if you need the wider channel, run `enact chat history` and open a specific thread with `enact chat thread <thread_id>`.\n")
			} else {
				b.WriteString("You were @mentioned at the channel top level: start with `enact chat history` to see the channel, then read a specific thread's contents with `enact chat thread <thread_id>`.\n")
			}
			...
			fmt.Fprintf(&b, "The conversation happens in %s, and Enact stores a transcript of it. The message below may be only what triggered you — read it back with `enact chat history` when you need earlier context that is not below.\n", platform)
		} else {
			fmt.Fprintf(&b, "This conversation and its history live in %s, NOT in Enact, and Enact has no history reader for it. Work from the context already provided to you below — no command can fetch more of this conversation. If you genuinely need earlier context that is not here, ask the user for it rather than guessing.\n", platform)
		}
	...
	// have expired. `enact attachment download <id>` re-signs at click
	...
	if len(task.ChatMessageAttachments) > 0 {
		...
		b.WriteString("Use `enact attachment download <id>` to fetch each file locally before referring to it.\n")
		b.WriteString("When creating an issue that should preserve one of these attachments, pass `--attachment-id <id>` to `enact issue create` in addition to keeping the attachment markdown inline.\n")
	}
...
func buildAutopilotPrompt(task Task) string {
	...
	if task.AutopilotID != "" {
		fmt.Fprintf(&b, "Start by running `enact autopilot get %s --output json` if you need the full autopilot configuration, then complete the instructions above.\n", task.AutopilotID)
	} else {
...
// promoted to leader and handed the leader rules (mandatory `enact squad
