...
type AgentEntry struct {
	...
	// Command is the bare command name or ENACT_*_PATH value that Path was
...
// workspace resource as delivered to the daemon. resource_ref is
// type-specific JSON.
...
type Task struct {
	...
	// The JSON tag stays `project_resources`: installed daemons and servers
...
// run `enact attachment download <id>` instead of guessing from a
