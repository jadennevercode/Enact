...
func TestBuildQuickCreatePromptRules(t *testing.T) {
	...
	mustContain := []string{
		...
		// auto-subscribe path fires (enact issue create has no --subscriber flag)
		"CC exception",
		...
		// fixed prefix.
		"enact issue create --output json",
		"JSON response",
		"identifier",
		...
		// attachment boundary (ENA-5696): the ban is scoped to URLs, and file
	...
	if strings.Contains(out, "do NOT pass `--attachment`") {
		t.Errorf("buildQuickCreatePrompt carries the unconditional --attachment ban that conflicts with the quick-create ## Output delivery channel (ENA-5696)\n--- output ---\n%s", out)
	}
}

// TestBuildQuickCreatePromptAssigneeIncludesSquads locks in the ENA-2165
...
// TestBuildQuickCreatePromptSquadDefaultsToSquad locks in the ENA-2203
...
// ENA-5811 itself: the previous implementation grepped Instructions for the
...
// negative regression for ENA-5811: on a current server, a plain agent that
...
func TestBuildChatPromptChannelAwareness(t *testing.T) {
	t.Run("slack-backed prompt teaches both read commands", func(t *testing.T) {
		...
		for _, want := range []string{"Slack", "NOT in Enact", "enact chat history", "enact chat thread", "Do NOT narrate"} {
			if !strings.Contains(out, want) {
	...
	t.Run("top-level mention starts with history", func(t *testing.T) {
		...
		if !strings.Contains(out, "top level: start with `enact chat history`") {
	...
	t.Run("in-thread mention starts with thread", func(t *testing.T) {
		...
		if !strings.Contains(out, "inside a thread: start with `enact chat thread`") {
	...
	t.Run("web-only session has no channel block", func(t *testing.T) {
		...
		if strings.Contains(out, "enact chat history") {
			t.Fatalf("web-only chat prompt should not mention channel history, got:\n%s", out)
		}
	})

	// A transcript surface must not be told its history is "NOT in Enact" and
	// then handed a Enact command to read that history. The claim used to be
	...
	for _, channelType := range []string{
		...
		t.Run(channelType+" transcript prompt does not contradict itself", func(t *testing.T) {
			...
			if strings.Contains(out, "NOT in Enact") {
				t.Errorf("transcript surface told its history is NOT in Enact, then told to read it from Enact\n--- output ---\n%s", out)
			}
			// The useful half of the original sentence must survive: the agent
			// still must not go hunting through issues and comments.
			if !strings.Contains(out, "Never look in Enact issues or comments") {
...
// the ENA-4899 split moved it into the Slack branch along with the read commands
...
// chat channel policy (ENA-4899). Collapsing them into one condition is exactly
...
//   - history: `enact chat history` is injected for Slack (live channel) and
//     for every surface that persists a transcript (Feishu, WeCom, DingTalk);
//     `enact chat thread` is Slack-only. handler/chat_history.go reads the
...
func TestBuildChatPromptTwoLayerChannelPolicy(t *testing.T) {
	...
	// `enact attachment upload` on purpose — to state that it does not apply
	...
	const uploadGuidance = "run `enact attachment upload <local-path>`"
	const historyGuidance = "enact chat history"
	...
	cases := []struct {
		...
		},
		{
			...
			wantPhrases: []string{
				"Feishu/Lark",
				"read it back with `enact chat history`",
...
func TestBuildChatPromptFeishuIgnoresChatInThread(t *testing.T) {
	...
	if strings.Contains(out, "enact chat thread") {
		t.Errorf("feishu prompt must not teach `enact chat thread` (no thread reader)\n--- output ---\n%s", out)
	}
}
...
func TestBuildPromptDefaultScansRootsFirst(t *testing.T) {
	...
	for _, s := range []string{
		"enact issue comment list issue-default-1 --roots-only --summary --compact --output json",
		"--since",
	} {
		if !strings.Contains(out, s) {
	...
	// ENA-5372: the per-turn prompt names only the reads it wants run. Flag
	...
	// ENA-5372: this path now leads with a cheap roots scan, and the scan is
	...
	if strings.Contains(out, "enact issue comment list issue-default-1 --output json") {
...
func TestBuildPromptWarnsAboutActiveSiblingRuns(t *testing.T) {
	...
	for _, want := range []string{
		...
		"ENA-6000",
		"task-existing",
		"enact issue comment list issue-target --roots-only --summary --compact --output json",
		"enact issue run-messages task-existing",
		"--no-start",
	} {
		if !strings.Contains(out, want) {
	...
	if strings.Contains(out, "enact issue runs") {
...
func TestBuildPromptNewCommentsHint(t *testing.T) {
	...
	if !strings.Contains(out, "enact issue comment list "+issueID+" --thread thread-root-1 --since "+since+" --compact --output json") {
	...
	// a rerun of the thread command minus `--thread` (ENA-5721 OPT-1) instead
	...
	if strings.Contains(out, "enact issue comment list "+issueID+" --since "+since+" --output json") {
		t.Errorf("warm hint must not render a second full issue-wide command (ENA-5721 OPT-1), got:\n%s", out)
	}
...
func TestBuildPromptColdStartThreadRead(t *testing.T) {
	...
	if !strings.Contains(out, "enact issue comment list "+issueID+" --thread thread-root-1 --tail 30 --compact --output json") {
		t.Errorf("cold start must point at the triggering thread read, got:\n%s", out)
	}
	// ENA-5372: cross-thread background is a cheap roots scan. The hint names
	...
	// command restating the UUID (ENA-5721 OPT-1).
	...
	if strings.Contains(out, "enact issue comment list "+issueID+" --roots-only --summary --output json") {
		t.Errorf("cold hint must not render a second full command for the roots scan (ENA-5721 OPT-1), got:\n%s", out)
	}
	if strings.Contains(out, "--recent") {
...
func TestBuildPromptResumedNoDeltaDoesNotForceThreadRead(t *testing.T) {
	...
	for _, want := range []string{
		...
		"enact issue comment list " + issueID + " --thread thread-root-1 --tail 30 --compact --output json",
	} {
		if !strings.Contains(out, want) {
...
// TestBuildCommentPromptCoalescedCrossThread pins ENA-4195 review should-fix #3:
...
// read rather than an issue-wide bulk pull (ENA-5442).
//
...
func TestBuildCommentPromptCoalescedIDsOnlyFallback(t *testing.T) {
	...
	t.Run("with since anchor", func(t *testing.T) {
		...
		want := "enact issue comment list issue-fallback-1 --since 2026-08-03T06:00:00Z --compact --output json"
		if !strings.Contains(out, want) {
	...
	t.Run("without since anchor", func(t *testing.T) {
		...
		// recent enough to hold the ids (ENA-5442 review).
...
func assertBoundedIDOnlyFallback(t *testing.T, out string) {
	...
	if strings.Contains(out, "--recent") {
		t.Errorf("id-only fallback must not send the agent at an issue-wide --recent pull (ENA-5442), got:\n%s", out)
	}
	...
	for _, want := range []string{
		"enact issue comment list issue-fallback-1 --thread <comment-id> --tail 30 --compact --output json",
		"accepts a reply id",
		"Next reply cursor",
		"--before-id",
...
// per-thread reply routing (ENA-4348). The invariants:
...
func TestBuildCommentPromptCrossThreadFansOutReplies(t *testing.T) {
	...
	// ENA-5825: the fan-out block points at the brief's `## Comment
	...
	if strings.Contains(out, "enact issue comment add") {
		t.Errorf("cross-thread prompt re-grew embedded comment-add commands (mechanism lives in ## Comment Formatting — ENA-5825), got:\n%s", out)
	}
	...
	// Chronological ordering (ENA-4348 test-round-2 problem #1): replies must be
...
// ENA-5377: the per-run context that was removed from the runtime brief must
...
func TestPerTurnContextBlocksCarryMovedBriefSections(t *testing.T) {
	...
	for _, want := range []string{
		...
		// Issue wording: this task has an IssueID, and since ENA-5722 the two
...
// TestTurnModeMarkersRetired pins ENA-6417: the Reply/Ownership turn-mode
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if strings.Contains(prompt, "Turn mode") {
				t.Errorf("%s prompt must not carry a turn-mode marker (ENA-6417)\n---\n%s", tc.name, prompt)
			}
		})
	}
}
...
func TestBriefCarriesNoModeRouter(t *testing.T) {
	...
	for _, banned := range []string{"Turn mode", "Ownership mode", "Reply mode", "mode block"} {
		if strings.Contains(brief, banned) {
			t.Errorf("brief still references the retired turn-mode split via %q (ENA-6417)\n---\n%s", banned, brief)
		}
	}
...
func TestChatChannelDeliversFilesDefaultsOffAcrossVersions(t *testing.T) {
	...
	out := buildChatPrompt(task)
	if strings.Contains(out, "run `enact attachment upload <local-path>`") {
	...
	if !strings.Contains(buildChatPrompt(delivering), "run `enact attachment upload <local-path>`") {
