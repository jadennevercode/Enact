...
// ParseSelfVersion pulls the version out of `enact --version` output, whose
// first line is rendered by cmd/enact's version template:
//
//	enact 0.3.7 (commit: abc1234, built: 2026-07-29T10:00:00Z)
...
//
// Exported solely so cmd/enact can pin that contract from the producing side:
...
func ParseSelfVersion(raw string) string {
	...
	if fields := strings.Fields(line); len(fields) >= 2 && fields[0] == "enact" {
		return fields[1]
	}
	return line
}
...
// different enact binary". It runs two independent checks on one goroutine,
...
func (d *Daemon) autoUpdateLoop(ctx context.Context) {
	...
	if reloadEnabled {
		d.logger.Info("auto-reload: watching the enact binary on disk",
...
// trySelfReload restarts the daemon when the enact binary on disk no longer
// reports the version compiled into this process.
//
// This is the out-of-band half of self-update. `brew upgrade enact`, a
...
// has its own switch (--no-auto-reload / ENACT_DAEMON_AUTO_RELOAD=false /
// disable_auto_reload) rather than riding on ENACT_DAEMON_AUTO_UPDATE.
//
...
func (d *Daemon) trySelfReload(ctx context.Context) {
	...
	reason := fmt.Sprintf("enact binary on disk reports %s, running %s", onDisk, d.cfg.CLIVersion)
...
// setReloadPending records that a enact version change is confirmed but the
