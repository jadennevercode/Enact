...
func TestBuiltinSkillsConformToTemplate(t *testing.T) {
	...
	for _, skill := range skills {
		t.Run(skill.Name, func(t *testing.T) {
			// The enact- prefix keeps the on-disk slug from colliding with a
			// user-authored workspace skill.
			if !strings.HasPrefix(skill.Name, "enact-") {
				t.Errorf("skill name %q must carry the enact- prefix", skill.Name)
			}
...
// must declare user-invocable:false and fence itself to the enact CLI. New
// contract skills should copy this shape.
func TestMentioningSkillFollowsContractFrontmatter(t *testing.T) {
	skill, ok := findSkill(t, "enact-mentioning")
	if !ok {
		return
	}
	...
	if got := strings.TrimSpace(fm["allowed-tools"]); got != "Bash(enact *)" {
		t.Errorf("allowed-tools = %q, want Bash(enact *) (fence the skill to the CLI it teaches)", got)
	}
}
...
func TestWorkingOnIssuesSkillCoversIssueLoopContracts(t *testing.T) {
	skill, ok := findSkill(t, "enact-working-on-issues")
	if !ok {
		return
	}
	...
	mustContain := []string{
		...
		"open or update a PR before posting the final Enact issue comment",
	...
	mustNotContain := []string{
		...
		"enact issue get <issue-id> --output json",
		"enact issue metadata list <issue-id> --output json",
		"enact issue comment list <issue-id> --thread <trigger-comment-id>",
		"enact issue comment add <issue-id> --parent <trigger-comment-id>",
	}
...
func TestSkillImportingSkillCoversWorkspaceImportContracts(t *testing.T) {
	skill, ok := findSkill(t, "enact-skill-importing")
	if !ok {
		return
	}
	fm, body, _ := splitFrontmatter(skill.Content)
	...
	mustNotContain := []string{
		"enact agent skills set <agent-id> --skill-ids <skill-id>",
...
func TestCreatingAgentsSkillCoversAgentCreationContracts(t *testing.T) {
	skill, ok := findSkill(t, "enact-creating-agents")
	if !ok {
		return
	}
	fm, body, _ := splitFrontmatter(skill.Content)
...
func TestSquadsSkillCoversLeaderRoutingContract(t *testing.T) {
	skill, ok := findSkill(t, "enact-squads")
	if !ok {
		return
	}
	fm, body, _ := splitFrontmatter(skill.Content)
	...
	for _, banned := range []string{
		"enact issue comment list <issue-id> --output json",
		"--recent 10",
	} {
...
func TestAutopilotsSkillCoversDispatchAndSideEffects(t *testing.T) {
	skill, ok := findSkill(t, "enact-autopilots")
	if !ok {
		return
	}
	fm, body, _ := splitFrontmatter(skill.Content)
	...
	mustContain := []string{
		...
		"create_issue",
		"run_only",
		"enact autopilot trigger-add <autopilot-id> --kind schedule",
...
func TestRuntimesAndReposSkillCoversClaimAndCheckoutChain(t *testing.T) {
	skill, ok := findSkill(t, "enact-runtimes-and-repos")
	if !ok {
		return
	}
	fm, body, _ := splitFrontmatter(skill.Content)
	...
	mustContain := []string{
		...
		// runtime gets a linked worktree (enact-ai/enact#2925, #6449).
