...
Enact 原生构建以 `shared/semantic-native.md` 为准：Issue 与 Construction 保存协作过程，
Semantica 原生 RDF/OWL、SHACL、规则、数据和行动绑定、来源与验证报告组成唯一的本体产物。
发布后的 Native Release 是消费与兼容导出的来源。不得同时维护可编辑的 candidate.yaml。

下文保留给用户明确请求的独立 Ontologizer 兼容导出工作区，不能用来推导 Enact 的构建状态。

## 1 · 工作区

**独立兼容工作区是一个目录。** 这些脚本保留独立导出的本地封存能力，
不决定 Enact 的原生本体来源或发布状态。服务端操作回执不倒写兼容导出目录。
...
Enact 语义预览和评估是对明确选定候选、测试数据的受控运行，使用当前工作区身份，
不等于发现新的外部材料。`evaluate` 可以调用已配置的适配器并保存执行证据；
`review` 只看冻结的预览结果，保持所审版本一致。
