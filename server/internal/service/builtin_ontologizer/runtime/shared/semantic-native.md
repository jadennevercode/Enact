...
Read `enact-ontology-authoring/references/native-api-source-map.md` from the
installed main Skill for the exact current routes, payloads, binding templates,
operations preflight and recovery behavior. That reference is authoritative;
this shared capability map must not be used to reconstruct an older wire shape.
...
| ontology | Inspect compiled RDF classes, restrictions, domain/range and SHACL. Domain/range is not property cardinality. The saved business definition remains editable authority; these outputs are Semantica's technical compilation. |
