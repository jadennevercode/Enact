...
// Publishing.
//
// An author uploads an artifact bundle; Enact stores it and serves it. There
...
// PublishLocalBundle publishes from ENACT_PLUGIN_DIR — the development
...
func (s *PluginService) PublishLocalBundle(ctx context.Context, workspaceID, userID pgtype.UUID, name string) (PluginPackageSummary, error) {
	if s.LocalDir == "" {
		return PluginPackageSummary{}, pluginErrf(PluginErrorInvalid, "local plugin sources require ENACT_PLUGIN_DIR")
	}
	if name == "" || strings.ContainsAny(name, `/\`) || strings.HasPrefix(name, ".") {
		return PluginPackageSummary{}, pluginErrf(PluginErrorInvalid, "local plugin source must be a single directory name under ENACT_PLUGIN_DIR")
	}
...
func (s *PluginService) readLocalFile(name, entry string) ([]byte, error) {
	if s.LocalDir == "" {
		return nil, pluginErrf(PluginErrorInvalid, "local plugin sources require ENACT_PLUGIN_DIR")
	}
	if name == "" || strings.ContainsAny(name, `/\`) || strings.HasPrefix(name, ".") {
		return nil, pluginErrf(PluginErrorInvalid, "local plugin source must be a single directory name under ENACT_PLUGIN_DIR")
	}
