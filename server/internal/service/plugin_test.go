...
func TestNormalizeConfigValue(t *testing.T) {
	...
	if value, err := normalizeConfigValue(field("repo"), "enact-ai/enact"); err != nil || value != "enact-ai/enact" {
...
// escape ENACT_PLUGIN_DIR or name a dotfile path.
func TestReadLocalFileStaysInsideThePluginDirectory(t *testing.T) {
	...
	if _, err := disabled.readLocalFile("hello", plugincontract.ManifestFilename); err == nil {
		t.Fatal("local sources must require ENACT_PLUGIN_DIR")
	}
}
