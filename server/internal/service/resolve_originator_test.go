...
func seedOriginatorFanout(t *testing.T, pool *pgxpool.Pool) (memberCommentID, agentCommentID, taskAID, userID, workspaceUUID pgtype.UUID) {
	...
	if err := pool.QueryRow(ctx, `
		...
		VALUES ('Resolve Originator User', 'resolve-originator-fanout@enact.test')
		RETURNING id
	...
	t.Cleanup(func() {
		pool.Exec(context.Background(),
			`DELETE FROM "user" WHERE email = 'resolve-originator-fanout@enact.test'`)
	})
...
// the Stage-completion cascade (ENA-4302; raised by Bohan). Closing the last
...
func TestAttributionForIssueTask_SystemCommentFallsThroughToIssueProvenance(t *testing.T) {
	...
	// WorkspaceID is set so the workspace-scoped trigger-comment lookup (ENA-4252)
...
// the ENA-4305 fix: an agent that creates an issue through the ordinary
...
// consistency guarantee from ENA-4305: the exported OriginatorForIssueTask
...
func TestEnqueueTaskForIssueStoresRuntimeMCPOverlayInQueuedRow(t *testing.T) {
	...
	email := fmt.Sprintf("runtime-overlay-insert-%d@enact.test", suffix)
