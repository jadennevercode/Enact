...
For building mention links, load `enact-mentioning` instead — not this skill.
...
is to open or update a PR before posting the final Enact issue comment, unless
...
When a step depends on PR state, query Enact's link table — do not infer it
from branch names, GitHub search, memory, or `pr_url` metadata (which can be
stale).

```bash
enact issue pull-requests <issue-id> --output json
```
...
```bash
enact issue metadata set <issue-id> --key <key> --value <value>
enact issue metadata delete <issue-id> --key <stale-key>
```
...
- Read what exists before writing: `enact property list` shows the catalog;
  `enact issue property list <issue-id>` shows values set on the issue.
- Set values by property name and option name — the CLI translates to ids:

```bash
enact issue property set <issue-id> --name Environment --value staging
enact issue property set <issue-id> --name Platforms --value "iOS,Android"
enact issue property set <issue-id> --name Reviewer --value Bohan
enact issue property unset <issue-id> --name Environment
```
...
```bash
enact issue assign <issue-id> --to-id <agent-id> --no-start
enact issue update <issue-id> --assignee-id <agent-id> --no-start
enact issue status <issue-id> in_progress --no-start
```
...
```bash
enact issue create --title "..." --parent <issue-id> --assignee <agent> --status todo
```

Strictly serial children — park later steps, promote one at a time:

```bash
enact issue create --title "Step 2: ..." --parent <issue-id> --assignee <agent> --status backlog
enact issue status <child-id> todo   # promote when the previous step is truly done
```
...
enact issue create --title "Research A" --parent <id> --assignee <agent> --stage 1 --status todo
enact issue create --title "Research B" --parent <id> --assignee <agent> --stage 1 --status todo
enact issue create --title "Build"      --parent <id> --assignee <agent> --stage 2 --status backlog
enact issue create --title "Ship"       --parent <id> --assignee <agent> --stage 3 --status backlog
```
...
```bash
enact issue children <parent-id>             # sub-issues grouped by stage
enact issue status <stage-2-child-id> todo   # promote when its deps are met
```
...
enact issue create --title "Step 2" --parent <issue-id> --assignee <agent> --status todo
enact issue create --title "Step 3" --parent <issue-id> --assignee <agent> --status todo

# correct — stage them; Stage 1 runs, later stages park and are promoted as
# each stage's barrier closes
enact issue create --title "Step 1" --parent <issue-id> --assignee <agent> --stage 1 --status todo
enact issue create --title "Step 2" --parent <issue-id> --assignee <agent> --stage 2 --status backlog
enact issue create --title "Step 3" --parent <issue-id> --assignee <agent> --stage 3 --status backlog
```
