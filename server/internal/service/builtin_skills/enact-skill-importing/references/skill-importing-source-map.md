...
Evidence layer for `enact-skill-importing`. Every behavioral claim in `SKILL.md`
...
grep -n "func runSkillImport"           server/cmd/enact/cmd_skill.go
...
## CLI: `enact skill import --url` / `--file`

| Behavior | File:line |
|---|---|
| `skill import` command def | `server/cmd/enact/cmd_skill.go:59-63` |
| `--url` flag | `server/cmd/enact/cmd_skill.go:143` |
| `--file` flag (local `.skill` / `.zip`; mutually exclusive with `--url`) | `server/cmd/enact/cmd_skill.go:144` |
| `--on-conflict` flag (default `fail`) | `server/cmd/enact/cmd_skill.go:145` |
| `--output` flag (default `json`) | `server/cmd/enact/cmd_skill.go:146` |
| `runSkillImport` | `server/cmd/enact/cmd_skill.go:412` |
| Requires exactly one of `--url` / `--file` | `server/cmd/enact/cmd_skill.go:420-427` |
| `--file` reads the archive and posts multipart via `ImportSkillFile` | `server/cmd/enact/cmd_skill.go:436-447`, client method `server/internal/cli/client.go:535` |
| `POST /api/skills/import` (URL, JSON body) | `server/cmd/enact/cmd_skill.go:455` |
| Structured HTTP error body handling | `server/cmd/enact/cmd_skill.go:437-440`, `handleSkillImportError` at `:454` |
| Prints structured result (`json` or table) | `server/cmd/enact/cmd_skill.go:443`, helper at `:497` |
...
| CLI normalizes legacy `{existing_skill}` body into `status:"conflict"` | `server/cmd/enact/cmd_skill.go:454-482`, helper at `:484` |
...
| CLI `agent skills add` def ("without replacing existing assignments") | `server/cmd/enact/cmd_agent.go:125-130` |
| `runAgentSkillsAdd` → `POST .../skills/add` | `server/cmd/enact/cmd_agent.go:797`; POST `:818` |
| CLI `agent skills set` def ("replaces all current assignments") | `server/cmd/enact/cmd_agent.go:118-123` |
| `runAgentSkillsSet` → `PUT .../skills` | `server/cmd/enact/cmd_agent.go:772`; PUT `:790` |
| CLI `agent skills list` | `server/cmd/enact/cmd_agent.go:740`; GET `:750` |
...
| CLI `skill refresh <id>` def / runner | `server/cmd/enact/cmd_skill.go:66`, `runSkillRefresh` at `:425` |
...
| Result envelope (`status`, `reason`, `entity_kind`, `entity_id`, `skill`, `existing_skill`) | `MarketplaceInstallResult`, same file |
...
| Supplying them at install; an agent template prefixes each with its server's name | `restoreMcpEntrySecrets` / `scopedSecrets`, `server/internal/handler/marketplace_install.go` |
...
| Handler tests | `server/internal/handler/marketplace_test.go`, `marketplace_sanitize_test.go` |
