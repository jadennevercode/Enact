---
name: enact-creating-agents
description: "Use when creating, inspecting, or debugging a Enact agent definition via the `enact agent` CLI or POST /api/agents. Not for assigning issues to agents that already exist, and not for runtime task prompts."
user-invocable: false
allowed-tools: Bash(enact *)
---

# Creating Enact agents

This is the contract for Enact's agent-creation path: what the create entry
...
```bash
enact agent get <agent-id> --output json      # full persisted agent record
enact agent skills list <agent-id> --output json   # current skill bindings
enact agent env get <agent-id> --output json  # plaintext env (agent owner or ws owner/admin; agents denied)
```
...
`POST /api/agents` (`enact agent create`). At task claim time the daemon
...
```bash
enact agent create --name <name> --runtime-id <runtime-id> \
...
`enact agent copy <source-agent-id>` forks an existing agent's portable
...
```bash
enact agent copy <source-agent-id> --name "My Agent (copy)"   # same runtime
enact agent copy <source-agent-id> --runtime-id <target> --model <model>  # cross-runtime fork
```
...
```bash
enact agent create --name <name> --runtime-id <runtime-id> --custom-env-stdin --output json
enact agent create --name <name> --runtime-id <runtime-id> --custom-env-file <0600-json> --output json
```
...
- Reading plaintext values requires the dedicated `GET /api/agents/{id}/env`
  endpoint (`enact agent env get`). It is gated to the **agent's own human
...
- Writing values after creation does NOT go through `agent update`. The generic
  ...
  `PUT /api/agents/{id}/env` (`enact agent env set`), which carries the same
...
```bash
enact agent create --name <name> --runtime-id <runtime-id> --mcp-config-file <0600-json> --output json
enact agent update <agent-id> --mcp-config-stdin --output json
enact agent update <agent-id> --mcp-config 'null'   # clears the config
```
...
`enact workspace mcp list|add|update|remove`). Adding one there gives it to
NO agent — same shape as a workspace skill. It reaches an agent only when
someone assigns it:

```bash
enact workspace mcp list --output table        # find the server id
enact agent mcp add <agent-id> <server-id>     # give it to one agent
enact agent mcp disable <agent-id> <server-id> # stop sending it, keep the assignment
enact agent mcp remove <agent-id> <server-id>  # take it away
```
...
- `enact agent create` — inserts a new agent row.
- `enact agent copy` — inserts a new agent row (a fork of an existing agent);
  the source is left untouched.
- `enact agent skills add` / `set` — mutate bindings (`set` is destructive:
  it drops bindings not in the new list).
- `enact agent env set` — overwrites the full `custom_env` map and writes an
  audit row.
