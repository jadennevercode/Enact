---
name: enact-projects-and-resources
description: "Use when creating, inspecting, updating, or debugging Enact projects and their resources (github_repo, local_directory)."
user-invocable: false
allowed-tools: Bash(enact *)
---

# Enact Projects and Resources
...
enact project get <project-id> --output json
...
A project groups work and carries durable resources. A resource is not just display metadata; it is context later injected into task briefs and `.enact/project/resources.json`.

A project's `description` is also durable context: when an issue (or a quick-create task) is bound to a project, the project description is injected into the agent's brief under `## Project Context` and written to `.enact/project/resources.json` as `project_description`. Use it for project-wide rules/context that should apply to every task in the project.
...
enact project list --output json
enact project get <project-id> --output json
enact project create --title "<title>" --repo <github-url> --output json
enact project create --title "<title>" --start-date 2026-03-01 --due-date 2026-03-31 --output json
enact project update <project-id> --title "<title>" --output json
enact project update <project-id> --due-date 2026-04-15 --output json
enact project update <project-id> --start-date "" --output json   # clear the start date
enact project status <project-id> in_progress --output json
enact project resource list <project-id> --output json
...
enact project resource add <project-id> --type local_directory --local-path <abs-path> --daemon-id <daemon-id> --output json
enact project resource add <project-id> --type local_directory --local-path <abs-path> --daemon-id <daemon-id> --execution-mode worktree --output json
enact project resource update <project-id> <resource-id> --execution-mode in_place --output json
enact project resource update <project-id> <resource-id> --url <new-github-url> --output json
enact project resource update <project-id> <resource-id> --ref <branch-or-sha> --output json
enact project resource remove <project-id> <resource-id> --output json
```
...
not advertise the capability — the fix is updating the Enact app there, then retrying. Pass an empty value to clear
...
Project resources are durable and affect future tasks. `enact repo checkout`
is task-local checkout state.

## Debugging wrong context

1. `enact project get <project-id> --output json`.
2. `enact project resource list <project-id> --output json`.
