...
| private agent the actor cannot INVOKE (`canInvokeAgent`, not `canAccessPrivateAgent` — ENA-3963 split see-vs-run) | blocked `invocation_not_allowed` | `server/internal/handler/comment.go` (search `resolveMentionedAgentCommentTriggers`, the agent-branch `canInvokeAgent` call) |
...
| `@all` does NOT suppress an EXPLICIT `@agent`/`@squad` in the same comment — the explicit branch is evaluated first (ENA-5411) | `server/internal/handler/comment.go` (the `hasAgentOrSquadMention` branch immediately above the `HasMentionAll` short-circuit) |
...
| --- | --- | --- |
| `workspace member list` | `user_id` (NOT the membership-row id) | `server/cmd/enact/cmd_workspace.go:465` |
| `agent list` | `id` | `server/cmd/enact/cmd_agent.go:365` |
| `squad list` | `id` | `server/cmd/enact/cmd_squad.go:57` |
