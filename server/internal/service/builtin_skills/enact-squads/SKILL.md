---
name: enact-squads
description: "Use when creating, inspecting, updating, assigning to, or debugging a Enact squad, including how leader routing picks who runs."
...
enact squad get <squad-id> --output json
enact squad member list <squad-id> --output json
enact issue comment list <issue-id> --roots-only --summary --output json
enact issue comment list <issue-id> --thread <thread-id> --tail 30 --output json
```
...
```bash
enact squad --help
enact squad member --help
enact issue update --help
...
A Enact squad is a workspace routing and coordination object.
...
```bash
enact squad list --output json
enact squad get <squad-id> --output json
enact squad create --name <name> --leader <agent-name-or-id> --output json
enact squad update <squad-id> --instructions "<leader coordination policy>" --output json
enact squad delete <squad-id>
```
...
```bash
enact squad member list <squad-id> --output json
enact squad member add <squad-id> --member-id <id> --type agent|member --role <role> --output json
enact squad member remove <squad-id> --member-id <id> --type agent|member
...
```bash
enact squad activity <issue-id> action|no_action|failed --reason "<why>" --output json
```
...
```bash
enact issue get <issue-id> --output json
enact issue update <issue-id> --help
enact issue comment list <issue-id> --roots-only --summary --output json
enact issue comment add <issue-id> --help
```
...
For squad leader tasks, Enact appends a squad leader briefing to the leader
...
members carry no skills segment. Builtin `enact-*` skills are not listed —
...
- Getting the leader briefing does NOT imply status authority. A squad
  ...
  delegation rules yes, `enact issue status` no.
