...
enact repo push
enact repo change-request --title "…" [--body "…"] [--base main] [--draft]
```
...
For a managed GitHub or GitLab repository, use `enact repo push` from the checkout instead of embedding a token in the remote URL or running a provider login flow. The daemon obtains a repository-scoped HTTPS credential, exposes it to Git only through a 0600 temporary credential store, and removes it after the push. After pushing, use `enact repo change-request`; the Enact Server creates the PR/MR with its provider API credential, so that credential never enters the agent process.
