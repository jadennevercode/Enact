...
type PluginService struct {
	...
	// LocalDir is ENACT_PLUGIN_DIR: a directory of plugin sources an author
	// can publish from without zipping and uploading after every edit. Empty
	// disables the development publish channel.
	LocalDir string
	// DevOrigins is ENACT_PLUGIN_DEV_ORIGINS: hook endpoint origins an author
	...
	// DeploymentKey is the raw ENACT_PLUGIN_SECRET_KEY, used to derive each
...
func NewPluginService(queries *db.Queries, txStarter TxStarter) *PluginService {
	service := &PluginService{
		Queries:    queries,
		TxStarter:  txStarter,
		LocalDir:   strings.TrimSpace(os.Getenv("ENACT_PLUGIN_DIR")),
		DevOrigins: parseDevOrigins(os.Getenv("ENACT_PLUGIN_DEV_ORIGINS")),
		Host:       plugincontract.HostCapabilities(),
	}
	service.HookClient = devHookClient(service.DevOrigins, os.Getenv("ENACT_PLUGIN_DEV_CA"))
	return service
}
...
func devHookClient(devOrigins []string, caPath string) *http.Client {
	...
	if err != nil {
		slog.Warn("plugins: ENACT_PLUGIN_DEV_CA could not be read; dev hook endpoints will not be trusted", "path", caPath, "error", err)
		return nil
	}
	pool := x509.NewCertPool()
	if !pool.AppendCertsFromPEM(pem) {
		slog.Warn("plugins: ENACT_PLUGIN_DEV_CA contained no certificates", "path", caPath)
		return nil
	}
...
func (s *PluginService) SetConfig(ctx context.Context, installation db.PluginInstallation, values map[string]any) (db.PluginInstallation, error) {
	...
	if len(secrets) > 0 && s.Secrets == nil {
		return db.PluginInstallation{}, pluginErrf(PluginErrorUnavailable, "plugin secrets are disabled: ENACT_PLUGIN_SECRET_KEY is not configured")
	}
