...
// GetChatChannelHistory serves `enact chat history` — the channel overview:
// recent top-level messages, each thread tagged with its id + reply count (no
// thread contents). The agent drills into a thread with `enact chat thread`.
//
...
// paging contract. It backs `enact chat history` for sessions with no IM
// channel (web chat, Feishu, WeCom, DingTalk), whose history lives only in
// Enact — there is no platform to read back. It pages through the same
...
// GetChatThread serves `enact chat thread [id]` — one thread's messages. With
