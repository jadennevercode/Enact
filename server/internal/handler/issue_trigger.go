...
import (
	...
	agentver "github.com/enact-ai/enact/server/pkg/agent"
...
func (h *Handler) dispatchIssueRun(ctx context.Context, issue db.Issue, trigger service.IssueRunTrigger, actorType, actorID, handoffNote string) {
	...
	case "agent":
		...
		// for the run (ENA-4302 §4). An agent actor is not a human, so only a
...
// assign/promote actor into the attribution resolver (ENA-4302 §4).
...
// (ENA-3375). Mirrors PreviewCommentTriggers.
