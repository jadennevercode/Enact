...
VALUES ($1, 123456789, 'enact-ai', 'enact', 3366, 987654321, 'abc123', 15368, 'completed', now())
`, wsID)
	githubPRID := dbfx.Insert(t, "github_pull_request", testutil.Cols{
		...
		"state":           "open",
		"html_url":        "https://github.com/enact-ai/enact/pull/5265",
...
`, triggerName).Scan(&definition)
		if !strings.Contains(definition, "enact.workspace_teardown") {
...
func TestWorkspaceTeardownModeDoesNotLeakIntoOrdinaryDeletes(t *testing.T) {
	...
	if _, err := tx.Exec(ctx, `SELECT set_config('enact.workspace_teardown', 'on', true)`); err != nil {
		_ = tx.Rollback(ctx)
	...
	if err := conn.QueryRow(ctx, `SELECT current_setting('enact.workspace_teardown', true)`).Scan(&teardownMode); err != nil {
...
`, wsID, testUserID)
	...
	targetEmail := fmt.Sprintf("revocation-%s@enact.ai", slug)
...
`, wsID, testUserID)
	...
	targetUserID := dbfx.Insert(t, "user", testutil.Cols{
		...
		"email": "revocation-no-runtimes@enact.ai",
	})
	t.Cleanup(func() {
		_, _ = testPool.Exec(context.Background(), `DELETE FROM workspace WHERE id = $1`, wsID)
		_, _ = testPool.Exec(context.Background(), `DELETE FROM "user" WHERE id = $1`, targetUserID)
