...
type AppConfig struct {
	...
	// self-hosted instances can show `enact setup self-host` commands
	// with the operator's own domains instead of Enact Cloud defaults.
	DaemonServerURL string `json:"daemon_server_url,omitempty"`
	DaemonAppURL    string `json:"daemon_app_url,omitempty"`

	// VCSIntegrationAvailable mirrors the ENACT_VCS_INTEGRATION_ENABLED
	...
	// ENACT_VCS_SECRET_KEY" hint a cloud user cannot resolve. Omitted when
...
func daemonSetupURLsFromEnv() (string, string) {
	serverURL := normalizePublicURL(os.Getenv("ENACT_PUBLIC_URL"))
...
// (ENACT_APP_URL, falling back to FRONTEND_ORIGIN), normalized. Shared by
// the daemon-setup URLs and the managed-cloud detection so both read the same
// signal.
func resolveFrontendAppURL() string {
	appURL := normalizePublicURL(os.Getenv("ENACT_APP_URL"))
	if appURL == "" {
...
// Enact Cloud, identified by its frontend host alone (enact.ai). The
// daemon setup for the managed cloud is always
// `enact setup` (which hardcodes api.enact.ai), so the per-deployment URLs
// must be omitted from /api/config even when ENACT_PUBLIC_URL is unset or
// misconfigured. Previously this also required serverURL==api.enact.ai, so a
// cloud deployment that forgot ENACT_PUBLIC_URL fell through and emitted a
// `setup self-host --server-url https://enact.ai` command — pointing the
// daemon's backend at the frontend (no /health, no WebSocket proxy).
func isOfficialCloudDaemonConfig(appURL string) bool {
	return urlHostEquals(appURL, "enact.ai")
}

// isOfficialCloudDeployment reports whether this server is the official Enact
// Cloud, reusing the same frontend-host signal as the daemon setup (enact.ai).
