...
const (
	onboardingAssistantName       = "Enact Helper"
	onboardingIssueTitle          = "Start here: learn Enact with Enact Helper"
	onboardingAgentCreationSource = "enact_helper"
...
const onboardingAssistantDescription = "Built-in workspace assistant. Answers Enact questions and runs CLI operations."
...
// Enact Helper agent created by this shim. Pre-v3 desktop submits a
...
const onboardingAssistantInstructions = `You are Enact Helper, the built-in AI assistant for this Enact workspace. Your role is to help any member use Enact better — answer questions, give advice, and execute workspace operations on their behalf.
...
Your toolbox is the ` + "`enact`" + ` CLI. It's already on your PATH and authenticated as the workspace owner.

Your full capability surface = whatever ` + "`enact --help`" + ` shows. Run ` + "`enact --help`" + ` first, then ` + "`enact <command> --help`" + ` for any subcommand; use ` + "`--output json`" + ` for structured data. The CLI is your manifest — never invent commands or flags.
...
const onboardingIssueDescription = `Welcome to Enact.

This is your guided first run. Enact Helper is assigned to this issue and will help you try the core workflow:

1. Read Enact Helper's first comment.
2. Reply with something you want to build, fix, write, or plan.
3. @mention Enact Helper when you want it to continue.
...
//
// Creates or reuses one "Enact Helper" agent on the supplied runtime,
...
func enNoRuntimeIssueDescription() string {
	return strings.Join([]string{
		...
		"",
		"Full guide: https://enact.ai/docs/install-agent-runtime",
		"",
		...
		"",
		"When the runtime is connected, you can create Enact Helper for a guided first run.",
	}, "\n")
}
...
func zhNoRuntimeIssueDescription() string {
	return strings.Join([]string{
		...
		"## 安装第一个 Agent 运行时",
		"",
		"完整文档：https://enact.ai/docs/install-agent-runtime",
		"",
		"中文用户建议先装 Kimi CLI：",
		"",
		...
		"",
		"运行时连上后，你就可以创建 Enact Helper，开始一次有智能体参与的上手引导。",
	}, "\n")
}
