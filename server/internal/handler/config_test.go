...
func TestGetConfigIncludesRuntimeAuthConfig(t *testing.T) {
	...
	t.Setenv("ENACT_PUBLIC_URL", "https://api.example.com/")
	t.Setenv("ENACT_APP_URL", "https://app.example.com/")
...
func TestGetConfigUsesAppURLForSameOriginDaemonSetup(t *testing.T) {
	t.Setenv("ENACT_PUBLIC_URL", "")
	t.Setenv("ENACT_APP_URL", "https://enact.internal.example/")
	...
	if cfg.DaemonServerURL != "https://enact.internal.example" {
		t.Fatalf("daemon_server_url: want same-origin URL, got %q", cfg.DaemonServerURL)
	}
	if cfg.DaemonAppURL != "https://enact.internal.example" {
...
func TestGetConfigUsesFrontendOriginForSameOriginDaemonSetup(t *testing.T) {
	...
	t.Setenv("FRONTEND_ORIGIN", "https://enact.internal.example/")
...
func TestGetConfigOmitsOfficialCloudDaemonSetup(t *testing.T) {
	t.Setenv("ENACT_PUBLIC_URL", "https://api.enact.ai")
	t.Setenv("ENACT_APP_URL", "")
	t.Setenv("FRONTEND_ORIGIN", "https://enact.ai")
...
// frontend is enact.ai, but the deployment does not set ENACT_PUBLIC_URL to
// the api host. Previously this fell through to the same-origin branch and
// emitted daemon_server_url=https://enact.ai, which the dialog turned into
// `enact setup self-host --server-url https://enact.ai` — pointing the
...
// setup URLs are omitted and the dialog falls back to `enact setup`.
...
// cloud frontend when it is configured through ENACT_APP_URL.
func TestGetConfigOmitsCloudDaemonSetupForConfiguredAppURL(t *testing.T) {
	t.Setenv("ENACT_PUBLIC_URL", "")
	t.Setenv("ENACT_APP_URL", "https://enact.ai")
...
func TestURLHostEqualsCanonicalizesCommonHostForms(t *testing.T) {
	tests := []struct {
		...
		{name: "full URL", raw: "https://api.enact.ai", want: true},
		{name: "bare host", raw: "api.enact.ai", want: true},
		{name: "host port", raw: "api.enact.ai:8080", want: true},
		{name: "trailing dot", raw: "https://api.enact.ai.", want: true},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := urlHostEquals(tt.raw, "api.enact.ai"); got != tt.want {
...
func TestGetConfigExposesServerVersion(t *testing.T) {
	...
	t.Setenv("ENACT_APP_URL", "https://enact.self-hosted.example")
	t.Setenv("FRONTEND_ORIGIN", "")
...
// suppressed on the managed cloud (frontend host enact.ai) even when the
...
func TestGetConfigOmitsServerVersionOnOfficialCloud(t *testing.T) {
	...
	// Official cloud: frontend host enact.ai -> version omitted.
	t.Setenv("ENACT_APP_URL", "https://enact.ai")
	t.Setenv("FRONTEND_ORIGIN", "")
	w := httptest.NewRecorder()
	testHandler.GetConfig(w, req)
