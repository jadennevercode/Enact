...
func TestWriteMeasuredJSONByteIdenticalToWriteJSON(t *testing.T) {
	...
	cases := []struct {
		...
		{"large_claim_with_skills", map[string]any{"task": claimResp{
			...
			Skills: []skill{
				{Name: "enact-working-on-issues", Description: "do work <safely> & well", Files: map[string]string{"SKILL.md": "# Title\n<b>x</b> & y"}},
				{Name: "enact-mentioning", Description: "ping people", Files: map[string]string{"SKILL.md": "line1\nline2"}},
			},
