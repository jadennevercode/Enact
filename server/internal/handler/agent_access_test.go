...
func privateAgentTestFixture(t *testing.T) (agentID, ownerID, memberID string) {
	...
	if err := testPool.QueryRow(ctx, `
		...
		VALUES ('Private Agent Owner', 'private-agent-owner@enact.test')
		RETURNING id
	...
	t.Cleanup(func() {
		testPool.Exec(context.Background(),
			`DELETE FROM "user" WHERE email = 'private-agent-owner@enact.test'`)
	})
	...
	if err := testPool.QueryRow(ctx, `
		INSERT INTO "user" (name, email)
		VALUES ('Plain Member', 'plain-member@enact.test')
		RETURNING id
	...
	t.Cleanup(func() {
		testPool.Exec(context.Background(),
			`DELETE FROM "user" WHERE email = 'plain-member@enact.test'`)
	})
...
func TestMentionAgent_RejectsCrossWorkspaceAgentUUID(t *testing.T) {
	...
	if err := testPool.QueryRow(ctx, `
		INSERT INTO "user" (name, email)
		VALUES ('Foreign Owner', 'cross-ws-foreign@enact.test')
		RETURNING id
	...
	t.Cleanup(func() {
		testPool.Exec(context.Background(),
			`DELETE FROM "user" WHERE email = 'cross-ws-foreign@enact.test'`)
	})
	...
	// Enact's mention format is markdown-linked: [@Name](mention://agent/<uuid>).
