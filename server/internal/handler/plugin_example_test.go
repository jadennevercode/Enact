...
// stageExamplePlugin copies the example into a temp ENACT_PLUGIN_DIR and
...
func installExamplePlugin(t *testing.T, servers exampleServers) string {
	...
	plugins.CallbackBaseURL = "https://enact.test/api/v1/plugin"
...
// ENACT_PLUGIN_DEV_CA, pointing at a CA bundle.
...
// The whole round trip: an agent calls the tool, Enact signs the request, the
...
func TestExamplePluginAgentHookRoundTripIsSigned(t *testing.T) {
	...
	sentinel := quietServer(t, func(w http.ResponseWriter, r *http.Request) {
		...
		seenSignature = r.Header.Get("X-Enact-Signature")
		seenTimestamp = r.Header.Get("X-Enact-Timestamp")
