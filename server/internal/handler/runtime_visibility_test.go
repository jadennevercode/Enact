...
func runtimeVisibilityFixture(t *testing.T) (runtimeID, runtimeOwnerID, plainMemberID string) {
	...
	if err := testPool.QueryRow(ctx, `
		...
		VALUES ('Runtime Owner', 'runtime-owner@enact.test')
		RETURNING id
	...
	t.Cleanup(func() {
		testPool.Exec(context.Background(),
			`DELETE FROM "user" WHERE email = 'runtime-owner@enact.test'`)
	})
	...
	if err := testPool.QueryRow(ctx, `
		INSERT INTO "user" (name, email)
		VALUES ('Plain Runtime Member', 'plain-runtime-member@enact.test')
		RETURNING id
	...
	t.Cleanup(func() {
		testPool.Exec(context.Background(),
			`DELETE FROM "user" WHERE email = 'plain-runtime-member@enact.test'`)
	})
