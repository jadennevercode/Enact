...
func (h *Handler) RegisterTelegramBot(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		case errors.Is(err, telegram.ErrBotOwnedByAnotherWorkspace):
			writeError(w, http.StatusConflict, "this Telegram bot is already connected to a different Enact workspace — disconnect it there before connecting it here")
...
// for audit and chat history stays in Enact; a re-install (re-pasting the
...
// Telegram user id carried by the token to the logged-in Enact user. The
...
func (h *Handler) RedeemTelegramBindingToken(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		case errors.Is(err, telegram.ErrBindingAlreadyAssigned):
			writeError(w, http.StatusConflict, "this Telegram account is already bound to a different Enact user")
