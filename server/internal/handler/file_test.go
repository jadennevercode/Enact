...
func TestBuildMarkdownURL_PublicCdnAbsoluteURLReusedVerbatim(t *testing.T) {
	...
	testHandler.cfg.PublicURL = "https://api.enact.test"
	testHandler.CFSigner = nil
	...
	id := seedAttachmentURL(t, "https://cdn.enact.test/uploads/abc.png", "abc.png", "image/png", 1)
	att, err := testHandler.Queries.GetAttachment(context.Background(), db.GetAttachmentParams{
		ID:          parseUUID(id),
		WorkspaceID: parseUUID(testWorkspaceID),
	})
	...
	resp := testHandler.attachmentToResponse(att, attachmentURLModeSigned)
	if resp.MarkdownURL != "https://cdn.enact.test/uploads/abc.png" {
...
func TestBuildMarkdownURL_PrivateBucketWithoutCdnDomainRoutesThroughAPIEndpoint(t *testing.T) {
	...
	resp := testHandler.attachmentToResponse(att, attachmentURLModeSigned)
	want := "https://api.enact.test/api/attachments/" + id + "/download"
...
func TestIsDurablePublicURL(t *testing.T) {
	cases := []struct {
		...
		{"absolute https no signature", "https://cdn.enact.test/foo.png", true},
		{"absolute http no signature", "http://cdn.enact.test/foo.png", true},
