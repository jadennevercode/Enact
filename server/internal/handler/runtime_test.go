...
func TestResolveViewingTZ(t *testing.T) {
	...
	if err := testPool.QueryRow(ctx,
		`INSERT INTO "user" (name, email, timezone)
		 VALUES ('TZ Resolve', 'tz-resolve@enact.ai', 'Asia/Tokyo') RETURNING id`,
	).Scan(&userID); err != nil {
	...
	if err := testPool.QueryRow(ctx,
		`INSERT INTO "user" (name, email)
		 VALUES ('TZ Bare', 'tz-bare@enact.ai') RETURNING id`,
	...
	if err := testPool.QueryRow(ctx,
		`INSERT INTO "user" (name, email, timezone)
		 VALUES ('TZ Bad', 'tz-bad@enact.ai', 'Bad/Zone') RETURNING id`,
