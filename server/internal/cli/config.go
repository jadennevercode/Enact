...
const (
	defaultCLIConfigPath = ".enact/config.json"

	// TaskConfigRootEnv points daemon-managed CLI invocations at a private,
	// per-task Enact config directory. It is deliberately Enact-specific:
	...
	TaskConfigRootEnv = "ENACT_TASK_CONFIG_ROOT"
)
...
type CLIConfig struct {
	...
	// ENACT_DAEMON_DEVICE_NAME env, this field, os.Hostname().
	...
	// ENACT_AGENT_RUNTIME_NAME env, this field, the built-in
	// DefaultRuntimeName.
	...
	// --workspaces-root flag, ENACT_WORKSPACES_ROOT env, this field, the
	...
	// ENACT_DAEMON_MAX_CONCURRENT_TASKS env, this field, default.
	...
	// ENACT_DAEMON_POLL_INTERVAL env, this field, DefaultPollInterval.
	...
	// flag, ENACT_DAEMON_HEARTBEAT_INTERVAL env, this field,
	...
	// explicit 0), ENACT_AGENT_TIMEOUT env, this field,
	// DefaultAgentTimeout.
	...
	// ENACT_CODEX_SEMANTIC_INACTIVITY_TIMEOUT env, this field,
	...
	// ENACT_CODEX_HANDSHAKE_TIMEOUT env, this field,
	...
	// already resolves to enabled on Enact Cloud. Absent / false means
	// "let env/default decide". Resolution precedence:
	// --no-auto-update flag, ENACT_DAEMON_AUTO_UPDATE=false env, this
	...
	// --auto-update-interval flag, ENACT_DAEMON_AUTO_UPDATE_INTERVAL
	...
	// enact binary that was replaced on disk out of band. Single-direction
	...
	// --no-auto-reload flag, ENACT_DAEMON_AUTO_RELOAD=false env, this
	...
	// `enact runtime profile set-path`; the daemon prefers it over the
...
//
//	BinaryPath: ENACT_OPENCLAW_PATH (env)  > backends.openclaw.binary_path > PATH lookup
//	StateDir:   OPENCLAW_STATE_DIR (env)     > backends.openclaw.state_dir   > OpenClaw's built-in default (~/.openclaw)
//	CLITimeout: ENACT_OPENCLAW_CLI_TIMEOUT (env) > backends.openclaw.cli_timeout > built-in default
//
// The StateDir env var here is OpenClaw's own OPENCLAW_STATE_DIR — NOT a new
// ENACT_OPENCLAW_STATE_DIR. Rationale: OpenClaw already honors its own env
...
// OpenClaw's own env, ENACT_OPENCLAW_STATE_DIR can be layered on top
...
type OpenClawOverride struct {
	...
	//
	// Resolution mirrors the fields above: ENACT_OPENCLAW_CLI_TIMEOUT (env) >
...
// An empty profile returns the default path (~/.enact/config.json).
// A named profile returns ~/.enact/profiles/<name>/config.json.
...
func CLIConfigPathForProfile(profile string) (string, error) {
	root, taskLocal, err := enactConfigRoot()
	if err != nil {
	...
	return filepath.Join(root, ".enact", "profiles", profile, "config.json"), nil
}

// ProfileDir returns the base directory for a profile's state files (pid, log).
// An empty profile returns ~/.enact/. A named profile returns ~/.enact/profiles/<name>/.
...
func ProfileDir(profile string) (string, error) {
	...
	if profile == "" {
		...
		return filepath.Join(root, ".enact"), nil
	}
	if taskLocal {
		return filepath.Join(root, "profiles", profile), nil
	}
	return filepath.Join(root, ".enact", "profiles", profile), nil
}

func enactConfigRoot() (root string, taskLocal bool, err error) {
...
func validateTaskLocalProfile(profile string) error {
	...
	if profile == "." || profile == ".." || filepath.IsAbs(profile) || strings.ContainsAny(profile, `/\\`) || filepath.Clean(profile) != profile {
		return fmt.Errorf("invalid task-local Enact profile name %q", profile)
	}
	return nil
}
