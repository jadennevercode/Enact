...
func TestClassifyNetworkError(t *testing.T) {
	cases := []struct {
		...
		{"dns", &net.DNSError{Err: "no such host", Name: "api.enact.ai", IsNotFound: true}, KindNetworkDNS},
		...
		{"x509 hostname", x509.HostnameError{Host: "api.enact.ai"}, KindNetworkTLS},
		{"timeout string fallback", errors.New("Get \"https://x\": net/http: request canceled (Client.Timeout exceeded)"), KindNetworkTimeout},
		{"dns string fallback", errors.New("dial tcp: lookup api.enact.ai: no such host"), KindNetworkDNS},
...
func TestFormatErrorNetwork(t *testing.T) {
	...
	raw := errors.New("Get \"https://api.enact.ai/api/issues/abc\": context deadline exceeded")
	...
	if strings.Contains(got, "api.enact.ai") || strings.Contains(got, "resolve issue") {
...
func TestHTTPTimeout(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			t.Setenv("ENACT_HTTP_TIMEOUT", tc.val)
...
func TestFormatErrorActionableHints(t *testing.T) {
	cases := []struct {
		...
		{401, []string{"enact login", "self-hosted", "administrator"}, []string{"enact login", "自托管", "管理员"}},
...
// `enact login` failure guidance visible without losing classification.
func TestUserMessageError(t *testing.T) {
	withLang(t, "en_US.UTF-8")
	const hint = "Could not sign in with that token — make sure it is valid and not expired, then run `enact login --token <token>` again."
	...
	t.Run("wrapped NetworkError classifies as network", func(t *testing.T) {
		...
		err := WithUserMessage("Sign-in did not complete: the server did not accept the new credential. Run `enact login` again.", underlying)
