...
// Set by the enact binary at init() so the package doesn't depend on the
...
// APIClient is a REST client for the Enact server API.
...
// reading a capped slice of the body. Every Enact API helper funnels its
...
// It can be overridden with the ENACT_HTTP_TIMEOUT environment variable
...
// httpTimeout returns the HTTP client timeout, honoring ENACT_HTTP_TIMEOUT.
...
func httpTimeout() time.Duration {
	v := strings.TrimSpace(os.Getenv("ENACT_HTTP_TIMEOUT"))
	if v == "" {
...
// which honors ENACT_HTTP_TIMEOUT) plus a small grace margin, so a
...
// hardcoded duration so the deadline always respects ENACT_HTTP_TIMEOUT.
