...
func TestHelperStatusErrorsAreClassified(t *testing.T) {
	...
	statusCases := []struct {
		...
		{http.StatusUnauthorized, ExitAuth, "enact login"},
...
// context that truncated a longer ENACT_HTTP_TIMEOUT.
func TestCommandContextNotTruncatedBelowHTTPTimeout(t *testing.T) {
	...
	t.Setenv("ENACT_HTTP_TIMEOUT", "400ms")
...
// ENACT_HTTP_TIMEOUT and the AtLeast floor.
func TestAPITimeoutRespectsEnv(t *testing.T) {
	t.Run("default", func(t *testing.T) {
		t.Setenv("ENACT_HTTP_TIMEOUT", "")
	...
	t.Run("AtLeast budget wins when larger than floor", func(t *testing.T) {
		t.Setenv("ENACT_HTTP_TIMEOUT", "120s")
	...
	// responsive, but it must still honor ENACT_HTTP_TIMEOUT (it is not a
