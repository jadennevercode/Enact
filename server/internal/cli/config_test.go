...
func TestCLIConfig_BackwardCompat_OldFileLoadsWithNilBackends(t *testing.T) {
	...
	cfgDir := filepath.Join(tmp, ".enact")
	if err := os.MkdirAll(cfgDir, 0o755); err != nil {
		t.Fatal(err)
	}
	historical := `{
  "server_url": "https://api.enact.ai",
  "app_url": "https://enact.ai",
  "workspace_id": "ws-123",
  "token": "enact_abcdef"
}`
	...
	if cfg.ServerURL != "https://api.enact.ai" {
		t.Errorf("ServerURL: got %q, want historical value", cfg.ServerURL)
	}
	if cfg.Token != "enact_abcdef" {
...
func TestCLIConfig_BackwardCompat_NilBackendsOmittedFromJSON(t *testing.T) {
	...
	data, err := os.ReadFile(filepath.Join(tmp, ".enact", "config.json"))
	if err != nil {
		t.Fatal(err)
	}
	if string(data) == "" {
...
func TestCLIConfig_ProfileCommandOverrides_OmittedWhenEmpty(t *testing.T) {
	...
	cfg := CLIConfig{ServerURL: "https://api.enact.ai", Token: "enact_xyz"}
	if err := SaveCLIConfig(cfg); err != nil {
		t.Fatal(err)
	}
...
func TestCLIConfig_TaskRootOverridesOwnerHome(t *testing.T) {
	ownerHome := t.TempDir()
	taskRoot := filepath.Join(t.TempDir(), "task-enact")
	t.Setenv("HOME", ownerHome)
	t.Setenv("ENACT_TASK_CONFIG_ROOT", taskRoot)
...
func TestCLIConfig_NoTaskRootKeepsInteractiveHomeResolution(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)
	t.Setenv("ENACT_TASK_CONFIG_ROOT", "")

	path, err := CLIConfigPathForProfile("dev")
	if err != nil {
		t.Fatalf("CLIConfigPathForProfile: %v", err)
	}
	want := filepath.Join(home, ".enact", "profiles", "dev", "config.json")
	if path != want {
...
func TestCLIConfig_TaskRootMustBeAbsolute(t *testing.T) {
	t.Setenv("ENACT_TASK_CONFIG_ROOT", "relative/task-enact")
