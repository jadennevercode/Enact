...
func TestRegistrationClient_Begin_HappyPath(t *testing.T) {
	...
	res, err := c.Begin(context.Background(), "Ada - Enact", "")
	if err != nil {
	...
	if !strings.HasPrefix(q.Get("source"), "go-sdk/enact") {
		t.Errorf("qr source=%q want go-sdk/enact", q.Get("source"))
	}
	// The name preset pre-fills the Lark PersonalAgent creation form so
	// the bot defaults to "<agent> - Enact" rather than the
	// auto-generated "{用户姓名}的智能助手".
	if q.Get("name") != "Ada - Enact" {
		t.Errorf("qr name=%q want %q", q.Get("name"), "Ada - Enact")
	}
}
