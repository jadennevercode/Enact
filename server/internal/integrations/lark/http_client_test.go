...
func TestHTTPClient_SendBindingPromptCard_HappyPath(t *testing.T) {
	...
	if err := c.SendBindingPromptCard(context.Background(), BindingPromptParams{
		...
		BindURL:        "https://enact.test/lark/bind?token=abc",
	}); err != nil {
	...
	if !strings.Contains(capturedBody["content"], "enact.test/lark/bind") {
...
func TestBindingPromptTemplate_Shape(t *testing.T) {
	raw, err := bindingPromptTemplate("https://enact.test/bind?token=abc")
	if err != nil {
	...
	if btn["url"] != "https://enact.test/bind?token=abc" {
