...
// ENACT_LARK_CALLBACK_BASE_URL still overrides every installation when
// set (staging / mock).
...
// `enact` -> register a PersonalAgent -> enable WS -> watch logs.
...
func (c HTTPConnectionTokenConfig) withDefaults() HTTPConnectionTokenConfig {
	...
	// (ENACT_LARK_CALLBACK_BASE_URL, or an httptest URL in tests)
...
func (f *HTTPConnectionTokenFetcher) Endpoint(ctx context.Context, creds InstallationCredentials) (WSEndpoint, error) {
	...
	// the audience Enact server logs are read by today; if i18n
