...
type LarkOutcomeReplier struct {
	...
	appURL       string // e.g. https://enact.example, trailing slash trimmed
...
// OutcomeReplierConfig wires the production replier. AppURL is the Enact web
// app host the user clicks into to redeem the binding token or open an issue
// (e.g. https://enact.example). It comes from ENACT_APP_URL and is
// intentionally separate from ENACT_PUBLIC_URL, which is the backend/API
...
func NewLarkOutcomeReplier(cfg OutcomeReplierConfig) OutcomeReplier {
	...
	if cfg.AppURL == "" {
		log.Warn("lark outcome replier: ENACT_APP_URL not set; binding prompt CTA will not work")
	}
...
// when empty (self-host operators who haven't configured ENACT_APP_URL) the
