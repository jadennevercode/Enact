...
type InstallerBindParams struct {
	WorkspaceID    pgtype.UUID
	InstallationID pgtype.UUID
	EnactUserID  pgtype.UUID // the installer's Enact account
...
// identity is the supplied enactUserID (taken from the session by
...
//     (installation, open_id), pointing at a DIFFERENT Enact user.
...
func (s *BindingTokenService) RedeemAndBind(ctx context.Context, raw string, enactUserID pgtype.UUID) (RedeemedBindingToken, error) {
	...
	isMember, err := qtx.IsWorkspaceMember(ctx, row.WorkspaceID, enactUserID)
	if err != nil {
...
// open_id) pair and points at a different Enact user. Account
