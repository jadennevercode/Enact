	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			rep := NewLarkOutcomeReplier(OutcomeReplierConfig{
				...
				Queries: stubReplierQueries{}, AppURL: "https://enact.test", Logger: log,
			})
...
func TestLarkOutcomeReplierUsesAppURLForWebLinks(t *testing.T) {
	...
	if got := stub.bindingCalls[0].BindURL; got != "https://app.enact.test/lark/bind?token=token+with+space" {
