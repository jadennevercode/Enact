...
func (s *RegistrationService) BeginInstall(ctx context.Context, p BeginInstallParams) (BeginInstallResult, error) {
	...
	// bot reads "<agent> - Enact" instead of "{用户姓名}的智能助手".
...
// "connected to a different Enact workspace" that lied when the real owner sat
...
func (s *RegistrationService) liveOwnerConflictMessage(ctx context.Context, requestingWorkspaceID pgtype.UUID, appID string) string {
	...
	case owner.WorkspaceID != requestingWorkspaceID:
		return "This Feishu app is already connected to a different Enact workspace. Disconnect it there before connecting it here."
...
// "<agent> - Enact" instead of Lark's auto-generated
...
// "Enact" rather than a dangling " - Enact".
