...
func TestReplyNeedsBindingDoesNotMintBearerLinkInGroup(t *testing.T) {
	...
	if strings.Contains(got.Text, "secret-token") || strings.Contains(got.Text, "enact.example") {
...
func TestReplyNeedsBindingInPrivateChatMintsQuotedLink(t *testing.T) {
	...
	r := NewOutboundReplier(OutboundReplierConfig{
		Binding: minter, AppURL: "https://enact.example/", APIBase: srv.URL,
	...
	if minter.calls != 1 || !strings.Contains(got.Text, "https://enact.example/telegram/bind?token=one-time-token") {
