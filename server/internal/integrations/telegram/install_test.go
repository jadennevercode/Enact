...
func telegramInstallAPIServer(t *testing.T, webhookURL string) *httptest.Server {
	...
	return httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case strings.HasSuffix(r.URL.Path, "/getMe"):
			_, _ = w.Write([]byte(`{"ok":true,"result":{"id":12345,"is_bot":true,"first_name":"Enact","username":"enact_test_bot"}}`))
...
func TestRegisterValidatesAndEncryptsBotToken(t *testing.T) {
	...
	if cfg.AppID != "12345" || cfg.BotUsername != "enact_test_bot" || strings.Contains(cfg.BotTokenEncrypted, "secret") {
