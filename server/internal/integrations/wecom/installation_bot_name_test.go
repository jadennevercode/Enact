...
VALUES ($1, $2, 'wecom bot name runtime', 'local', 'enact_daemon') ON CONFLICT (id) DO NOTHING`, wcNameRuntime, wcNameWS)
...
func TestRotatingTheSecretKeepsTheBotName(t *testing.T) {
	...
	if _, err := svc.Upsert(ctx, botNameParams(wcNameBotA, "Enact Bot")); err != nil {
	...
	if again.BotDisplayName != "Enact Bot" {
...
func TestTheBotNameSurvivesTheConfigRoundTrip(t *testing.T) {
	t.Parallel()
	cfg, err := encodeInstallConfig(Installation{BotID: "bot-xyz", BotDisplayName: "Enact Bot"})
	if err != nil {
	...
	if out.BotDisplayName != "Enact Bot" {
		t.Fatalf("BotDisplayName round trip = %q, want Enact Bot", out.BotDisplayName)
	}
}
