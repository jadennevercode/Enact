...
// page, and their WeCom userid is bound to their Enact account. Mirrors
...
var (
	...
	// different Enact user (account transfer must go through explicit
...
func (s *BindingTokenService) RedeemAndBind(ctx context.Context, raw string, enactUserID pgtype.UUID) (RedeemedBindingToken, error) {
	...
	if _, err := qtx.CreateChannelUserBinding(ctx, db.CreateChannelUserBindingParams{
		...
		// user — the ON CONFLICT DO UPDATE WHERE enact_user_id=… gating
		// rejected it.
		if errors.Is(err, pgx.ErrNoRows) {
