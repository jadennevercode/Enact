...
//
// With ENACT_WECOM_TRACE=1 the server records enough to check a real-device
...
// ENACT_WECOM_TRACE; returns what it set so the caller can log it.
...
// "👋 请先绑定你的 Enact 账号，才能与我对话：\n" + appURL + "/wecom/bind?token=" +
// a 43-character token, and with a normal ENACT_APP_URL the token's last
...
// could bind that sender's WeCom identity to their own Enact account before
