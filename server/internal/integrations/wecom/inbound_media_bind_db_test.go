...
func seedMediaBindFixture(t *testing.T, pool *pgxpool.Pool) mediaBindFixture {
	...
	if err := pool.QueryRow(ctx, `INSERT INTO "user" (name, email) VALUES ($1, $2) RETURNING id`,
		"wecom media bind", fmt.Sprintf("wecom-media-bind-%d@enact.test", suffix)).Scan(&f.userID); err != nil {
	...
	if err := pool.QueryRow(ctx, `
		...
		VALUES ($1, $2, 'local', 'enact_daemon', $3) RETURNING id`,
...
// the front of the text run: "@Enact Bot /issue 登录坏了". main strips that
...
// the parser sees "@Enact Bot"; derive the command from the resolved body
...
func TestGroupMentionedIssueAfterAnImageIsStillFiled(t *testing.T) {
	issues, body := runMixedIssueThroughTheRouter(t, "Enact Bot", "group", "GROUP-BIND-1", "@Enact Bot /issue 登录坏了", false)
	...
	if want := "[Image]\n@Enact Bot /issue 登录坏了"; body != want {
...
func TestGroupMentionedSpokenIssueAfterAnImageIsStillFiled(t *testing.T) {
	issues, body := runMixedIssueThroughTheRouter(t, "Enact Bot", "group", "GROUP-BIND-2", "@Enact Bot /issue 登录坏了", true)
