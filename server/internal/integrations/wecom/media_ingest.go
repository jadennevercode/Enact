	...
	for i, m := range wm.Media {
		...
		if err != nil {
			...
			if failure == mediaFailureBlocked {
				logLine = "wecom media ingest refused by the media address guard: the host resolved to a non-public address and was not dialed. If this deployment sits behind a fake-IP proxy, declare its range in ENACT_WECOM_MEDIA_ALLOW_CIDRS; otherwise this is a URL that should not have been sent."
			}
