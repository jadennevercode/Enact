...
//
// `enact attachment upload` already bound the file to the assistant message;
...
// WeCom can be continued from the Enact web UI, and that answer's artifacts
// belong in Enact — pushing them into a group chat puts a file in front of
...
func TestProcessEvent_AWebUIAnswerDeliversNeitherWordsNorFile(t *testing.T) {
	...
	q.channelIngested = askedInTheWebUI() // asked in Enact, not over WeCom
