...
// trace_test.go — guards for the ENACT_WECOM_TRACE operator switch. Six
...
// with ENACT_WECOM_TRACE unset, a real inbound message and the frames around
...
// and puts it in a URL inside the message body; with a normal ENACT_APP_URL
...
func TestTraceNeverLogsABindingToken(t *testing.T) {
	...
	r := NewOutboundReplier(OutboundReplierConfig{Senders: reg, AppURL: "https://enact.example"})
...
func TestBindingPromptFitsInsideThePreviewCap(t *testing.T) {
	...
	bindURL := "https://enact.example" + "/wecom/bind" + "?token=" + url.QueryEscape(rawToken)
	upToTokenEnd := "👋 请先绑定你的 Enact 账号，才能与我对话：\n" + bindURL
...
// cut lands inside an escape — so an operator who turned ENACT_WECOM_TRACE
