...
// mention is delivered as part of the message text: "@Enact Bot /issue 登录
...
// "@Enact" and decide this is ordinary prose. Every slash command in every
...
// TestAMultiWordBotNameDoesNotSwallowTheCommand: a bot named "Enact Bot" is
// mentioned as "@Enact Bot", and cutting the mention at the first space
...
func TestAMultiWordBotNameDoesNotSwallowTheCommand(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"two-word name", "Enact Bot", "@Enact Bot /new 重新分析这批数据", "/new 重新分析这批数据"},
		...
		{"a colleague's mention before ours", "Enact Bot", "@李雷 @Enact Bot /new 分析", "/new 分析"},
...
func TestAMultiWordBotNameReachesTheParser(t *testing.T) {
	t.Parallel()
	msg := dispatchGroupMessage(t, "Enact Bot", "@Enact Bot /issue 登录失败")
	cmd, ok := engine.ParseIssueCommand(msg.CommandText)
...
func TestAConfiguredNameDoesNotManufactureACommand(t *testing.T) {
	...
	got := stripLeadingMentions(in, "Enact Bot")
...
// "@Enact Bot /issue …" again. Same two lines as
...
func TestSessionBinder_AdapterCommandTextWins(t *testing.T) {
	...
	if fb.appendIn.Body != "@Enact Bot /issue 登录失败" {
