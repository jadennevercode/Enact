// Package composio is the Stage 2 business-integration glue between Enact and
// the standalone Composio SDK (server/pkg/composio). It owns Enact semantics:
...
import (
	...
	sdk "github.com/enact-ai/enact/server/pkg/composio"
...
type Config struct {
	...
	// trailing slash (e.g. "https://enact.ai"). The Composio callback
	...
	// browser redirect (e.g. "https://enact.ai"). May be empty, in which
...
//
// The composio_user_id sent to Composio is the Enact user id verbatim — the
...
func (s *Service) CompleteCallback(ctx context.Context, state, status, connectedAccountID string) (string, error) {
	...
	// composio_user_id == enact user id invariant) and was created under the
	...
	if _, err := s.store.UpsertUserComposioConnection(ctx, db.UpsertUserComposioConnectionParams{
		...
		// Invariant: composio_user_id == Enact user id.
