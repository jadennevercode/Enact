...
type Source struct {
	...
	// one Enact chat_session via the channel_chat_session_binding.
	ChatID string
...
type MediaRef struct {
	...
	Type MsgType
	// StorageKey locates the persisted object in Enact object storage.
	StorageKey string
...
// fields. Per the boundary rule (ENA-3515 §2) the struct holds only
...
// webhooks are deliberately NOT modeled here (ENA-3515 decision §6) — an
