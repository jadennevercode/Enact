...
// `enact chat history` (the channel OVERVIEW: top-level messages + thread
// metadata, not thread contents) and `enact chat thread [id]` (one thread's
...
// platform so the agent reads a uniform list, like `enact issue comment list
// --output json`.
type HistoryMessage struct {
	...
	// so the agent can `enact chat thread <thread_id>` to read its contents.
	// They are absent on a plain message and on thread-read rows.

	// ThreadID is the identifier to pass to `enact chat thread <id>` to read
