...
func TestRedisLeaseStoreReadyPingsAndLoadsScripts(t *testing.T) {
	...
	mock.Regexp().ExpectEvalSha(redisTryAcquireLease.Hash(), []string{`enact:channel-lease:v1:prod:__ready__:.+`}, "readiness-check", int64(5000)).SetVal(int64(1))
	mock.Regexp().ExpectEvalSha(redisRenewLease.Hash(), []string{`enact:channel-lease:v1:prod:__ready__:.+`}, "readiness-check", int64(5000)).SetVal(int64(1))
	mock.Regexp().ExpectEvalSha(redisReleaseLease.Hash(), []string{`enact:channel-lease:v1:prod:__ready__:.+`}, "readiness-check").SetVal(int64(1))
...
func TestRedisLeaseStoreAtomicOperationsAreTokenFenced(t *testing.T) {
	...
	key := "enact:channel-lease:v1:prod:12121212-1212-1212-1212-121212121212"
