...
const (
	...
	issueNotMemberText = "You're not a member of this Enact workspace, so I can't file an issue for you. Ask a workspace admin to invite you, then send the command again."
	issueDisabledText  = "This DingTalk robot isn't connected to Enact (or was disconnected). Ask the agent owner or a workspace owner/admin to reconnect it."
)
...
type OutboundReplierConfig struct {
	...
	// binding token (e.g. https://enact.example). The bind page (/dingtalk/bind)
...
func (r *OutboundReplier) sendBindingPrompt(ctx context.Context, inst engine.ResolvedInstallation, msg channel.InboundMessage, res engine.Result) error {
	...
	text := "👋 To start chatting with me, link your DingTalk account to Enact: [link your account](" +
