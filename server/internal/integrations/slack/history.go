...
// failed) read so the unified `enact chat history` / `enact chat thread`
...
// `enact chat history` (channel overview) and `enact chat thread [id]`
// (one thread) commands (ENA-3871). Both are scoped to the session's OWN
...
// boundary for `enact chat thread <id>` (the agent supplies only a
...
// drills into with `enact chat thread <id>`. Backs `enact chat history`.
...
// threads) reads its linear conversation. Backs `enact chat thread [id]`.
...
// agent can drill in with `enact chat thread <id>`.
...
// dropped (ENA-3931 / #4803). Order: top-level text, then each attachment's
