...
// The agent turns the prompt into a well-formed `enact issue create` in the
...
// completion surfaces to the invoker as a Enact inbox notification through the
...
const (
	...
	slashQueuedText          = "✅ On it — I'm turning that into an issue. You'll get a Enact notification when it's ready."
	slashNotMemberText       = "You're not a member of this Enact workspace, so I can't file an issue for you."
	slashLinkAccountFallback = "Link your Slack account to Enact first, then try `/issue` again."
	slashInternalErrorText   = "⚠️ Something went wrong creating the issue. Please try again."
	slashDisabledText        = "This Slack app isn't connected to Enact (or was disconnected). Ask a workspace admin to reconnect it."
)
...
type quickCreateEnqueuer interface {
	EnqueueQuickCreateTask(ctx context.Context, workspaceID, requesterID, agentID, squadID pgtype.UUID, prompt, priority, dueDate string, parentIssueID pgtype.UUID, attachmentIDs []pgtype.UUID) (db.AgentTaskQueue, error)
}
...
func (p *SlashCommandProcessor) process(ctx context.Context, cmd slack.SlashCommand) string {
	...
	// background and attributes it to the bound member. No parent /
...
// resolveUser maps the Slack user id to the bound Enact user, re-checking
...
func (p *SlashCommandProcessor) resolveUser(ctx context.Context, inst engine.ResolvedInstallation, slackUserID string) (pgtype.UUID, error) {
	...
	return binding.EnactUserID, nil
}
...
func (p *SlashCommandProcessor) bindingText(ctx context.Context, inst engine.ResolvedInstallation, slackUserID string) string {
	...
	return "👋 To file issues, link your Slack account to Enact: <" +
