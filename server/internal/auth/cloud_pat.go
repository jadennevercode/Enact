...
// (Enact Cloud Node) PAT. Tokens with this prefix are validated by
// calling the Enact Cloud Fleet service rather than by hitting our
...
// enact_/mdt_ caches so the three token kinds can't accidentally share
// keys. The trailing slash mirrors the existing patCachePrefix /
// daemonTokenCachePrefix conventions.
const cloudPATCachePrefix = "enact:auth:mcn:"
...
// ENACT_CLOUD_FLEET_URL configured simply rejects mcn_ tokens at
...
type CloudPATVerifierConfig struct {
	...
	// https://fleet.enact.cloud). Trailing slashes are trimmed.
