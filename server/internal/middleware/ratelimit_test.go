...
func TestExtractIP_IPv6Normalization(t *testing.T) {
	req1 := httptest.NewRequest(http.MethodPost, "/auth/email-login", nil)
	req1.RemoteAddr = "[::1]:9000"

	req2 := httptest.NewRequest(http.MethodPost, "/auth/email-login", nil)
...
func TestRateLimit_LuaScript_SetsTTL(t *testing.T) {
	...
	key := rateLimitKey("/auth/email-login", "10.0.99.1")
