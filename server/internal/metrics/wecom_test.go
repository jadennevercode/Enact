...
func TestEveryWecomCounterActuallyCounts(t *testing.T) {
	...
	for _, want := range []string{
		"enact_wecom_connect_failures_total",
		"enact_wecom_auth_failures_total",
		"enact_wecom_inbound_callbacks_total",
		"enact_wecom_inbound_queue_blocked_total",
	} {
		if seen[want] != 1 {
...
func TestAuthAndConnectFailuresAreSeparateSeries(t *testing.T) {
	...
	seen := gatherWecomValues(t, reg)
	if got := seen["enact_wecom_auth_failures_total"]; got != 2 {
		t.Errorf("auth failures = %v, want 2", got)
	}
	if got := seen["enact_wecom_connect_failures_total"]; got != 1 {
...
func TestTheRegistryExposesTheWecomCounters(t *testing.T) {
	...
	for _, f := range families {
		if f.GetName() == "enact_wecom_auth_failures_total" {
			return
		}
	}
	t.Fatal("enact_wecom_auth_failures_total is not on the registry the metrics server scrapes")
}
