...
-- lark_installation
-- =====================
-- One row per (workspace, agent) — each Enact Agent owns at most one
...
-- lark_user_binding
-- =====================
-- Maps a Lark `open_id` to a Enact user, per-installation. open_id is
...
-- =====================
-- One Lark chat (`chat_id`) ↔ one Enact `chat_session`. The Lark side
...
-- =====================
-- Maps a Enact task (or session bootstrap "thinking…" card) to the
...
-- click here" flow that links a Lark `open_id` to a Enact user. The
